...
def validate(root: Path):
    master = root / 'wave53_57_master_itemized_list.csv'
    source_index = root / 'Coverage_Audit/ultra_blueprint_source_section_index.csv'
    schema = json.loads((root / 'Schemas/item_row_schema.json').read_text(encoding='utf-8'))
    ...
    with master.open(newline='', encoding='utf-8-sig', errors='ignore') as f:
        ...
        for row in reader:
            ...
            if row.get('Source_Key'):
                item_source_keys.add(row['Source_Key'])

    expected_waves = {'53','54','55','56','57'}
    ...
    missing_source_keys = sorted(source_keys - item_source_keys)
    if missing_source_keys:
        errors.append(f'missing Ultra source coverage keys: {len(missing_source_keys)}')

    report = {
        'package': 'Items Waves 53-57 Coverage Verified',
        'row_count': row_count,
    ...
    (root / 'Reports/items_validation_report.json').write_text(json.dumps(report, indent=2) + '\n', encoding='utf-8')
