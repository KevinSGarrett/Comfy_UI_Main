...
EVIDENCE_ID = f"W70_MF70_LEFT_UPPER_ARM_{RUN_STAMP}"
SCRIPT_REL = "Plan/07_IMPLEMENTATION/scripts/implement_wave70_mf70_left_upper_arm.py"

QA_DIR = PROJECT_ROOT / "Plan/Instructions/QA/Evidence/Mask_Factory/Wave70"
TRACKER_EVIDENCE_DIR = PROJECT_ROOT / "Plan/Tracker/Evidence"
RUNTIME_DIR = PROJECT_ROOT / "runtime_artifacts/mask_factory/wave70_mf70_left_upper_arm" / RUN_STAMP
...
def evaluate_left_upper_arm_readiness(prereqs: dict[str, object], width: int, height: int) -> dict[str, object]:
    source_visibility = {
        "source_framing": "head_neck_upper_chest_jacket_portrait",
        "image_dimensions": [width, height],
        "left_upper_arm_visible_as_source_body_geometry": False,
        "left_shoulder_to_elbow_chain_visible": False,
    ...
    required_passes = {
        "source_left_upper_arm_visible": source_visibility["left_upper_arm_visible_as_source_body_geometry"],
        "source_left_shoulder_to_elbow_chain_visible": source_visibility["left_shoulder_to_elbow_chain_visible"],
...
def make_blocker_panel(source: Image.Image, evaluation: dict[str, object]) -> Path:
    ...
    panel_path = RUNTIME_DIR / "mf70_left_upper_arm_blocker_panel.png"
...
def update_wave70_rows(evidence_paths: list[str], note: str) -> dict[str, int]:
    ...
    targets = [(path, "TRK-W70-0155") for path in TRACKER_FILES] + [(path, "ITEM-W70-0155") for path in ITEM_FILES]
    for csv_path, target_id in targets:
        ...
        for row in rows:
            ...
            if "Status_Decision" in row:
                row["Status_Decision"] = "blocked_exact_local_left_upper_arm_not_source_visible"
            if "Coverage_Audit_Status" in row:
                row["Coverage_Audit_Status"] = append_unique(
                    row.get("Coverage_Audit_Status", ""),
                    ["blocked_exact_local_left_upper_arm_not_source_visible"],
                )
            if "Notes" in row:
...
Next highest-value local tracker row found from current CSV state is TRK-W70-0156 / ITEM-W70-0156, prove Mask Factory mask type mf70_right_upper_arm. Work it locally under the same authority rules; if source/authority prerequisites remain blocked, write exact local blocker evidence and keep masks fail-closed."""
    next_body = f"""TRK-W70-0155 / ITEM-W70-0155 is exactly blocked with local source-visibility and whole-body authority evidence. The active portrait does not expose a source-derived left upper-arm region or shoulder-to-elbow chain.
...
Next local task: implement or exactly block TRK-W70-0156 / ITEM-W70-0156, mf70_right_upper_arm. Use only source-derived whole-body/model-backed geometry evidence. Do not start EC2, run generated-output proof, checkpoint Git, rerun broad helper loops, or promote any mask."""
    session_body = (
        "Worked TRK-W70-0155 / ITEM-W70-0155 locally. mf70_left_upper_arm is blocked because the "
        "active source crop shows jacket shoulder/upper torso but no source-derived left upper-arm or "
    ...
    blocker_body = (
        "TRK-W70-0155 / ITEM-W70-0155: Blocked_Body_Part_Not_Visible / "
        "blocked_exact_local_left_upper_arm_not_source_visible."
    )

    prepend_section(HYDRATION_DIR / "CURRENT_PURSUING_GOAL.md", f"## Wave70 Left Upper Arm Blocked Locally - {ISO_STAMP}", current_body)
    prepend_section(HYDRATION_DIR / "NEXT_ACTION.md", f"## Immediate Next Action - {ISO_STAMP} - Work TRK-W70-0156 Right Upper Arm Locally", next_body)
    prepend_section(HYDRATION_DIR / "RESUME_HERE_NEXT_CODEX_SESSION.md", f"## Resume Update - Wave70 Left Upper Arm Blocked - {ISO_STAMP}", session_body + "\n\nNext exact action: work TRK-W70-0156 locally.")
    prepend_section(HYDRATION_DIR / "CURRENT_SESSION_STATE.md", f"## Session State Update - Wave70 Left Upper Arm Blocked - {ISO_STAMP}", session_body)
    prepend_section(HYDRATION_DIR / "QA_EVIDENCE_INDEX.md", f"## Wave70 Left Upper Arm Evidence - {ISO_STAMP}", f"{session_body}\n\n{evidence_block}")
    prepend_section(HYDRATION_DIR / "BLOCKERS.md", f"## Wave70 Left Upper Arm Blocker - {ISO_STAMP}", blocker_body + "\n\n" + evidence_block)
    ...
    with proof_log.open("a", newline="", encoding="utf-8") as handle:
        ...
        writer.writerow(
            [
                ISO_STAMP,
                "70",
                "Wave70 mf70 left upper arm blocker",
                "Worked TRK/ITEM-W70-0155 locally. Active source does not expose source-derived left upper-arm or shoulder-to-elbow geometry and whole-body/model-backed authority remains blocked. Wrote stamped/canonical evidence, generated blocker panel, and kept masks fail-closed.",
                "; ".join(evidence_paths),
                ...
                "Next work TRK-W70-0156 right upper arm locally; write exact blocker if authority remains blocked.",
            ]
        )
...
def main() -> int:
    ...
    evaluation = evaluate_left_upper_arm_readiness(prereqs, width, height)
    panel_path = make_blocker_panel(source, evaluation)

    qa_evidence_path = QA_DIR / f"{EVIDENCE_ID}.json"
    qa_canonical_path = QA_DIR / "mf70_left_upper_arm.json"
    tracker_evidence_path = TRACKER_EVIDENCE_DIR / f"{EVIDENCE_ID}.json"
    tracker_canonical_path = TRACKER_EVIDENCE_DIR / "mf70_left_upper_arm.json"
    runtime_evidence_path = RUNTIME_DIR / "mf70_left_upper_arm.json"
    ...
    note = (
        f"mf70_left_upper_arm {RUN_STAMP}: exact local blocker. "
        "The active source crop shows jacket shoulder/upper torso but no source-derived left upper-arm or "
    ...
    payload = {
        ...
        "task": "Prove or exactly block Mask Factory mask type mf70_left_upper_arm for TRK-W70-0155 / ITEM-W70-0155.",
        "script": SCRIPT_REL,
        "source_image": {
        ...
        "left_upper_arm_readiness": evaluation,
        "artifacts": {
        ...
        "mask_factory_row": {
            "mask_type_id": "mf70_left_upper_arm",
            "target_region": "left_upper_arm",
            "body_part": "arms",
            "subregion": "left_upper_arm",
        ...
        "whole_body_geometry_authority": {
            ...
            "findings": [
                ...
                "The visible jacket shoulder cannot prove the requested left upper-arm source geometry.",
                "No left shoulder-to-elbow chain, elbow anchor, or unoccluded left upper-arm body surface is visible.",
        ...
        "qa_decision": "blocked_exact_local_left_upper_arm_not_source_visible",
        "promotion_decision": "no_mask_promoted_no_active_input_changed_left_upper_arm_blocked",
        "tracker_item_updates": row_updates,
        "next_step": "Work TRK-W70-0156 / ITEM-W70-0156 mf70_right_upper_arm locally.",
    }
