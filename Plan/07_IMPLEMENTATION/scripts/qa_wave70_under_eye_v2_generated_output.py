...
RUN_STAMP = "20260707T234500-0500"
TIMESTAMP = "2026-07-07T23:45:00-05:00"
...
STATUS_DECISION = "under_eye_v2_strict_visual_alignment_pass_generated_output_safe_target_runtime_pending_reference_matrix_pending"
...
CANDIDATE_OVERLAY = Path("Plan/Instructions/Operations/Prepared_Input_Assets/wave70_mf70_under_eye_v2_20260707T232500-0500/wave70_mf70_under_eye_v2_overlay.png")
CANDIDATE_MASK = Path("Plan/Instructions/Operations/Prepared_Input_Assets/wave70_mf70_under_eye_v2_20260707T232500-0500/wave70_mf70_under_eye_v2_mask.png")
STRICT_REPAIR = Path("Plan/Instructions/QA/Evidence/Mask_Factory/Wave70/W70_MF70_UNDER_EYE_V2_SOURCE_LANDMARK_REPAIR_20260707T232500-0500.json")
PROMPT_PROFILE = Path("PromptProfiles/base_generation/inpaint_detail_v4_robustness/inpaint_wave70_mf70_under_eye_seed210814.json")
RUN_PACKAGE = Path("runtime_artifacts/run_packages/wave70_mf70_under_eye_seed210814/RUN_PACKAGE_MANIFEST.json")
RUNTIME_EVIDENCE = Path("Plan/Instructions/QA/Evidence/Workflow_Runtime/W70_LOCAL_MF70_UNDER_EYE_V2_SEED210814_EXECUTE_20260707T233500-0500.json")
PULLBACK_DIR = Path("Plan/Instructions/Operations/Pulled_Back_Artifacts/wave70_mf70_under_eye_v2_seed210814_20260707T233500-0500")
GENERATED_OUTPUT = PULLBACK_DIR / "images/codex_wave70_mf70_under_eye_seed210814_00002_.png"
MASK_PREVIEW = PULLBACK_DIR / "images/codex_sdxl_realvisxl_inpaint_detail_micro_nomouth_v4_mask_preview_00028_.png"
...
def make_comparison(root: Path) -> str:
    ...
    tiles = [
        ...
        label_tile(overlay.crop(crop), "v2 overlay"),
    ...
    out_dir = root / "runtime_artifacts/mask_factory/wave70_mf70_under_eye/qa_comparisons"
    out_dir.mkdir(parents=True, exist_ok=True)
    panel_path = out_dir / "wave70_mf70_under_eye_v2_source_landmark_source_overlay_output_compare.png"
...
def update_ledgers(root: Path, visual_qa_rel: str, tracker_rel: str, comparison_rel: str) -> None:
    note = (
        " Under-eye v2 generated-output proof executed locally after the old mask failed protected-overlap. "
        "Strict whole-image QA passed with notes: eyes/gaze/lids, under-eye skin, nose, mouth, hair, blazer, lighting, and background remain stable. "
...
def update_mask_qa(root: Path, visual_qa_rel: str, tracker_rel: str, comparison_rel: str) -> None:
    ...
    repair = data.setdefault("source_landmark_v2_repair", {})
...
def update_hydration(root: Path, visual_qa_rel: str, tracker_rel: str, comparison_rel: str) -> None:
    section = f"""## Wave70 mf70_under_eye V2 Local Generated-Output Proof - {TIMESTAMP}

Ran one bounded local ComfyUI generated-output proof for the repaired `mf70_under_eye` v2 mask. Runtime evidence is `{RUNTIME_EVIDENCE.as_posix()}`. Strict visual QA is `{visual_qa_rel}`, tracker evidence is `{tracker_rel}`, and comparison panel is `{comparison_rel}`.
...
"""
    ...
    qa_index_row = (
        f"| W70-LOCAL-MF70-UNDER-EYE-V2-SEED210814-VISUAL-QA-{RUN_STAMP} | "
        "Repaired mf70_under_eye v2 local generated-output proof passed strict whole-image QA with notes; target-runtime/reference-matrix proof remains pending | "
        "image_artifact_qa | pass_with_notes_candidate_local_only | "
        f"{visual_qa_rel} |"
    )
    append_unique_text(root / "Plan/Instructions/Hydration_Rehydration/QA_EVIDENCE_INDEX.md", qa_index_row, f"W70-LOCAL-MF70-UNDER-EYE-V2-SEED210814-VISUAL-QA-{RUN_STAMP}")
    append_unique_csv_row(
        root / "Plan/Instructions/Hydration_Rehydration/PROOF_OF_MOVEMENT_LOG.csv",
        [
            TIMESTAMP,
            "70",
            "mf70_under_eye v2 local generated-output proof",
            "Executed one bounded local ComfyUI sample using the repaired v2 under-eye mask and recorded strict whole-image QA pass with notes.",
        ...
        f"W70_LOCAL_MF70_UNDER_EYE_V2_SEED210814_{RUN_STAMP}",
    )
...
def main() -> int:
    ...
    visual_qa_path = root / "Plan/Instructions/QA/Evidence/Image_Artifact_QA" / f"W70_LOCAL_MF70_UNDER_EYE_V2_SEED210814_VISUAL_QA_{RUN_STAMP}.json"
    tracker_path = root / "Plan/Tracker/Evidence" / f"W70_MF70_UNDER_EYE_V2_GENERATED_OUTPUT_{RUN_STAMP}.json"
    visual_qa_rel = rel(visual_qa_path, root)
    tracker_rel = rel(tracker_path, root)

    visual_qa = {
        "schema_version": "1.0",
        "evidence_id": f"W70-LOCAL-MF70-UNDER-EYE-V2-SEED210814-VISUAL-QA-{RUN_STAMP}",
        "timestamp": TIMESTAMP,
        "project_root": str(root),
        "qa_type": "strict_whole_image_visual_qa_for_wave70_mf70_under_eye_v2_generated_output",
        ...
        "visual_findings": [
            ...
            "Eyes, gaze direction, iris/sclera/catchlights, lower-lid edges, and eyebrow shape remain stable.",
            "Under-eye skin remains natural; no swollen-eye, heavy-bag, dark-circle, visible seam, or over-smoothed patch is visible.",
            "Nose, mouth/lips, philtrum, cheeks, hair, blazer, lighting, and gray background remain stable.",
            "Diagnostic mask preview confirms the local run used the repaired lower/narrower v2 under-eye mask.",
        ...
        "strict_whole_image_qa": {
            ...
            "eyes_gaze_safe": True,
            "lower_lids_safe": True,
            "under_eye_skin_safe": True,
            "nose_mouth_cheeks_safe": True,
        ...
        "result": "pass_with_notes_local_wave70_under_eye_v2_generated_output",
    ...
    write_json(
        ...
        {
            "schema_version": "1.0",
            "tracker_evidence_id": f"W70_MF70_UNDER_EYE_V2_GENERATED_OUTPUT_{RUN_STAMP}",
            ...
            "actual_work_performed": [
                "Executed one bounded local ComfyUI generated-output proof for repaired mf70_under_eye v2 mask.",
            ...
            "next_action": "Repair the next downgraded Wave70 mask with the same fail-closed standard.",
        },
    )
