...
RUN_STAMP = "20260707T190000-0500"
MASK_TYPE_ID = "mf70_eyebrows"
TRACKER_ID = "TRK-W70-0016"
ITEM_ID = "ITEM-W70-0016"
CONTRACT_ID = "mask_contract_wave70_mf70_eyebrows"
SCENE_ID = "scene_wave70_canny_v3_eyebrows"
MASK_ID = "scene_wave70_canny_v3__person_001__mf70_eyebrows__minor"
RESULT = "pass_local_wave70_mask_artifact_routing_support_final_blocked_generated_output_target_runtime"
SOURCE_REQUIREMENT = "Plan/07_IMPLEMENTATION/mask_factory/ULTIMATE_MASK_FACTORY_TAXONOMY.md#L47"
...
def eyebrow_polygons(width: int, height: int) -> list[list[tuple[int, int]]]:
    return [
        [
            (int(width * 0.362), int(height * 0.374)),
            (int(width * 0.392), int(height * 0.363)),
            (int(width * 0.444), int(height * 0.367)),
            (int(width * 0.467), int(height * 0.381)),
            (int(width * 0.458), int(height * 0.394)),
            (int(width * 0.410), int(height * 0.386)),
            (int(width * 0.371), int(height * 0.386)),
        ],
        [
            (int(width * 0.514), int(height * 0.369)),
            (int(width * 0.543), int(height * 0.357)),
            (int(width * 0.594), int(height * 0.362)),
            (int(width * 0.617), int(height * 0.377)),
            (int(width * 0.608), int(height * 0.390)),
            (int(width * 0.558), int(height * 0.382)),
            (int(width * 0.523), int(height * 0.381)),
        ],
    ]
...
def protected_eye_boxes(width: int, height: int) -> list[tuple[int, int, int, int]]:
    return [
        (int(width * 0.372), int(height * 0.392), int(width * 0.456), int(height * 0.458)),
        (int(width * 0.517), int(height * 0.386), int(width * 0.603), int(height * 0.452)),
    ]


def protected_forehead_hairline_boxes(width: int, height: int) -> list[tuple[int, int, int, int]]:
    return [
        (0, 0, width, int(height * 0.340)),
        (0, 0, int(width * 0.340), height),
        (int(width * 0.635), 0, width, height),
        (0, int(height * 0.410), width, height),
    ]
...
def make_mask(source: Image.Image) -> Image.Image:
    ...
    for poly in eyebrow_polygons(width, height):
        draw.polygon(poly, fill=255)
    for box in protected_eye_boxes(width, height):
        draw.ellipse(box, fill=0)
    for box in protected_forehead_hairline_boxes(width, height):
...
def make_overlay(source: Image.Image, mask: Image.Image) -> Image.Image:
    ...
    alpha = mask.point(lambda v: min(170, int(v * 0.68)))
    color.putalpha(alpha)
    outline = Image.new("RGBA", rgba.size, (0, 0, 0, 0))
    draw = ImageDraw.Draw(outline)
    for poly in eyebrow_polygons(*rgba.size):
...
def main() -> int:
    ...
    package_dir = root / "runtime_artifacts" / "mask_factory" / "wave70_mf70_eyebrows"
    prepared_dir = root / "Plan" / "Instructions" / "Operations" / "Prepared_Input_Assets" / f"wave70_mf70_eyebrows_{RUN_STAMP}"
    qa_path = root / "Plan" / "Instructions" / "QA" / "Evidence" / "Mask_Factory" / "Wave70" / "mf70_eyebrows.json"
    tracker_path = root / "Plan" / "Tracker" / "Evidence" / f"W70_MF70_EYEBROWS_LOCAL_MASK_SUPPORT_{RUN_STAMP}.json"
    ...
    mask_path = prepared_dir / "wave70_mf70_eyebrows_mask.png"
    overlay_path = prepared_dir / "wave70_mf70_eyebrows_overlay.png"
    mask.save(mask_path)
    ...
    protected_regions = ["forehead", "eyes", "hairline", "skin", "eyelids", "iris", "hair_occlusion"]
    mask_sha = sha256_file(mask_path)
    overlay_sha = sha256_file(overlay_path)
    source_sha = sha256_file(source_image)

    request = {
        "request_id": "wave70_mf70_eyebrows_local_support",
        ...
        "mask_layers": [{
            ...
            "body_region_id": "left_right_eyebrows",
    ...
    contract = {
        ...
        "wave70_taxonomy": {"mask_type_id": MASK_TYPE_ID, "body_part": "brows", "subregion": "left_right_eyebrows", "role": "edit_or_protect_facial_detail", "protected_regions": protected_regions, "source_citation": SOURCE_REQUIREMENT},
    }
    paths = {
