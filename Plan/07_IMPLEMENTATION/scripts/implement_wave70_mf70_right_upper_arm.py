...
EVIDENCE_ID = f"W70_MF70_RIGHT_UPPER_ARM_{RUN_STAMP}"
SCRIPT_REL = "Plan/07_IMPLEMENTATION/scripts/implement_wave70_mf70_right_upper_arm.py"

QA_DIR = PROJECT_ROOT / "Plan/Instructions/QA/Evidence/Mask_Factory/Wave70"
TRACKER_EVIDENCE_DIR = PROJECT_ROOT / "Plan/Tracker/Evidence"
RUNTIME_DIR = PROJECT_ROOT / "runtime_artifacts/mask_factory/wave70_mf70_right_upper_arm" / RUN_STAMP
...
def evaluate_right_upper_arm_readiness(prereqs: dict[str, object], width: int, height: int) -> dict[str, object]:
    source_visibility = {
        "source_framing": "head_neck_upper_chest_jacket_portrait",
        "image_dimensions": [width, height],
        "right_upper_arm_visible_as_source_body_geometry": False,
        "right_shoulder_to_elbow_chain_visible": False,
        "right_elbow_visible": False,
        "skin_or_unoccluded_arm_surface_visible": False,
        "visible_region": "jacket shoulder and upper torso only",
        "protected_neighbors": ["shoulder", "elbow", "clothing", "background", "hair"],
        "source_region_blocker": "Blocked_Body_Part_Not_Visible",
    }
    required_passes = {
        "source_right_upper_arm_visible": source_visibility["right_upper_arm_visible_as_source_body_geometry"],
        "source_right_shoulder_to_elbow_chain_visible": source_visibility["right_shoulder_to_elbow_chain_visible"],
...
def make_blocker_panel(source: Image.Image, evaluation: dict[str, object]) -> Path:
    ...
    panel_path = RUNTIME_DIR / "mf70_right_upper_arm_blocker_panel.png"
    ...
    shoulder_band_top = int(height * 0.52)
    draw.rectangle([18, shoulder_band_top, width - 18, height - 18], outline=(230, 40, 40), width=4)
    draw.line([18, shoulder_band_top, width - 18, height - 18], fill=(230, 40, 40), width=3)
    draw.line([width - 18, shoulder_band_top, 18, height - 18], fill=(230, 40, 40), width=3)

    draw.rectangle([20, 20, width - 20, 300], fill=(255, 255, 255), outline=(230, 40, 40), width=3)
    lines = [
        "TRK-W70-0156 blocked",
        "mf70_right_upper_arm not source-visible.",
        f"Failed requirements: {len(evaluation['failed_requirements'])}",
        "Visible jacket shoulder is not an upper-arm chain.",
        "No right shoulder-to-elbow or elbow geometry.",
...
def update_wave70_rows(evidence_paths: list[str], note: str) -> dict[str, int]:
    ...
    targets = [(path, "TRK-W70-0156") for path in TRACKER_FILES] + [(path, "ITEM-W70-0156") for path in ITEM_FILES]
    for csv_path, target_id in targets:
        ...
        for row in rows:
            ...
            if "Status_Decision" in row:
                row["Status_Decision"] = "blocked_exact_local_right_upper_arm_not_source_visible"
            if "Coverage_Audit_Status" in row:
                row["Coverage_Audit_Status"] = append_unique(
                    row.get("Coverage_Audit_Status", ""),
                    ["blocked_exact_local_right_upper_arm_not_source_visible"],
                )
            if "Notes" in row:
...
def update_hydration(evidence_paths: list[str]) -> None:
    ...
    current_body = f"""Wave70 remains the active local-first mask-geometry milestone. TRK-W70-0156 / ITEM-W70-0156 was worked locally and is exactly blocked: mf70_right_upper_arm cannot be produced from the active source because the right upper arm is not visible as source-derived body geometry. The portrait shows jacket shoulder/upper torso only, with no right shoulder-to-elbow chain or elbow anchor, and whole-body/model-backed geometry authority remains blocked.
...
Next highest-value local tracker row found from current CSV state is TRK-W70-0157 / ITEM-W70-0157, prove Mask Factory mask type mf70_left_forearm. Work it locally under the same authority rules; if source/authority prerequisites remain blocked, write exact local blocker evidence and keep masks fail-closed."""
    next_body = f"""TRK-W70-0156 / ITEM-W70-0156 is exactly blocked with local source-visibility and whole-body authority evidence. The active portrait does not expose a source-derived right upper-arm region or shoulder-to-elbow chain.
...
Next local task: implement or exactly block TRK-W70-0157 / ITEM-W70-0157, mf70_left_forearm. Use only source-derived whole-body/model-backed geometry evidence. Do not start EC2, run generated-output proof, checkpoint Git, rerun broad helper loops, or promote any mask."""
    session_body = (
        "Worked TRK-W70-0156 / ITEM-W70-0156 locally. mf70_right_upper_arm is blocked because the "
        "active source crop shows jacket shoulder/upper torso but no source-derived right upper-arm or "
        "shoulder-to-elbow chain, and whole-body/model-backed authority remains blocked. No mask artifact, "
        "overlay, generated output, or promotion occurred."
    )
    blocker_body = (
        "TRK-W70-0156 / ITEM-W70-0156: Blocked_Body_Part_Not_Visible / "
        "blocked_exact_local_right_upper_arm_not_source_visible."
    )

    prepend_section(HYDRATION_DIR / "CURRENT_PURSUING_GOAL.md", f"## Wave70 Right Upper Arm Blocked Locally - {ISO_STAMP}", current_body)
    prepend_section(HYDRATION_DIR / "NEXT_ACTION.md", f"## Immediate Next Action - {ISO_STAMP} - Work TRK-W70-0157 Left Forearm Locally", next_body)
    prepend_section(HYDRATION_DIR / "RESUME_HERE_NEXT_CODEX_SESSION.md", f"## Resume Update - Wave70 Right Upper Arm Blocked - {ISO_STAMP}", session_body + "\n\nNext exact action: work TRK-W70-0157 locally.")
    prepend_section(HYDRATION_DIR / "CURRENT_SESSION_STATE.md", f"## Session State Update - Wave70 Right Upper Arm Blocked - {ISO_STAMP}", session_body)
    prepend_section(HYDRATION_DIR / "QA_EVIDENCE_INDEX.md", f"## Wave70 Right Upper Arm Evidence - {ISO_STAMP}", f"{session_body}\n\n{evidence_block}")
    prepend_section(HYDRATION_DIR / "BLOCKERS.md", f"## Wave70 Right Upper Arm Blocker - {ISO_STAMP}", blocker_body + "\n\n" + evidence_block)
    ...
    with proof_log.open("a", newline="", encoding="utf-8") as handle:
        ...
        writer.writerow(
            [
                ISO_STAMP,
                "70",
                "Wave70 mf70 right upper arm blocker",
                "Worked TRK/ITEM-W70-0156 locally. Active source does not expose source-derived right upper-arm or shoulder-to-elbow geometry and whole-body/model-backed authority remains blocked. Wrote stamped/canonical evidence, generated blocker panel, and kept masks fail-closed.",
                "; ".join(evidence_paths),
                ...
                "Next work TRK-W70-0157 left forearm locally; write exact blocker if authority remains blocked.",
            ]
        )
...
def main() -> int:
    ...
    evaluation = evaluate_right_upper_arm_readiness(prereqs, width, height)
    panel_path = make_blocker_panel(source, evaluation)

    qa_evidence_path = QA_DIR / f"{EVIDENCE_ID}.json"
    qa_canonical_path = QA_DIR / "mf70_right_upper_arm.json"
    tracker_evidence_path = TRACKER_EVIDENCE_DIR / f"{EVIDENCE_ID}.json"
    tracker_canonical_path = TRACKER_EVIDENCE_DIR / "mf70_right_upper_arm.json"
    runtime_evidence_path = RUNTIME_DIR / "mf70_right_upper_arm.json"
    ...
    note = (
        f"mf70_right_upper_arm {RUN_STAMP}: exact local blocker. "
        "The active source crop shows jacket shoulder/upper torso but no source-derived right upper-arm or "
        "shoulder-to-elbow geometry, and whole-body/model-backed geometry authority remains blocked. "
    ...
    payload = {
        ...
        "task": "Prove or exactly block Mask Factory mask type mf70_right_upper_arm for TRK-W70-0156 / ITEM-W70-0156.",
        "script": SCRIPT_REL,
        "source_image": {
        ...
        "right_upper_arm_readiness": evaluation,
        "artifacts": {
        ...
        "mask_factory_row": {
            "mask_type_id": "mf70_right_upper_arm",
            "target_region": "right_upper_arm",
            "body_part": "arms",
            "subregion": "right_upper_arm",
            "protected_regions": ["shoulder", "elbow", "clothing", "background", "hair"],
            "result": "blocked",
        ...
        "whole_body_geometry_authority": {
            ...
            "findings": [
                ...
                "The visible jacket shoulder cannot prove the requested right upper-arm source geometry.",
                "No right shoulder-to-elbow chain, elbow anchor, or unoccluded right upper-arm body surface is visible.",
        ...
        "qa_decision": "blocked_exact_local_right_upper_arm_not_source_visible",
        "promotion_decision": "no_mask_promoted_no_active_input_changed_right_upper_arm_blocked",
        "tracker_item_updates": row_updates,
        "next_step": "Work TRK-W70-0157 / ITEM-W70-0157 mf70_left_forearm locally.",
    }
