...
ISO_TS = "2026-07-08T14:51:01-05:00"
HAIR_STAMP = "20260708T145043-0500"
FULL_REF_STAMP = "20260708T143650-0500"
GATE_STAMP = "20260708T145101-0500"
TRACKER_ID = "TRK-W70-0170"
ITEM_ID = "ITEM-W70-0170"
NEXT_TRACKER_ID = "TRK-W70-0171"
NEXT_ITEM_ID = "ITEM-W70-0171"

STATUS_DECISION = "ref_image_1_hair_body_skin_references_available_route_not_complete_global_gates_pass"
COVERAGE_TOKENS = [
    "ref_image_1_hair_body_skin_references_available",
    "hair_body_skin_route_not_complete_no_promotion",
    "post_hair_body_skin_ref_image_1_geometry_gate_pass",
    "post_hair_body_skin_ref_image_1_promotion_gate_pass",
]

EVIDENCE_PATHS = [
    f"Plan/Instructions/QA/Evidence/Mask_Factory/Wave70/W70_HAIR_BODY_SKIN_MARKS_AUTHORITY_{HAIR_STAMP}.json",
    "Plan/Instructions/QA/Evidence/Mask_Factory/Wave70/hair_body_skin_marks_authority.json",
    f"Plan/Tracker/Evidence/W70_HAIR_BODY_SKIN_MARKS_AUTHORITY_{HAIR_STAMP}.json",
    "Plan/Tracker/Evidence/hair_body_skin_marks_authority.json",
    f"runtime_artifacts/mask_factory/wave70_hair_body_skin_marks_authority/{HAIR_STAMP}/hair_body_skin_marks_authority.json",
    f"runtime_artifacts/mask_factory/wave70_hair_body_skin_marks_authority/{HAIR_STAMP}/hair_body_skin_marks_authority_reference_route_panel.png",
    ...
    f"Plan/Instructions/QA/Evidence/Mask_Factory/Wave70/W70_MASK_GEOMETRY_HARD_GATE_POST_HAIR_BODY_SKIN_REF_IMAGE_1_{GATE_STAMP}.json",
    f"Plan/Instructions/QA/Evidence/Mask_Factory/Wave70/W70_MASK_PROMOTION_HARD_GATE_POST_HAIR_BODY_SKIN_REF_IMAGE_1_{GATE_STAMP}.json",
    f"Plan/Tracker/Evidence/W70_MASK_GEOMETRY_HARD_GATE_POST_HAIR_BODY_SKIN_REF_IMAGE_1_{GATE_STAMP}.json",
    f"Plan/Tracker/Evidence/W70_MASK_PROMOTION_HARD_GATE_POST_HAIR_BODY_SKIN_REF_IMAGE_1_{GATE_STAMP}.json",
]
...
HYDRATION_FILES = {
    "CURRENT_SESSION_STATE.md": "Session State Update - 0170 Hair Body Skin Ref_Image_1 Gates Passed",
    "CURRENT_PURSUING_GOAL.md": "Current Pursuing Goal Update - 0170 Hair Body Skin Ref_Image_1 Gates Passed",
    "RESUME_HERE_NEXT_CODEX_SESSION.md": "Resume Update - 0170 Hair Body Skin Ref_Image_1 Gates Passed",
    "NEXT_ACTION.md": "Immediate Next Action - 2026-07-08T14:51:01-05:00 - Re-evaluate TRK-W70-0171 Contact Occlusion Ownership",
    "QA_EVIDENCE_INDEX.md": "Wave70 0170 Hair Body Skin Ref_Image_1 Gate Evidence",
}
...
def update_csv(path: Path, key: str, target: str) -> int:
    ...
    note = (
        "Hair body skin marks authority 20260708T145043-0500: Ref_Image_1 gold standard provides 1 hair mask, "
        "32 body-skin part references, the body-skin composite manifest, and 8 Full context references. Prior "
        "portrait-only authority blocker is superseded for this reference evaluation. Post-evaluation geometry and "
        "promotion gates passed with 332 checked rows, zero pass-like rows, and zero failures. Row remains "
        "Required_Not_Complete; no masks promoted."
    )
    changed = 0
    for row in rows:
...
def block(name: str) -> str:
    ...
    if name == "NEXT_ACTION.md":
        tail = f"Next exact local action: re-evaluate `{NEXT_TRACKER_ID}` / `{NEXT_ITEM_ID}` contact occlusion ownership authority."
    elif name == "RESUME_HERE_NEXT_CODEX_SESSION.md":
        tail = f"Resume at `{NEXT_TRACKER_ID}` / `{NEXT_ITEM_ID}` contact occlusion ownership authority."
        ...
        tail = f"Current pursuit advances to `{NEXT_TRACKER_ID}` / `{NEXT_ITEM_ID}` contact occlusion ownership authority."
    else:
        tail = f"Next local action is `{NEXT_TRACKER_ID}` / `{NEXT_ITEM_ID}` contact occlusion ownership authority."
    return f"""## {heading}

Re-ran `{TRACKER_ID}` / `{ITEM_ID}` hair/body-skin authority with Ref_Image_1 gold references and Full context.
...
def append_proof_log() -> None:
    row = [
        ISO_TS,
        "70",
        "Wave70 0170 hair body skin Ref_Image_1 rerun and gates passed",
        (
            "Reran TRK/ITEM-W70-0170 with Ref_Image_1 hair/body-skin references and Full context, attached evidence "
            "and post-rerun geometry/promotion gates, and kept the row Required_Not_Complete/non-promotional."
        ),
        "; ".join(EVIDENCE_PATHS),
        "python py_compile; Ref_Image_1 gold mask manifest validation; body-skin composite manifest validation; JSON validation; reference panel inspection; Test-Wave70MaskGeometryGate.ps1; Test-Wave70MaskPromotionGate.ps1; CSV row verification",
        "HAIR_BODY_SKIN_REF_IMAGE_1_REFERENCE_AVAILABLE_ROUTE_NOT_COMPLETE_GATES_PASS",
        f"Plan/Instructions/QA/Evidence/Mask_Factory/Wave70/W70_HAIR_BODY_SKIN_MARKS_AUTHORITY_{HAIR_STAMP}.json",
        f"Re-evaluate {NEXT_TRACKER_ID} / {NEXT_ITEM_ID} contact occlusion ownership authority; no promotion without row-level proof.",
    ]
