...
TRK = "TRK-W64-064"
ITEM = "ITEM-W64-064"
STATUS = "Blocked_Prompt_Profile_Lane_Authority_And_Runtime_QA_Gaps"
...
def prepend(path: Path, block: str) -> None:
    ...
    pattern = re.compile(r"(?ms)^## Wave64 Row064 Prompt And Negative-Prompt QA[^\n]*\n.*?(?=^## |\Z)")
    current = pattern.sub("", current).lstrip()
...
def validate_prompt_source(data: dict, positive: str, negative: str) -> dict | None:
    source = data.get("prompt_source")
    if not isinstance(source, dict):
        return None
    required = ("path", "sha256", "positive_node_id", "negative_node_id")
    if any(not source.get(key) for key in required):
        return {"path": source.get("path"), "valid": False, "failure": "prompt_source_contract_incomplete"}
    try:
        source_path = (ROOT / source["path"]).resolve()
        source_path.relative_to(ROOT.resolve())
    except (OSError, ValueError):
        return {"path": source.get("path"), "valid": False, "failure": "prompt_source_path_invalid"}
    if not source_path.is_file():
        return {"path": source["path"], "valid": False, "failure": "prompt_source_missing"}
    try:
        request = load(source_path)
        nodes = request["prompt"]
        source_positive = nodes[str(source["positive_node_id"])]["inputs"]["text"]
        source_negative = nodes[str(source["negative_node_id"])]["inputs"]["text"]
    except (KeyError, TypeError, json.JSONDecodeError):
        return {"path": source["path"], "valid": False, "failure": "prompt_source_request_invalid"}
    checks = {
        "sha256_match": sha(source_path) == source["sha256"],
        "positive_prompt_match": source_positive == positive,
        "negative_prompt_match": source_negative == negative,
    }
    return {"path": source["path"], "valid": all(checks.values()), "checks": checks}
...
def main() -> None:
    ...
    for path, data in parsed:
        ...
        prompt_source = validate_prompt_source(data, positive, negative)
        record = {
            ...
            "prompt_source_binding": prompt_source,
    ...
    runtime_missing = [record for record in prompt_records if not record["runtime_evidence_paths"]]
    prompt_source_bound = [record for record in prompt_records if record["prompt_source_binding"] is not None]
    ...
    checks = {
        ...
        "PNQ-008_prompt_pairs_complete_109": sum(record["prompt_pair_complete"] for record in prompt_records) == 109,
        "PNQ-009_missing_prompt_links_zero": not missing_prompts,
        "PNQ-010_prompt_metadata_complete_109": sum(record["metadata_complete"] for record in prompt_records) == 109,
        "PNQ-011_negative_prompts_present_109": sum(record["negative_prompt_present"] for record in prompt_records) == 109,
        "PNQ-012_zero_exact_clause_contradictions": not contradictions,
        "PNQ-013_duplicate_groups_21": len(duplicate_groups) == 21,
        ...
        "PNQ-019_four_profiles_have_verified_prompt_sources_and_runtime_qa_links": len(prompt_source_bound) == 4 and all(record["prompt_source_binding"]["valid"] for record in prompt_source_bound) and len(runtime_linked) == 4 and all(len(record["runtime_evidence_paths"]) >= 2 for record in runtime_linked),
    ...
    blockers = [
        ...
        {"blocker_id": "REPRESENTATIVE_RUNTIME_OUTPUT_LINK_MISSING", "count": len(runtime_missing), "resolution": "Link per-profile representative output and visual QA evidence, or explicitly record pending runtime; do not approve from static text."},
    ...
    payload = {
        ...
        "row_complete": False, "qa_decision": "static_prompt_pair_inventory_complete_approval_fail_closed",
        "inventory_summary": {
        ...
        "safety_boundary": {"generation_executed": False, "profile_approved": False, "profiles_reconciled_from_hash_verified_local_requests": 4, "aws_contacted": False, "ec2_started": False, "wave71_activated": False, "mask_or_jira_touched": False},
    ...
    note = f"Wave64 Row064 {stamp}: parsed 112/112 JSON artifacts; all 109 prompt profiles now have durable positive/negative pairs; four profiles are hash-bound to existing local runtime and visual-QA evidence; 93 lack lane authority, 105 lack direct runtime evidence, and 14 Wave71+ names remain deferred; 20/20 audit checks pass."
    tags = ["wave64_row064_prompt_inventory_complete", "all_prompt_pairs_durable", "four_runtime_links_reconciled", "prompt_approval_blocked", "ninety_three_lane_authority_gaps", "runtime_alignment_pending", "advance_row065"]
...
"""
    ...
    if not any(row.get("Task") == TRK and row.get("Result") == payload["qa_decision"] for row in csv.DictReader(proof.open("r", encoding="utf-8-sig", newline=""))):
