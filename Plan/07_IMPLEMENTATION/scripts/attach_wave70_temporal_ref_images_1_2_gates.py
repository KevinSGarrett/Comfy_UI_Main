...
TEMPORAL_STAMP = "20260708T182800-0500"
GATE_STAMP = "20260708T182800-0500"
TRACKER_ID = "TRK-W70-0175"
ITEM_ID = "ITEM-W70-0175"
NEXT_TRACKER_ID = "TRK-W70-0176"
NEXT_ITEM_ID = "ITEM-W70-0176"

STATUS_DECISION = "ref_images_1_2_temporal_reference_context_available_route_not_complete_global_gates_pass"
COVERAGE_TOKENS = [
    ...
    "post_temporal_ref_images_1_2_geometry_gate_pass",
    "post_temporal_ref_images_1_2_promotion_gate_pass",
]

QA_GEOMETRY_GATE = f"Plan/Instructions/QA/Evidence/Mask_Factory/Wave70/W70_MASK_GEOMETRY_HARD_GATE_POST_TEMPORAL_REF_IMAGES_1_2_{GATE_STAMP}.json"
QA_PROMOTION_GATE = f"Plan/Instructions/QA/Evidence/Mask_Factory/Wave70/W70_MASK_PROMOTION_HARD_GATE_POST_TEMPORAL_REF_IMAGES_1_2_{GATE_STAMP}.json"
TRACKER_GEOMETRY_GATE = f"Plan/Tracker/Evidence/W70_MASK_GEOMETRY_HARD_GATE_POST_TEMPORAL_REF_IMAGES_1_2_{GATE_STAMP}.json"
TRACKER_PROMOTION_GATE = f"Plan/Tracker/Evidence/W70_MASK_PROMOTION_HARD_GATE_POST_TEMPORAL_REF_IMAGES_1_2_{GATE_STAMP}.json"

EVIDENCE_PATHS = [
    f"Plan/Instructions/QA/Evidence/Mask_Factory/Wave70/W70_TEMPORAL_BODY_PART_TRACKING_AUTHORITY_{TEMPORAL_STAMP}.json",
    "Plan/Instructions/QA/Evidence/Mask_Factory/Wave70/temporal_body_part_tracking_authority.json",
    f"Plan/Tracker/Evidence/W70_TEMPORAL_BODY_PART_TRACKING_AUTHORITY_{TEMPORAL_STAMP}.json",
    "Plan/Tracker/Evidence/temporal_body_part_tracking_authority.json",
    f"runtime_artifacts/mask_factory/wave70_temporal_body_part_tracking_authority/{TEMPORAL_STAMP}/temporal_body_part_tracking_authority.json",
    f"runtime_artifacts/mask_factory/wave70_temporal_body_part_tracking_authority/{TEMPORAL_STAMP}/temporal_body_part_tracking_authority_reference_route_panel.png",
...
HYDRATION_FILES = {
    "CURRENT_SESSION_STATE.md": "Session State Update - 0175 Ref Images 1 2 Temporal Gates Passed",
    "CURRENT_PURSUING_GOAL.md": "Current Pursuing Goal Update - 0175 Ref Images 1 2 Temporal Gates Passed",
    "RESUME_HERE_NEXT_CODEX_SESSION.md": "Resume Update - 0175 Ref Images 1 2 Temporal Gates Passed",
    "NEXT_ACTION.md": "Immediate Next Action - Ref Images 1 2 - Work TRK-W70-0176 Body Reference Matrix",
    "QA_EVIDENCE_INDEX.md": "Wave70 0175 Ref Images 1 2 Temporal Gate Evidence",
}
...
def update_csv(path: Path, key: str, target: str) -> int:
    ...
    note = (
        "Temporal body-part tracking authority 20260708T182800-0500: reran with Ref_Image_1 plus Ref_Image_2 combined body matrix. "
        "Combined context records 9 full/near-full references and 78 gold masks, but they are static references rather than ordered temporal frames. "
        "Geometry and promotion hard gates passed with 332 checked rows, zero pass-like rows, and zero failures. Row remains Required_Not_Complete; no masks promoted."
    )
    changed = 0
    for row in rows:
...
def hydration_block(name: str) -> str:
    ...
    if name == "NEXT_ACTION.md":
        tail = f"Next exact local action: work `{NEXT_TRACKER_ID}` / `{NEXT_ITEM_ID}` body reference matrix using Ref_Image_1+Ref_Image_2 combined references."
...
Re-ran `{TRACKER_ID}` / `{ITEM_ID}` temporal body-part tracking authority with Ref_Image_1 plus Ref_Image_2 combined body-reference context.
...
def append_proof_log() -> None:
    row = [
        ISO_TS,
        "70",
        "Wave70 0175 Ref_Image_1+2 temporal gates passed",
        "Attached Ref_Image_1+2 temporal evidence and gates; static references do not prove temporal tracking; kept no-promotion Required_Not_Complete state.",
        "; ".join(EVIDENCE_PATHS),
        "python py_compile; implement_wave70_temporal_body_part_tracking_authority.py; Test-Wave70MaskGeometryGate.ps1; Test-Wave70MaskPromotionGate.ps1; CSV row verification",
        "REF_IMAGES_1_2_TEMPORAL_ROUTE_NOT_COMPLETE_GATES_PASS",
        f"Plan/Instructions/QA/Evidence/Mask_Factory/Wave70/W70_TEMPORAL_BODY_PART_TRACKING_AUTHORITY_{TEMPORAL_STAMP}.json",
