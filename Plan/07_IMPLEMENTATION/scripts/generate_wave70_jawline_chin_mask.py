...
RUN_STAMP = "20260707T160000-0500"
MASK_TYPE_ID = "mf70_jawline_chin"
TRACKER_ID = "TRK-W70-0006"
ITEM_ID = "ITEM-W70-0006"
CONTRACT_ID = "mask_contract_wave70_mf70_jawline_chin"
SCENE_ID = "scene_wave70_canny_v3_jawline_chin"
MASK_ID = "scene_wave70_canny_v3__person_001__mf70_jawline_chin__minor"
...
def jawline_polygon(width: int, height: int) -> list[tuple[int, int]]:
    points = [
        (0.392, 0.615),
        (0.450, 0.642),
        (0.505, 0.668),
...
def make_mask(source: Image.Image) -> Image.Image:
    ...
    draw.polygon(jawline_polygon(width, height), fill=255)
    return base.filter(ImageFilter.GaussianBlur(radius=max(3, width // 180)))
...
def make_overlay(source: Image.Image, mask: Image.Image) -> Image.Image:
    ...
    polygon = jawline_polygon(*rgba.size)
    ImageDraw.Draw(outline).line(polygon + [polygon[0]], fill=(255, 255, 255, 230), width=3)
...
def main() -> int:
    ...
    package_dir = root / "runtime_artifacts" / "mask_factory" / "wave70_mf70_jawline_chin"
    prepared_dir = root / "Plan" / "Instructions" / "Operations" / "Prepared_Input_Assets" / f"wave70_mf70_jawline_chin_{RUN_STAMP}"
    qa_path = root / "Plan" / "Instructions" / "QA" / "Evidence" / "Mask_Factory" / "Wave70" / "mf70_jawline_chin.json"
    tracker_path = root / "Plan" / "Tracker" / "Evidence" / f"W70_MF70_JAWLINE_CHIN_LOCAL_MASK_SUPPORT_{RUN_STAMP}.json"
    input_copy = root / "ComfyUI" / "input" / "wave70_mf70_jawline_chin_mask.png"
    ...
    mask_path = prepared_dir / "wave70_mf70_jawline_chin_mask.png"
    overlay_path = prepared_dir / "wave70_mf70_jawline_chin_overlay.png"
    mask.save(mask_path)
    mask.save(input_copy)
    overlay.save(overlay_path)

    protected_regions = ["mouth", "neck", "hair", "clothing"]
    ...
    request = {
        "request_id": "wave70_mf70_jawline_chin_local_support",
        ...
        "mask_layers": [{
            ...
            "body_region_id": "jawline_chin_contour",
    ...
    contract = {
        ...
        "wave70_taxonomy": {
            ...
            "body_part": "face",
            "subregion": "jawline_chin_contour",
    ...
    with patch_path.open("w", encoding="utf-8", newline="") as handle:
        ...
        writer.writerow({
            ...
            "pass_id": "wave70_mf70_jawline_chin_local_support",
            "output_prefix": "codex_wave70_mf70_jawline_chin",
        })
    ...
    quality_report = {
        ...
        "protected_neighbor_check": {
            ...
            "protected_regions": protected_regions,
            "notes": [
                "Mask covers a narrow jawline and chin contour band.",
                "Mask intentionally avoids mouth/lips above and neck/clothing below.",
    ...
    runtime_evidence = {
        "schema_version": "1.0",
        "evidence_id": f"W70-MF70-JAWLINE-CHIN-RUNTIME-EVIDENCE-{RUN_STAMP}",
        "timestamp": "2026-07-07T16:00:00-05:00",
    ...
    qa_evidence = {
        "schema_version": "1.0",
        "evidence_id": f"W70-MF70-JAWLINE-CHIN-LOCAL-SUPPORT-{RUN_STAMP}",
        "timestamp": "2026-07-07T16:00:00-05:00",
        "project_root": str(root),
        ...
        "result": RESULT,
        "boundary": "This proves local Wave70 mf70_jawline_chin mask artifact, overlay, quality score, and workflow routing support only. It does not mark the item complete because generated-output proof and target-runtime proof remain required.",
    }
    write_json(qa_path, qa_evidence)

    tracker_evidence = {
        "schema_version": "1.0",
        "tracker_evidence_id": f"W70_MF70_JAWLINE_CHIN_LOCAL_MASK_SUPPORT_{RUN_STAMP}",
        "created_at": "2026-07-07T16:00:00-05:00",
        "project_root": str(root),
        "tracker_id": TRACKER_ID,
        "item_id": ITEM_ID,
        "mask_type_id": MASK_TYPE_ID,
        ...
        "actual_work_performed": [
            "Generated deterministic local jawline/chin mask PNG.",
