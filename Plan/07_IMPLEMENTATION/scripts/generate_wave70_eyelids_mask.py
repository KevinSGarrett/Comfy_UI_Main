...
RUN_STAMP = "20260707T181500-0500"
MASK_TYPE_ID = "mf70_eyelids"
TRACKER_ID = "TRK-W70-0013"
ITEM_ID = "ITEM-W70-0013"
CONTRACT_ID = "mask_contract_wave70_mf70_eyelids"
SCENE_ID = "scene_wave70_canny_v3_eyelids"
MASK_ID = "scene_wave70_canny_v3__person_001__mf70_eyelids__minor"
RESULT = "pass_local_wave70_mask_artifact_routing_support_final_blocked_generated_output_target_runtime"
SOURCE_REQUIREMENT = "Plan/07_IMPLEMENTATION/mask_factory/ULTIMATE_MASK_FACTORY_TAXONOMY.md#L44"
...
def eyelid_polygons(width: int, height: int) -> list[list[tuple[int, int]]]:
    return [
        [
            (int(width * 0.368), int(height * 0.404)),
            (int(width * 0.394), int(height * 0.397)),
            (int(width * 0.439), int(height * 0.400)),
            (int(width * 0.458), int(height * 0.412)),
            (int(width * 0.450), int(height * 0.424)),
            (int(width * 0.405), int(height * 0.421)),
            (int(width * 0.376), int(height * 0.414)),
        ],
        [
            (int(width * 0.515), int(height * 0.400)),
            (int(width * 0.542), int(height * 0.394)),
            (int(width * 0.588), int(height * 0.397)),
            (int(width * 0.607), int(height * 0.409)),
            (int(width * 0.597), int(height * 0.421)),
            (int(width * 0.552), int(height * 0.416)),
            (int(width * 0.522), int(height * 0.409)),
        ],
        [
            (int(width * 0.391), int(height * 0.447)),
            (int(width * 0.418), int(height * 0.451)),
            (int(width * 0.446), int(height * 0.445)),
            (int(width * 0.439), int(height * 0.455)),
            (int(width * 0.407), int(height * 0.461)),
            (int(width * 0.386), int(height * 0.454)),
        ],
        [
            (int(width * 0.532), int(height * 0.442)),
            (int(width * 0.560), int(height * 0.447)),
            (int(width * 0.591), int(height * 0.439)),
            (int(width * 0.583), int(height * 0.450)),
            (int(width * 0.551), int(height * 0.457)),
            (int(width * 0.527), int(height * 0.450)),
        ],
    ]
...
def make_mask(source: Image.Image) -> Image.Image:
    ...
    for poly in eyelid_polygons(width, height):
...
def make_overlay(source: Image.Image, mask: Image.Image) -> Image.Image:
    ...
    for poly in eyelid_polygons(*rgba.size):
...
def main() -> int:
    ...
    package_dir = root / "runtime_artifacts" / "mask_factory" / "wave70_mf70_eyelids"
    prepared_dir = root / "Plan" / "Instructions" / "Operations" / "Prepared_Input_Assets" / f"wave70_mf70_eyelids_{RUN_STAMP}"
    qa_path = root / "Plan" / "Instructions" / "QA" / "Evidence" / "Mask_Factory" / "Wave70" / "mf70_eyelids.json"
    tracker_path = root / "Plan" / "Tracker" / "Evidence" / f"W70_MF70_EYELIDS_LOCAL_MASK_SUPPORT_{RUN_STAMP}.json"
    input_copy = root / "ComfyUI" / "input" / "wave70_mf70_eyelids_mask.png"
    ...
    mask_path = prepared_dir / "wave70_mf70_eyelids_mask.png"
    overlay_path = prepared_dir / "wave70_mf70_eyelids_overlay.png"
    mask.save(mask_path)
    ...
    protected_regions = ["iris", "sclera", "pupils", "eyelashes", "eyebrows", "skin", "hair_occlusion"]
    mask_sha = sha256_file(mask_path)
    overlay_sha = sha256_file(overlay_path)
    source_sha = sha256_file(source_image)

    request = {
        "request_id": "wave70_mf70_eyelids_local_support",
        ...
        "mask_layers": [{
            ...
            "body_region_id": "upper_lower_eyelids",
    ...
    contract = {
        ...
        "wave70_taxonomy": {"mask_type_id": MASK_TYPE_ID, "body_part": "eyes", "subregion": "upper_lower_eyelids", "role": "edit_or_protect_facial_detail", "protected_regions": protected_regions, "source_citation": SOURCE_REQUIREMENT},
    }
    paths = {
