...
ISO_TS = "2026-07-08T14:59:30-05:00"
BODY_STAMP = "20260708T145908-0500"
FULL_REF_STAMP = "20260708T143650-0500"
GATE_STAMP = "20260708T145930-0500"
TRACKER_ID = "TRK-W70-0172"
ITEM_ID = "ITEM-W70-0172"
NEXT_TRACKER_ID = "TRK-W70-0173"
NEXT_ITEM_ID = "ITEM-W70-0173"

STATUS_DECISION = "ref_image_1_body_region_references_available_route_not_complete_global_gates_pass"
COVERAGE_TOKENS = [
    "ref_image_1_body_region_references_available",
    "body_region_geometry_route_not_complete_no_promotion",
    "post_body_region_ref_image_1_geometry_gate_pass",
    "post_body_region_ref_image_1_promotion_gate_pass",
]
...
EVIDENCE_PATHS = [
    ...
    f"Plan/Instructions/QA/Evidence/Mask_Factory/Wave70/W70_MASK_GEOMETRY_HARD_GATE_POST_BODY_REGION_REF_IMAGE_1_{GATE_STAMP}.json",
    f"Plan/Instructions/QA/Evidence/Mask_Factory/Wave70/W70_MASK_PROMOTION_HARD_GATE_POST_BODY_REGION_REF_IMAGE_1_{GATE_STAMP}.json",
    f"Plan/Tracker/Evidence/W70_MASK_GEOMETRY_HARD_GATE_POST_BODY_REGION_REF_IMAGE_1_{GATE_STAMP}.json",
    f"Plan/Tracker/Evidence/W70_MASK_PROMOTION_HARD_GATE_POST_BODY_REGION_REF_IMAGE_1_{GATE_STAMP}.json",
]
...
HYDRATION_FILES = {
    "CURRENT_SESSION_STATE.md": "Session State Update - 0172 Body Region Ref_Image_1 Gates Passed",
    "CURRENT_PURSUING_GOAL.md": "Current Pursuing Goal Update - 0172 Body Region Ref_Image_1 Gates Passed",
    "RESUME_HERE_NEXT_CODEX_SESSION.md": "Resume Update - 0172 Body Region Ref_Image_1 Gates Passed",
    "NEXT_ACTION.md": "Immediate Next Action - 2026-07-08T14:59:30-05:00 - Re-evaluate TRK-W70-0173 Model Consensus Geometry",
    "QA_EVIDENCE_INDEX.md": "Wave70 0172 Body Region Ref_Image_1 Gate Evidence",
}
...
def update_csv(path: Path, key: str, target: str) -> int:
    ...
    note = (
        "Body region geometry authority 20260708T145908-0500: Ref_Image_1 gold standard provides 33 body-region "
        "masks and Ref_Image_1/Full provides 8 body context references. Prior portrait-only body-region blocker is "
...
def block(name: str) -> str:
    ...
    if name == "NEXT_ACTION.md":
        tail = f"Next exact local action: re-evaluate `{NEXT_TRACKER_ID}` / `{NEXT_ITEM_ID}` model consensus geometry validator."
    elif name == "RESUME_HERE_NEXT_CODEX_SESSION.md":
        tail = f"Resume at `{NEXT_TRACKER_ID}` / `{NEXT_ITEM_ID}` model consensus geometry validator."
        ...
        tail = f"Current pursuit advances to `{NEXT_TRACKER_ID}` / `{NEXT_ITEM_ID}` model consensus geometry validator."
    else:
        tail = f"Next local action is `{NEXT_TRACKER_ID}` / `{NEXT_ITEM_ID}` model consensus geometry validator."
    return f"""## {heading}

Re-ran `{TRACKER_ID}` / `{ITEM_ID}` body region geometry authority with Ref_Image_1 gold body-region masks and Full context.
...
def append_proof_log() -> None:
    row = [
        ISO_TS,
        "70",
        "Wave70 0172 body region Ref_Image_1 rerun and gates passed",
        (
            "Reran TRK/ITEM-W70-0172 with Ref_Image_1 body-region references and Full context, attached evidence "
            "and post-rerun geometry/promotion gates, and kept the row Required_Not_Complete/non-promotional."
        ),
        "; ".join(EVIDENCE_PATHS),
        "python py_compile; Ref_Image_1 body-region manifest validation; Full reference manifest validation; JSON validation; reference panel inspection; Test-Wave70MaskGeometryGate.ps1; Test-Wave70MaskPromotionGate.ps1; CSV row verification",
        "BODY_REGION_REF_IMAGE_1_REFERENCE_AVAILABLE_ROUTE_NOT_COMPLETE_GATES_PASS",
        f"Plan/Instructions/QA/Evidence/Mask_Factory/Wave70/W70_BODY_REGION_GEOMETRY_AUTHORITY_{BODY_STAMP}.json",
        f"Re-evaluate {NEXT_TRACKER_ID} / {NEXT_ITEM_ID} model consensus geometry validator; no promotion without row-level proof.",
    ]
