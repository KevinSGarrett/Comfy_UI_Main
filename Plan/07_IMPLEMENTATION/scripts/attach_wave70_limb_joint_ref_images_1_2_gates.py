...
LIMB_STAMP = "20260708T155224-0500"
GATE_STAMP = "20260708T155224-0500"
TRACKER_ID = "TRK-W70-0168"
ITEM_ID = "ITEM-W70-0168"
NEXT_TRACKER_ID = "TRK-W70-0169"
NEXT_ITEM_ID = "ITEM-W70-0169"

STATUS_DECISION = "ref_images_1_2_full_body_limb_references_available_route_not_complete_global_gates_pass"
COVERAGE_TOKENS = [
    ...
    "post_limb_joint_ref_images_1_2_geometry_gate_pass",
    "post_limb_joint_ref_images_1_2_promotion_gate_pass",
]

QA_GEOMETRY_GATE = f"Plan/Instructions/QA/Evidence/Mask_Factory/Wave70/W70_MASK_GEOMETRY_HARD_GATE_POST_LIMB_JOINT_REF_IMAGES_1_2_{GATE_STAMP}.json"
QA_PROMOTION_GATE = f"Plan/Instructions/QA/Evidence/Mask_Factory/Wave70/W70_MASK_PROMOTION_HARD_GATE_POST_LIMB_JOINT_REF_IMAGES_1_2_{GATE_STAMP}.json"
TRACKER_GEOMETRY_GATE = f"Plan/Tracker/Evidence/W70_MASK_GEOMETRY_HARD_GATE_POST_LIMB_JOINT_REF_IMAGES_1_2_{GATE_STAMP}.json"
TRACKER_PROMOTION_GATE = f"Plan/Tracker/Evidence/W70_MASK_PROMOTION_HARD_GATE_POST_LIMB_JOINT_REF_IMAGES_1_2_{GATE_STAMP}.json"
...
HYDRATION_FILES = {
    "CURRENT_SESSION_STATE.md": "Session State Update - 0168 Ref Images 1 2 Limb Joint Gates Passed",
    "CURRENT_PURSUING_GOAL.md": "Current Pursuing Goal Update - 0168 Ref Images 1 2 Limb Joint Gates Passed",
    "RESUME_HERE_NEXT_CODEX_SESSION.md": "Resume Update - 0168 Ref Images 1 2 Limb Joint Gates Passed",
    "NEXT_ACTION.md": "Immediate Next Action - Ref Images 1 2 - Re-evaluate TRK-W70-0169 Foot Toe Authority",
    "QA_EVIDENCE_INDEX.md": "Wave70 0168 Ref Images 1 2 Limb Joint Gate Evidence",
}
...
def update_csv(path: Path, key: str, target: str) -> int:
    ...
    note = (
        "Limb joint authority 20260708T155224-0500: reran with Ref_Image_1 plus Ref_Image_2 combined body matrix. "
...
def hydration_block(name: str) -> str:
    ...
    if name == "NEXT_ACTION.md":
        tail = (
            f"Next exact local action: re-evaluate `{NEXT_TRACKER_ID}` / `{NEXT_ITEM_ID}` foot/toe authority using "
            "Ref_Image_1+Ref_Image_2 combined references. Do not use `Ref_Image_1/Full/New folder` for feet/toes/ankles/lower-calf proof."
        )
...
Re-ran `{TRACKER_ID}` / `{ITEM_ID}` limb/joint authority with Ref_Image_1 plus Ref_Image_2 combined body-reference context.
...
def append_proof_log() -> None:
    row = [
        ISO_TS,
        "70",
        "Wave70 0168 Ref_Image_1+2 limb joint rerun and gates passed",
        "Reran TRK/ITEM-W70-0168 with combined Ref_Image_1 and Ref_Image_2 body reference context; attached gates and kept no-promotion Required_Not_Complete state.",
        "; ".join(EVIDENCE_PATHS),
        "python py_compile; implement_wave70_limb_joint_region_authority.py; Test-Wave70MaskGeometryGate.ps1; Test-Wave70MaskPromotionGate.ps1; CSV row verification",
        "REF_IMAGES_1_2_LIMB_REFERENCE_AVAILABLE_ROUTE_NOT_COMPLETE_GATES_PASS",
        f"Plan/Instructions/QA/Evidence/Mask_Factory/Wave70/W70_LIMB_JOINT_REGION_AUTHORITY_{LIMB_STAMP}.json",
        f"Continue Wave70 at {NEXT_TRACKER_ID} / {NEXT_ITEM_ID} foot/toe authority with combined body references.",
    ]
