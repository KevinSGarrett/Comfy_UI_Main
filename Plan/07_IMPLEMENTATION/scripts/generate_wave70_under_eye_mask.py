...
RUN_STAMP = "20260707T184500-0500"
MASK_TYPE_ID = "mf70_under_eye"
TRACKER_ID = "TRK-W70-0015"
ITEM_ID = "ITEM-W70-0015"
CONTRACT_ID = "mask_contract_wave70_mf70_under_eye"
SCENE_ID = "scene_wave70_canny_v3_under_eye"
MASK_ID = "scene_wave70_canny_v3__person_001__mf70_under_eye__minor"
RESULT = "pass_local_wave70_mask_artifact_routing_support_final_blocked_generated_output_target_runtime"
SOURCE_REQUIREMENT = "Plan/07_IMPLEMENTATION/mask_factory/ULTIMATE_MASK_FACTORY_TAXONOMY.md#L46"
...
def under_eye_polygons(width: int, height: int) -> list[list[tuple[int, int]]]:
    return [
        [
            (int(width * 0.386), int(height * 0.456)),
            (int(width * 0.410), int(height * 0.463)),
            (int(width * 0.445), int(height * 0.456)),
            (int(width * 0.456), int(height * 0.471)),
            (int(width * 0.429), int(height * 0.486)),
            (int(width * 0.396), int(height * 0.480)),
            (int(width * 0.376), int(height * 0.468)),
        ],
        [
            (int(width * 0.529), int(height * 0.451)),
            (int(width * 0.558), int(height * 0.459)),
            (int(width * 0.592), int(height * 0.450)),
            (int(width * 0.603), int(height * 0.465)),
            (int(width * 0.579), int(height * 0.482)),
            (int(width * 0.545), int(height * 0.477)),
            (int(width * 0.518), int(height * 0.464)),
        ],
    ]
...
def protected_eye_boxes(width: int, height: int) -> list[tuple[int, int, int, int]]:
    return [
        (int(width * 0.382), int(height * 0.395), int(width * 0.454), int(height * 0.456)),
        (int(width * 0.525), int(height * 0.389), int(width * 0.599), int(height * 0.450)),
    ]


def protected_nose_cheek_boxes(width: int, height: int) -> list[tuple[int, int, int, int]]:
    return [
        (int(width * 0.465), int(height * 0.430), int(width * 0.522), int(height * 0.555)),
        (0, int(height * 0.495), width, height),
        (0, 0, int(width * 0.350), height),
        (int(width * 0.625), 0, width, height),
    ]
...
def make_mask(source: Image.Image) -> Image.Image:
    ...
    for poly in under_eye_polygons(width, height):
        draw.polygon(poly, fill=255)
    for box in protected_eye_boxes(width, height):
        draw.ellipse(box, fill=0)
    for box in protected_nose_cheek_boxes(width, height):
...
def make_overlay(source: Image.Image, mask: Image.Image) -> Image.Image:
    ...
    alpha = mask.point(lambda v: min(165, int(v * 0.66)))
    color.putalpha(alpha)
    outline = Image.new("RGBA", rgba.size, (0, 0, 0, 0))
    draw = ImageDraw.Draw(outline)
    for poly in under_eye_polygons(*rgba.size):
...
def main() -> int:
    ...
    package_dir = root / "runtime_artifacts" / "mask_factory" / "wave70_mf70_under_eye"
    prepared_dir = root / "Plan" / "Instructions" / "Operations" / "Prepared_Input_Assets" / f"wave70_mf70_under_eye_{RUN_STAMP}"
    qa_path = root / "Plan" / "Instructions" / "QA" / "Evidence" / "Mask_Factory" / "Wave70" / "mf70_under_eye.json"
    tracker_path = root / "Plan" / "Tracker" / "Evidence" / f"W70_MF70_UNDER_EYE_LOCAL_MASK_SUPPORT_{RUN_STAMP}.json"
    ...
    mask_path = prepared_dir / "wave70_mf70_under_eye_mask.png"
    overlay_path = prepared_dir / "wave70_mf70_under_eye_overlay.png"
    mask.save(mask_path)
    ...
    protected_regions = ["eyes", "eyelids", "cheeks", "nose", "iris", "sclera", "hair_occlusion"]
    mask_sha = sha256_file(mask_path)
    overlay_sha = sha256_file(overlay_path)
    source_sha = sha256_file(source_image)

    request = {
        "request_id": "wave70_mf70_under_eye_local_support",
    ...
    contract = {
        ...
        "wave70_taxonomy": {"mask_type_id": MASK_TYPE_ID, "body_part": "eyes", "subregion": "under_eye_skin", "role": "edit_or_protect_facial_detail", "protected_regions": protected_regions, "source_citation": SOURCE_REQUIREMENT},
    }
    paths = {
