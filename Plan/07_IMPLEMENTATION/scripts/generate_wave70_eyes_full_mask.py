...
RUN_STAMP = "20260707T171500-0500"
MASK_TYPE_ID = "mf70_eyes_full"
TRACKER_ID = "TRK-W70-0009"
ITEM_ID = "ITEM-W70-0009"
CONTRACT_ID = "mask_contract_wave70_mf70_eyes_full"
SCENE_ID = "scene_wave70_canny_v3_eyes_full"
MASK_ID = "scene_wave70_canny_v3__person_001__mf70_eyes_full__minor"
RESULT = "pass_local_wave70_mask_artifact_routing_support_final_blocked_generated_output_target_runtime"
SOURCE_REQUIREMENT = "Plan/07_IMPLEMENTATION/mask_factory/ULTIMATE_MASK_FACTORY_TAXONOMY.md#L40"
...
def eye_ellipses(width: int, height: int) -> list[tuple[int, int, int, int]]:
    return [
        (int(width * 0.365), int(height * 0.392), int(width * 0.461), int(height * 0.456)),
        (int(width * 0.511), int(height * 0.386), int(width * 0.610), int(height * 0.451)),
    ]
...
def make_mask(source: Image.Image) -> Image.Image:
    ...
    for box in eye_ellipses(width, height):
        draw.ellipse(box, fill=255)
    return base.filter(ImageFilter.GaussianBlur(radius=max(2, width // 260)))
...
def make_overlay(source: Image.Image, mask: Image.Image) -> Image.Image:
    ...
    for box in eye_ellipses(*rgba.size):
        draw.ellipse(box, outline=(255, 255, 255, 235), width=3)
...
def main() -> int:
    ...
    package_dir = root / "runtime_artifacts" / "mask_factory" / "wave70_mf70_eyes_full"
    prepared_dir = root / "Plan" / "Instructions" / "Operations" / "Prepared_Input_Assets" / f"wave70_mf70_eyes_full_{RUN_STAMP}"
    qa_path = root / "Plan" / "Instructions" / "QA" / "Evidence" / "Mask_Factory" / "Wave70" / "mf70_eyes_full.json"
    tracker_path = root / "Plan" / "Tracker" / "Evidence" / f"W70_MF70_EYES_FULL_LOCAL_MASK_SUPPORT_{RUN_STAMP}.json"
    input_copy = root / "ComfyUI" / "input" / "wave70_mf70_eyes_full_mask.png"
    ...
    mask_path = prepared_dir / "wave70_mf70_eyes_full_mask.png"
    overlay_path = prepared_dir / "wave70_mf70_eyes_full_overlay.png"
    mask.save(mask_path)
    mask.save(input_copy)
    overlay.save(overlay_path)

    protected_regions = ["pupils", "iris", "eyelids", "eyelashes", "eyebrows", "skin", "hair"]
    ...
    request = {
        "request_id": "wave70_mf70_eyes_full_local_support",
        ...
        "mask_layers": [{
            ...
            "body_region_id": "both_eyes_full_region",
    ...
    contract = {
        ...
        "wave70_taxonomy": {
            ...
            "body_part": "eyes",
            "subregion": "both_eyes_full_region",
    ...
    with patch_path.open("w", encoding="utf-8", newline="") as handle:
        ...
        writer.writeheader()
        writer.writerow({"workflow_id": "sdxl_realvisxl_inpaint_detail_lane", "node_id": "10", "input_name": "mask", "mask_id": MASK_ID, "mask_path": rel(input_copy, root), "pass_id": "wave70_mf70_eyes_full_local_support", "output_prefix": "codex_wave70_mf70_eyes_full"})
    ...
    quality_report = {
        ...
        "protected_neighbor_check": {
            ...
            "protected_regions": protected_regions,
            "notes": [
                "Mask covers two conservative eye aperture regions and avoids broad eyebrows, nose bridge, cheeks, background, and clothing.",
                "Subject-right eye has nearby hair occlusion, so generated proof must use very low denoise and strict whole-image QA for identity and gaze.",
                "This local support proof is not final completion until generated-output and target-runtime proof are present.",
            ],
        },
    }
    ...
    runtime_evidence = {
        "schema_version": "1.0",
        "evidence_id": f"W70-MF70-EYES-FULL-RUNTIME-EVIDENCE-{RUN_STAMP}",
        "timestamp": "2026-07-07T17:15:00-05:00",
    ...
    qa_evidence = {
        "schema_version": "1.0",
        "evidence_id": f"W70-MF70-EYES-FULL-LOCAL-SUPPORT-{RUN_STAMP}",
        "timestamp": "2026-07-07T17:15:00-05:00",
        "project_root": str(root),
        ...
        "mask_asset": {"mask_id": MASK_ID, "path": rel(mask_path, root), "sha256": mask_sha, "width": width, "height": height, "coverage_percent": metrics["coverage_percent"], "person_instance_id": "person_001", "body_region_id": "both_eyes_full_region", "scale": "minor"},
        ...
        "result": RESULT,
        "boundary": "This proves local Wave70 mf70_eyes_full mask artifact, overlay, quality score, and workflow routing support only. It does not mark the item complete because generated-output proof and target-runtime proof remain required.",
    }
    write_json(qa_path, qa_evidence)

    tracker_evidence = {
        "schema_version": "1.0",
        "tracker_evidence_id": f"W70_MF70_EYES_FULL_LOCAL_MASK_SUPPORT_{RUN_STAMP}",
        "created_at": "2026-07-07T17:15:00-05:00",
        "project_root": str(root),
        "tracker_id": TRACKER_ID,
        "item_id": ITEM_ID,
        "mask_type_id": MASK_TYPE_ID,
        "status": RESULT,
        "actual_work_performed": ["Generated deterministic local both-eyes mask PNG.", "Copied mask into ComfyUI input.", "Generated preview overlay for strict visual review.", "Created source-cited request, contract, validation, runtime evidence, quality report, and workflow routing manifest."],
