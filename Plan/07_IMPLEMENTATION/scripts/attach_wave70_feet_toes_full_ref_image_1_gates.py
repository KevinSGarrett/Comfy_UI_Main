...
ISO_TS = "2026-07-08T14:47:07-05:00"
FEET_STAMP = "20260708T144650-0500"
FULL_REF_STAMP = "20260708T143650-0500"
GATE_STAMP = "20260708T144707-0500"
TRACKER_ID = "TRK-W70-0169"
ITEM_ID = "ITEM-W70-0169"
NEXT_TRACKER_ID = "TRK-W70-0170"
NEXT_ITEM_ID = "ITEM-W70-0170"

STATUS_DECISION = "ref_image_1_full_body_feet_toes_references_available_route_not_complete_global_gates_pass"
COVERAGE_TOKENS = [
    ...
    "post_feet_toes_full_ref_image_1_geometry_gate_pass",
    "post_feet_toes_full_ref_image_1_promotion_gate_pass",
]
...
EVIDENCE_PATHS = [
    ...
    f"Plan/Instructions/QA/Evidence/Mask_Factory/Wave70/W70_FEET_TOES_CONTACT_AUTHORITY_{FEET_STAMP}.json",
    "Plan/Instructions/QA/Evidence/Mask_Factory/Wave70/feet_toes_authority.json",
    f"Plan/Tracker/Evidence/W70_FEET_TOES_CONTACT_AUTHORITY_{FEET_STAMP}.json",
    "Plan/Tracker/Evidence/feet_toes_authority.json",
    f"runtime_artifacts/mask_factory/wave70_feet_toes_contact_authority/{FEET_STAMP}/feet_toes_authority.json",
    f"runtime_artifacts/mask_factory/wave70_feet_toes_contact_authority/{FEET_STAMP}/feet_toes_contact_authority_reference_route_panel.png",
    "Plan/Instructions/QA/Evidence/Mask_Factory/Wave70/ref_image_1_body_mask_gold_standard.json",
    f"Plan/Instructions/QA/Evidence/Mask_Factory/Wave70/W70_MASK_GEOMETRY_HARD_GATE_POST_FEET_TOES_FULL_REF_IMAGE_1_{GATE_STAMP}.json",
    f"Plan/Instructions/QA/Evidence/Mask_Factory/Wave70/W70_MASK_PROMOTION_HARD_GATE_POST_FEET_TOES_FULL_REF_IMAGE_1_{GATE_STAMP}.json",
    f"Plan/Tracker/Evidence/W70_MASK_GEOMETRY_HARD_GATE_POST_FEET_TOES_FULL_REF_IMAGE_1_{GATE_STAMP}.json",
    f"Plan/Tracker/Evidence/W70_MASK_PROMOTION_HARD_GATE_POST_FEET_TOES_FULL_REF_IMAGE_1_{GATE_STAMP}.json",
]
...
HYDRATION_FILES = {
    "CURRENT_SESSION_STATE.md": "Session State Update - 0169 Full Reference Feet Toes Gates Passed",
    "CURRENT_PURSUING_GOAL.md": "Current Pursuing Goal Update - 0169 Full Reference Feet Toes Gates Passed",
    "RESUME_HERE_NEXT_CODEX_SESSION.md": "Resume Update - 0169 Full Reference Feet Toes Gates Passed",
    "NEXT_ACTION.md": "Immediate Next Action - 2026-07-08T14:47:07-05:00 - Re-evaluate TRK-W70-0170 Hair Body Skin Authority",
    "QA_EVIDENCE_INDEX.md": "Wave70 0169 Full Reference Feet Toes Gate Evidence",
}
...
def update_csv(path: Path, key: str, target: str) -> int:
    ...
    note = (
        "Feet toes contact authority 20260708T144650-0500: Ref_Image_1/Full provides 8 references, 7 eligible for "
        "feet/toes proof, and one user-corrected knees-to-head reference excluded from feet/toes/lower-leg proof. "
        "Ref_Image_1 gold standard provides 4 foot/toe masks. Prior portrait-only visibility blocker is superseded "
        "for this reference evaluation. Post-evaluation geometry and promotion gates passed with 332 checked rows, "
        "zero pass-like rows, and zero failures. Row remains Required_Not_Complete; no masks promoted."
    )
    changed = 0
    for row in rows:
...
def block(name: str) -> str:
    ...
    if name == "NEXT_ACTION.md":
        tail = f"Next exact local action: re-evaluate `{NEXT_TRACKER_ID}` / `{NEXT_ITEM_ID}` hair/body-skin authority with Ref_Image_1 gold masks and full-body context where applicable."
    elif name == "RESUME_HERE_NEXT_CODEX_SESSION.md":
        tail = f"Resume at `{NEXT_TRACKER_ID}` / `{NEXT_ITEM_ID}` hair/body-skin authority re-evaluation."
        ...
        tail = f"Current pursuit advances to `{NEXT_TRACKER_ID}` / `{NEXT_ITEM_ID}` hair/body-skin authority re-evaluation."
    else:
        tail = f"Next local action is `{NEXT_TRACKER_ID}` / `{NEXT_ITEM_ID}` hair/body-skin authority re-evaluation."
    return f"""## {heading}

Re-ran `{TRACKER_ID}` / `{ITEM_ID}` feet/toes/contact authority with the corrected body reference inputs.
...
def append_proof_log() -> None:
    row = [
        ISO_TS,
        "70",
        "Wave70 0169 feet toes full-reference rerun and gates passed",
        (
            "Reran TRK/ITEM-W70-0169 with Ref_Image_1/Full as body reference input, recorded 7 feet/toes eligible "
            "full-body references, excluded the knees-to-head reference, attached 4 foot/toe gold masks and post-rerun gates."
        ),
        "; ".join(EVIDENCE_PATHS),
        "python py_compile; Ref_Image_1/Full recursive manifest; Ref_Image_1 foot/toe gold mask manifest validation; JSON validation; reference panel inspection; Test-Wave70MaskGeometryGate.ps1; Test-Wave70MaskPromotionGate.ps1; CSV row verification",
        "FEET_TOES_FULL_REFERENCE_AVAILABLE_ROUTE_NOT_COMPLETE_GATES_PASS",
        f"Plan/Instructions/QA/Evidence/Mask_Factory/Wave70/W70_FEET_TOES_CONTACT_AUTHORITY_{FEET_STAMP}.json",
        f"Re-evaluate {NEXT_TRACKER_ID} / {NEXT_ITEM_ID} hair/body-skin authority; no promotion without row-level proof.",
    ]
