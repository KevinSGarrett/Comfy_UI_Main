...
RUN_STAMP = "20260707T214500-0500"
...
LANDMARKS = {
    ...
    "nose_bridge_top_right": [403, 326],
    "left_bridge_wall": [377, 360],
    "right_bridge_wall": [408, 360],
    "left_sidewall": [366, 394],
    "right_sidewall": [416, 393],
    "left_ala_outer": [360, 419],
    "right_ala_outer": [424, 419],
    "left_nostril_edge": [371, 431],
    "right_nostril_edge": [415, 431],
    "columella_base": [394, 436],
    "tip_highlight": [393, 415],
}
...
def make_mask(size: tuple[int, int]) -> Image.Image:
    ...
    # Hard guardrails keep this candidate above philtrum/lips and out of broad cheek/eye zones.
    draw.rectangle((0, 0, width, 314), fill=0)
    draw.rectangle((0, 438, width, height), fill=0)
    draw.rectangle((0, 0, 344, height), fill=0)
    draw.rectangle((436, 0, width, height), fill=0)
...
def make_panel(source: Image.Image, old_overlay: Image.Image, v1_overlay: Image.Image, v2_overlay: Image.Image, v2_mask: Image.Image, panel_path: Path) -> None:
    crop = crop_box()
    mask_rgb = Image.merge("RGB", (v2_mask, v2_mask, v2_mask))
    tiles = [
        label_tile(source.crop(crop), "source crop"),
        label_tile(old_overlay.crop(crop), "old failed overlay"),
        label_tile(v1_overlay.crop(crop), "v1 candidate"),
        label_tile(v2_overlay.crop(crop), "v2 tightened candidate"),
...
def main() -> int:
    ...
    old_overlay = Image.open(root / "Plan/Instructions/Operations/Prepared_Input_Assets/wave70_mf70_nose_20260707T204500-0500/wave70_mf70_nose_overlay.png").convert("RGB")
    v1_overlay = Image.open(root / "Plan/Instructions/Operations/Prepared_Input_Assets/wave70_mf70_nose_source_landmark_20260707T212500-0500/wave70_mf70_nose_overlay.png").convert("RGB")

    prepared_dir = root / "Plan" / "Instructions" / "Operations" / "Prepared_Input_Assets" / f"wave70_mf70_nose_source_landmark_v2_{RUN_STAMP}"
    audit_dir = root / "runtime_artifacts" / "mask_factory" / "wave70_mf70_nose" / "source_landmark_repair_v2"
    ...
    landmarks_path = audit_dir / f"mf70_nose_source_landmarks_v2_{RUN_STAMP}.json"
    panel_path = audit_dir / f"mf70_nose_source_landmark_repair_v2_panel_{RUN_STAMP}.png"
    mask.save(mask_path)
    ...
    write_json(
        ...
        {
            "schema_version": "1.0",
            "timestamp": "2026-07-07T21:45:00-05:00",
            ...
            "landmarks": LANDMARKS,
            "changes_from_v1": [
                "Raised lower guard from y=450 to y=438 to reduce philtrum/upper-lip contact.",
                "Narrowed cheek guardrails from x<336/x>442 to x<344/x>436.",
                "Reduced nostril support ellipses and ala width.",
            ],
        },
    )
    make_panel(source, old_overlay, v1_overlay, overlay, mask, panel_path)

    evidence_path = root / "Plan" / "Instructions" / "QA" / "Evidence" / "Mask_Factory" / "Wave70" / f"W70_MF70_NOSE_SOURCE_LANDMARK_REPAIR_V2_{RUN_STAMP}.json"
    tracker_path = root / "Plan" / "Tracker" / "Evidence" / f"W70_MF70_NOSE_SOURCE_LANDMARK_REPAIR_V2_{RUN_STAMP}.json"
    metrics = count_pixels(mask)
    evidence = {
        "schema_version": "1.0",
        "evidence_id": f"W70-MF70-NOSE-SOURCE-LANDMARK-REPAIR-V2-{RUN_STAMP}",
        "timestamp": "2026-07-07T21:45:00-05:00",
        "project_root": str(root),
        "qa_type": "wave70_mf70_nose_source_specific_landmark_repair_candidate_v2",
        ...
        "source_image": rel(source_path, root),
        "source_image_sha256": sha256_file(source_path),
        "prior_candidate_evidence": PRIOR_CANDIDATE_EVIDENCE,
        "prior_protected_overlap_audit": PRIOR_OVERLAP_AUDIT,
        "artifacts": {
        ...
        "hashes": {
            ...
            "landmarks_sha256": sha256_file(landmarks_path),
        ...
        "repair_candidate": {
            ...
            "status": "source_landmark_repair_v2_candidate_pending_protected_overlap_audit",
        },
        "result": "mf70_nose_source_landmark_repair_v2_candidate_created_pending_protected_overlap_audit",
        "next_required_action": "Run protected-overlap audit against v2 candidate before any pass or generated-output proof.",
    }
    write_json(evidence_path, evidence)
    tracker = {
        "schema_version": "1.0",
        "tracker_evidence_id": f"W70_MF70_NOSE_SOURCE_LANDMARK_REPAIR_V2_{RUN_STAMP}",
        "created_at": "2026-07-07T21:45:00-05:00",
        "project_root": str(root),
        "tracker_id": TRACKER_ID,
        "item_id": ITEM_ID,
        "mask_type_id": MASK_TYPE_ID,
        ...
        "result": "source_landmark_repair_v2_candidate_created_without_pass_promotion",
        "local_only": True,
        "ec2_started": False,
        "generation_executed": False,
    }
