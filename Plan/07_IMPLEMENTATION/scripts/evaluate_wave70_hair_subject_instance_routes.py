...
EVIDENCE_ID = f"W70_MF70_HAIR_SUBJECT_INSTANCE_ROUTE_EVAL_{RUN_STAMP}"
RUNTIME_DIR = PROJECT_ROOT / "runtime_artifacts" / "mask_factory" / "wave70_mf70_hair_subject_instance_routes" / RUN_STAMP
...
def keep_components_touching_zone(hair: np.ndarray, zone: np.ndarray) -> np.ndarray:
    labels_count, labels, _, _ = cv2.connectedComponentsWithStats(hair.astype(np.uint8), 8)
    if labels_count <= 1:
        return hair
    out = np.zeros_like(hair, dtype=np.uint8)
    for idx in range(1, labels_count):
        component = labels == idx
        if np.logical_and(component, zone > 0).any():
            out[component] = 1
    return out
...
def collect_records() -> tuple[list[dict[str, Any]], dict[str, str]]:
    ...
    samples = [sample for sample in grouped.values() if REGION in sample["regions"]]
...
def make_routes() -> dict[str, Route]:
    routes: dict[str, Route] = {"identity": lambda hair, anchor: hair}
    for radius in (8, 12, 16, 24, 32, 48, 64):
        routes[f"components_touch_anchor_dilate_r{radius}"] = (
            lambda hair, anchor, radius=radius: keep_components_touching_zone(hair, cv2.dilate(largest_component(anchor), kernel(radius)))
        )
    for expand_x in (0.45, 0.65, 0.85, 1.05, 1.25):
        for expand_top in (0.6, 0.9, 1.2, 1.5):
            for expand_bottom in (0.1, 0.25, 0.45):
                name = f"anchor_window_x{expand_x}_t{expand_top}_b{expand_bottom}"
                routes[name] = (
                    lambda hair, anchor, expand_x=expand_x, expand_top=expand_top, expand_bottom=expand_bottom:
                    hair & bbox_window(anchor, expand_x, expand_top, expand_bottom)
                )
                name2 = f"components_touch_{name}"
                routes[name2] = (
                    lambda hair, anchor, expand_x=expand_x, expand_top=expand_top, expand_bottom=expand_bottom:
                    keep_components_touching_zone(hair, bbox_window(anchor, expand_x, expand_top, expand_bottom))
                )
    return routes


def evaluate(samples: list[dict[str, Any]]) -> tuple[list[dict[str, Any]], list[dict[str, Any]], list[dict[str, Any]]]:
    routes = make_routes()
    ...
    best_route = route_records[0]["route"]
    return route_records, sample_metrics_by_route[best_route], make_panels(samples, best_route, routes[best_route])
...
def make_panels(samples: list[dict[str, Any]], best_route: str, route: Route) -> list[dict[str, Any]]:
    ...
    for panel_index, chunk in enumerate([item for item in chunks if item]):
        ...
        for sample in chunk:
            ...
            cells.extend(
                [
                    ...
                    tile(mask_rgb(largest_component(anchor), (210, 90, 255)), "primary anchor", "largest face anchor"),
        ...
        panel_path = PANEL_DIR / f"mf70_hair_subject_instance_{best_route}_panel_{panel_index + 1}.png"
...
def main() -> int:
    ...
    route_records, best_sample_metrics, panels = evaluate(samples)
    best = route_records[0]
    evidence = {
        "evidence_id": EVIDENCE_ID,
        "timestamp": RUN_STAMP,
        "scope": "mf70_hair subject-instance-aware route evaluation using combined CelebAMask-HQ and LaPa gold masks",
        "local_only": True,
        "ec2_started": False,
        ...
        "route_family": "subject-instance hair ownership from largest face/feature anchor, expanded anchor windows, and connected components touching anchor zones",
        ...
        "result": (
            "mf70_hair_subject_instance_route_candidate_found_no_promotion"
            if best["pass_gate"]
            else "mf70_hair_subject_instance_routes_blocked_stronger_model_or_policy_required"
        ),
        "next_required_action": (
            "If candidate found, proceed only to target-specific source-overlay review. If blocked, use a stronger person-instance "
            "segmentation or dataset-policy split before any target-portrait hair proof."
        ),
    }
