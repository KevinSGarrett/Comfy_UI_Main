...
EVIDENCE_ID = f"W70_LIPS_COMBINED_AUTHORITY_POLICY_DECISION_{RUN_STAMP}"
...
def main() -> int:
    ...
    lapa_record = lapa_region_record(lapa_gate, "mf70_lips_combined")
    postprocess_record = postprocess_region_record(postprocess, "mf70_lips_combined")
    mediapipe_record = mediapipe_region_record(mediapipe, "mf70_lips_combined")
    if lapa_record is None:
        raise RuntimeError("LaPa gate has no mf70_lips_combined record")
    if postprocess_record is None:
        raise RuntimeError("Postprocess eval has no mf70_lips_combined record")
    if mediapipe_record is None:
        raise RuntimeError("MediaPipe eval has no mf70_lips_combined record")
    ...
    policy_options = [
        {
            "policy": "promote_current_lapa_lips_combined_route",
            "decision": "rejected",
            "reason": "The current LaPa lips-combined route fails mean IoU and false-positive gates.",
        },
        {
        ...
        },
        {
            ...
            "decision": "rejected",
            "reason": "The MediaPipe landmark route overfills lips-combined and fails the current gold gate.",
        },
        {
            "policy": "fail_closed_until_boundary_aware_combined_lip_authority_or_explicit_row_policy",
            "decision": "selected",
            "reason": (
                "Current parser, postprocess, and MediaPipe landmark routes all fail gold-backed lips-combined gates. "
                "The row needs a boundary-aware combined-lip authority, stronger semantic parser/landmark route, or explicit row policy."
            ),
        },
    ]
    ...
    evidence = {
        ...
        "timestamp": RUN_STAMP,
        "scope": "Wave70 mf70_lips_combined authority-policy decision from current LaPa, postprocess, and MediaPipe evidence",
        "local_only": True,
        "ec2_started": False,
        "generation_executed": False,
        "active_input_mask_overwritten": False,
        "mask_promoted": False,
        "region": "mf70_lips_combined",
        "gold_mask_authority": {
        ...
        "lapa_lips_combined_gate_record": lapa_record,
        "combined_gold_postprocess_record": postprocess_record,
        "mediapipe_lips_combined_record": mediapipe_record,
        "lapa_lips_combined_policy_pass": lapa_pass,
        "combined_gold_postprocess_policy_pass": postprocess_pass,
        "mediapipe_lips_combined_policy_pass": mediapipe_pass,
        "current_lips_combined_policy_pass": current_policy_pass,
        "policy_options": policy_options,
        "selected_policy": "fail_closed_until_boundary_aware_combined_lip_authority_or_explicit_row_policy",
        "result": "mf70_lips_combined_authority_policy_fail_closed_no_promotion",
        "decision": (
            "Do not promote or target-proof mf70_lips_combined from the current LaPa route, combined-gold postprocess route, "
            "or MediaPipe landmark route. Resume this row only with a boundary-aware combined-lip authority, stronger semantic parser/landmark route, "
            "or explicit row policy backed by gold evidence."
        ),
        "next_required_action": (
            "Switch to another local gold-backed blocked row, or introduce a boundary-aware combined-lip authority / explicit row policy before any new lips-combined proof."
        ),
    }
