...
and a prepared human listening request. Classifies remaining blockers by named
class and exact codes. Never claims production COMPLETE, fabricates listening
authority, steals :8188, touches Row075, decodes full-library PCM, or writes
sound CSV rows.
"""
...
def verify_continuity_diagnostic(evaluation: dict[str, Any]) -> dict[str, Any]:
    ...
    if independent_refs != 1:
        raise ProofError(f"expected exactly one independent source reference, observed {independent_refs}")
    ...
    return {
        ...
        "note": (
            ...
            "reference. It does not satisfy Row124 multi-source-reference authority or leakage gates."
        ),
    }
    ...
    blockers.append(
        {
            ...
            "cleared_by_this_packet": [],
            "summary": (
                "The public-domain evaluation reference is not a locked production character "
                "identity authority."
            ),
...
def build_listening_request(root: Path, candidate: Path, automated_evidence: list[Path]) -> dict[str, Any]:
...
def build_proof_packet(
    ...
    *,
    ...
    raw_reference = reference_meta.get("path")
    if raw_reference:
        reference_path = Path(str(raw_reference))
        if not reference_path.is_file():
            # Durable path may differ from original runtime path; hash-only authority remains.
            reference_path = None
    ...
    blockers = classify_blockers(
        independent_source_reference_count=continuity["independent_source_reference_count"],
        listening_request_prepared=True,
    ...
    checks = [
        ...
        {"name": "R124-P002_continuity_multi_line_diagnostic_bound", "result": "pass"},
        {
            ...
            "result": "fail",
            "blocker_code": "INDEPENDENT_SOURCE_REFERENCE_COUNT_BELOW_TWO",
        },
        {
            "name": "R124-P004_multi_ref_drift_leakage_matrix_complete",
            "result": "fail",
            "blocker_code": "MULTI_REF_DRIFT_LEAKAGE_MATRIX_INCOMPLETE",
        },
        {
    ...
    packet = {
        ...
        "status": "BLOCKED_MULTI_REF_AND_LISTENING_AUTHORITY_PENDING",
        "decision": {
            "status": "blocked",
            "row_complete": False,
            ...
            "multi_source_reference_authority": False,
            "safe_next_action": (
                "Retain OFFLINE_PROOF_BOUNDED packet. Acquire/lock a second disjoint source "
                "reference for the same production character, complete multi-ref drift/leakage "
                "matrix evaluation, clear raw timing or document an approved timing waiver, then "
                "execute independent human listening against the prepared request. Do not steal "
                ":8188, touch Row075, decode full-library PCM, write sound CSV, or claim COMPLETE."
            ),
        },
        "bindings": {
            ...
            "reference_path_resolved": (
                display_relative(root, reference_path) if reference_path is not None else None
            ),
            "listening_review_request": listening_rel,
            "packager_script": display_relative(root, root / "Plan/07_IMPLEMENTATION/scripts/package_wave64_speech_row124_multi_ref_listening_proof.py"),
        },
        "continuity_diagnostic": continuity,
        "gate_snapshot": gate_snapshot,
        "multi_ref_continuity_contract": {
            "required_independent_source_references": 2,
            "observed_independent_source_references": continuity["independent_source_reference_count"],
            "required_checks": [
        ...
        json.dumps(
            {
                ...
                "blocker_classes": [
                    {"class": item["class"], "codes": item["codes"]} for item in packet["blocker_classes"]
                ],
