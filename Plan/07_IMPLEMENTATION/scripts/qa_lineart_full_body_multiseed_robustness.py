#!/usr/bin/env python3
"""Evaluate three-seed Lineart full-body local robustness."""
...
EXPECTED_SEEDS = (711370301, 711370302, 711370303)
EXPECTED_LANE = "sdxl_realvisxl_controlnet_lineart_lane"
EXPECTED_CONTROL_IMAGE = "controlnet_lineart_full_body_standing_w70_v1.png"
EXPECTED_CONTROLNET = "controlnet-lineart-sdxl-fp16.safetensors"
EXPECTED_DIMENSIONS = (704, 1056)
...
PREPARATION_MANIFEST = ROOT / (
    ...
    "lineart_full_body_standing_w70_v1/PREPARATION_MANIFEST.json"
)
PROFILE_PATHS = {
    711370301: ROOT
    / "PromptProfiles/base_generation/controlnet_lineart_v1_followup/lineart_full_body_standing_robustness_seed711370301.json",
    711370302: ROOT
    / "PromptProfiles/base_generation/controlnet_lineart_v1_followup/lineart_full_body_standing_robustness_seed711370302.json",
    711370303: ROOT
    / "PromptProfiles/base_generation/controlnet_lineart_v1_followup/lineart_full_body_standing_robustness_seed711370303.json",
}
...
VISUAL_CHECKS = (
    ...
    "both_feet_visible",
    "coherent_limbs",
    "no_lineart_leakage_or_ink_staining",
    "natural_skin_and_clothing_backdrop_coherence",
)
...
def prompt_binding_checks(
    ...
    return {
        "model_asset_bound": checkpoint.get("ckpt_name") == patch["model_asset"],
        "controlnet_asset_bound": controlnet.get("control_net_name")
        == EXPECTED_CONTROLNET,
...
def run_record(
    ...
    if np is None:
        raise ModuleNotFoundError("numpy is required for Lineart robustness QA")
    if Image is None or ImageChops is None:
        raise ModuleNotFoundError("Pillow is required for Lineart robustness QA")
    ...
    checks = {
        ...
        "package_lane_is_canonical_lineart": package.get("lane_id") == EXPECTED_LANE,
        "profile_lane_is_canonical_lineart": profile.get("target_lane_id")
        == EXPECTED_LANE,
        ...
        "output_is_704x1056": (width, height) == EXPECTED_DIMENSIONS,
...
def main() -> int:
    ...
    parser.add_argument("--runtime-711370301", type=Path, required=True)
    parser.add_argument("--image-711370301", type=Path, required=True)
    parser.add_argument("--runtime-711370302", type=Path, required=True)
    parser.add_argument("--image-711370302", type=Path, required=True)
    parser.add_argument("--runtime-711370303", type=Path, required=True)
    parser.add_argument("--image-711370303", type=Path, required=True)
    ...
    runtime_image_paths = {
        711370301: (
            args.runtime_711370301.resolve(),
            args.image_711370301.resolve(),
        ),
        711370302: (
            args.runtime_711370302.resolve(),
            args.image_711370302.resolve(),
        ),
        711370303: (
            args.runtime_711370303.resolve(),
            args.image_711370303.resolve(),
        ),
    }
    ...
    if missing:
        raise FileNotFoundError(f"Required Lineart robustness QA input missing: {missing}")
    ...
    profile_invariance = (
        normalize_profile_for_invariance(profiles[711370301])
        == normalize_profile_for_invariance(profiles[711370302])
        == normalize_profile_for_invariance(profiles[711370303])
    )
    ...
    if not technical_pass:
        result = "fail_local_lineart_full_body_multiseed_technical_qa"
        ...
        result = "pass_local_lineart_full_body_multiseed_strict_visual_qa"
    else:
        result = "fail_local_lineart_full_body_multiseed_visual_contract"

    evidence = {
        "schema_version": "1.0",
        "evidence_id": "W70-LOCAL-LINEART-FULL-BODY-MULTISEED-ROBUSTNESS-QA",
        ...
        "strict_decision": {
            ...
            "lineart_final_certification_allowed": False,
