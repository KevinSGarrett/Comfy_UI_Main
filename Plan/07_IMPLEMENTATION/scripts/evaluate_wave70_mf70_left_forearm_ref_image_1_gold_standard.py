...
EVIDENCE_ID = f"W70_MF70_LEFT_FOREARM_REF_IMAGE_1_GOLD_STANDARD_{RUN_STAMP}"
SCRIPT_REL = "Plan/07_IMPLEMENTATION/scripts/evaluate_wave70_mf70_left_forearm_ref_image_1_gold_standard.py"

RUNTIME_DIR = PROJECT_ROOT / "runtime_artifacts/mask_factory/wave70_mf70_left_forearm_ref_image_1" / RUN_STAMP
...
def make_panel(primary_record: dict[str, object], aggregate_record: dict[str, object]) -> Path:
    ...
    title_lines = [
        "TRK-W70-0157 / ITEM-W70-0157 - mf70_left_forearm",
        "Ref_Image_1 gold-standard left-forearm masks are available.",
    ...
    panel_path = RUNTIME_DIR / "mf70_left_forearm_ref_image_1_gold_standard_panel.png"
    ...
    for csv_path, target_id in targets:
        ...
        for row in rows:
            ...
            if "Status_Decision" in row:
                row["Status_Decision"] = "ref_image_1_left_forearm_gold_mask_available_route_not_complete"
            ...
            if "Coverage_Audit_Status" in row:
                row["Coverage_Audit_Status"] = append_unique(
                    ...
                    ["ref_image_1_left_forearm_gold_mask_available_route_not_complete"],
                )
            if "Notes" in row:
...
def update_hydration(evidence_paths: list[str]) -> None:
    ...
    body = f"""Re-evaluated `TRK-W70-0157` / `ITEM-W70-0157` against the corrected Ref_Image_1 gold-standard body-mask manifest.
...
Next local action: continue with `TRK-W70-0158` / `ITEM-W70-0158`, `mf70_right_forearm`, using Ref_Image_1 gold masks under the same non-promotional rules."""
    prepend_section(HYDRATION_DIR / "CURRENT_SESSION_STATE.md", f"## Session State Update - 0157 Ref_Image_1 Left Forearm Evaluated - {ISO_STAMP}", body)
    prepend_section(HYDRATION_DIR / "CURRENT_PURSUING_GOAL.md", f"## Wave70 0157 Ref_Image_1 Left Forearm Evaluated - {ISO_STAMP}", body)
    prepend_section(HYDRATION_DIR / "NEXT_ACTION.md", f"## Immediate Next Action - {ISO_STAMP} - Work TRK-W70-0158 Right Forearm With Ref_Image_1", body)
    prepend_section(HYDRATION_DIR / "QA_EVIDENCE_INDEX.md", f"## Wave70 0157 Ref_Image_1 Left Forearm Evidence - {ISO_STAMP}", body)
    prepend_section(HYDRATION_DIR / "RESUME_HERE_NEXT_CODEX_SESSION.md", f"## Resume Update - 0157 Ref_Image_1 Left Forearm Evaluated - {ISO_STAMP}", body)
    ...
    with proof_log.open("a", newline="", encoding="utf-8") as handle:
        ...
        writer.writerow(
            [
                ISO_STAMP,
                "70",
                "Wave70 0157 Ref_Image_1 left forearm evaluation",
                "Validated corrected Ref_Image_1 left forearm gold masks and attached row-level evidence while keeping TRK/ITEM-W70-0157 Required_Not_Complete and non-promotional.",
                "; ".join(evidence_paths),
                ...
                "Work TRK-W70-0158 right forearm with Ref_Image_1 gold masks.",
            ]
        )
...
def main() -> int:
    ...
    primary = find_extracted_record(manifest, "arms_left_lower_arm")
    ...
    if not primary_mask.exists() or not aggregate_mask.exists():
        raise FileNotFoundError("Missing Ref_Image_1 left forearm mask artifacts")
    ...
    qa_canonical_path = QA_DIR / "mf70_left_forearm_ref_image_1_gold_standard.json"
    tracker_evidence_path = TRACKER_EVIDENCE_DIR / f"{EVIDENCE_ID}.json"
    tracker_canonical_path = TRACKER_EVIDENCE_DIR / "mf70_left_forearm_ref_image_1_gold_standard.json"
    runtime_evidence_path = RUNTIME_DIR / "mf70_left_forearm_ref_image_1_gold_standard.json"
    ...
    note = (
        f"Ref_Image_1 left forearm row evaluation {RUN_STAMP}: corrected gold-standard masks exist for "
        "mf70_left_forearm. Top strip is partial upper-body only; lower strip is primary full-body validation. "
    ...
    payload = {
        ...
        "script": SCRIPT_REL,
        "task": "Re-evaluate mf70_left_forearm for TRK-W70-0157 / ITEM-W70-0157 using Ref_Image_1 gold-standard body masks.",
        "environment": {
        ...
        "left_forearm_gold_masks": {
            "primary_left_lower_arm": primary,
        ...
        "row_evaluation": {
            "tracker_id": "TRK-W70-0157",
            "item_id": "ITEM-W70-0157",
            "mask_type_id": "mf70_left_forearm",
            "ref_image_1_gold_standard_available_pass": True,
            "left_forearm_gold_mask_exists_pass": True,
            ...
            "obsolete_prior_blocker": "portrait_only_left_forearm_not_source_visible",
            "current_decision": "ref_image_1_left_forearm_gold_mask_available_route_not_complete",
        },
        "tracker_item_updates": row_updates,
        "next_step": "Work TRK-W70-0158 / ITEM-W70-0158 mf70_right_forearm using Ref_Image_1 gold masks under non-promotional rules.",
    }
