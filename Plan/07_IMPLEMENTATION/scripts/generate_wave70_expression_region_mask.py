...
RUN_STAMP = "20260707T151500-0500"
MASK_TYPE_ID = "mf70_expression_region"
TRACKER_ID = "TRK-W70-0003"
ITEM_ID = "ITEM-W70-0003"
CONTRACT_ID = "mask_contract_wave70_mf70_expression_region"
SCENE_ID = "scene_wave70_canny_v3_expression_region"
MASK_ID = "scene_wave70_canny_v3__person_001__mf70_expression_region__minor"
...
def normalized_face_polygon(width: int, height: int) -> list[tuple[int, int]]:
    points = [
        (0.505, 0.300),
        (0.382, 0.335),
        (0.325, 0.475),
...
def make_mask(source: Image.Image) -> Image.Image:
    ...
    draw.polygon(normalized_face_polygon(width, height), fill=255)
    return base.filter(ImageFilter.GaussianBlur(radius=max(3, width // 160)))
...
def make_overlay(source: Image.Image, mask: Image.Image) -> Image.Image:
    ...
    polygon = normalized_face_polygon(*rgba.size)
...
def main() -> int:
    ...
    package_dir = root / "runtime_artifacts" / "mask_factory" / "wave70_mf70_expression_region"
    prepared_dir = root / "Plan" / "Instructions" / "Operations" / "Prepared_Input_Assets" / f"wave70_mf70_expression_region_{RUN_STAMP}"
    qa_path = root / "Plan" / "Instructions" / "QA" / "Evidence" / "Mask_Factory" / "Wave70" / "mf70_expression_region.json"
    tracker_path = root / "Plan" / "Tracker" / "Evidence" / f"W70_mf70_expression_region_LOCAL_MASK_SUPPORT_{RUN_STAMP}.json"
    input_copy = root / "ComfyUI" / "input" / "wave70_mf70_expression_region_mask.png"
    ...
    mask_path = prepared_dir / "wave70_mf70_expression_region_mask.png"
    overlay_path = prepared_dir / "wave70_mf70_expression_region_overlay.png"
    mask.save(mask_path)
    ...
    request = {
        "request_id": "wave70_mf70_expression_region_local_support",
        "contract_id": CONTRACT_ID,
        "scene_id": SCENE_ID,
        "expected_character_count": 1,
        "required_mask_scales": ["minor"],
        "required_masks": [MASK_TYPE_ID],
        "source_requirement": "Plan/07_IMPLEMENTATION/mask_factory/ULTIMATE_MASK_FACTORY_TAXONOMY.md#L23-L31",
        ...
        "mask_layers": [
            {
                ...
                "person_instance_id": "person_001",
                "body_region_id": "brows_eyes_cheeks_mouth_expression",
                "source": "deterministic_wave70_local_artifact",
                "routing_intent": "edit_or_protect_identity_region",
                "protected_regions": ["identity_anchor", "hairline", "teeth", "background"],
    ...
    contract = {
        ...
        "wave70_taxonomy": {
            ...
            "body_part": "face",
            "subregion": "brows_eyes_cheeks_mouth_expression",
            "role": "edit_or_protect_identity_region",
            "protected_regions": ["identity_anchor", "hairline", "teeth", "background"],
            "source_citation": "Plan/07_IMPLEMENTATION/mask_factory/ULTIMATE_MASK_FACTORY_TAXONOMY.md#L23-L31",
        },
    }
        ...
        writer.writerow(
            {
                ...
                "pass_id": "wave70_mf70_expression_region_local_support",
                "output_prefix": "codex_wave70_mf70_expression_region",
            }
        )
    ...
    quality_report = {
        ...
        "protected_neighbor_check": {
            ...
            "protected_regions": ["identity_anchor", "hairline", "teeth", "background"],
            "notes": [
                "Mask covers the brows, eyes, cheeks, nose bridge/tip, and outer mouth/lip expression area.",
                "Mask intentionally avoids broad forehead, chin, hairline, teeth, neck, clothing, and background.",
    ...
    runtime_evidence = {
        "schema_version": "1.0",
        "evidence_id": f"W70-MF70-EXPRESSION-REGION-RUNTIME-EVIDENCE-{RUN_STAMP}",
        "timestamp": "2026-07-07T14:55:00-05:00",
    ...
    qa_evidence = {
        "schema_version": "1.0",
        "evidence_id": f"W70-MF70-EXPRESSION-REGION-LOCAL-SUPPORT-{RUN_STAMP}",
        "timestamp": "2026-07-07T14:55:00-05:00",
        "project_root": str(root),
        ...
        "generation_executed": False,
        "source_requirements": [
            "Plan/07_IMPLEMENTATION/mask_factory/ULTIMATE_MASK_FACTORY_TAXONOMY.md#L23-L31",
        ...
        "overlay_visual_review": {
            "review_method": "direct local image inspection required after generation",
            "result": "pending_direct_overlay_visual_review",
            "findings": [],
        },
        "result": RESULT,
        "boundary": "This proves local Wave70 mf70_expression_region mask artifact, overlay, quality score, and workflow routing support only. It does not mark the item complete because generated-output proof and target-runtime proof remain required.",
    }
    write_json(qa_path, qa_evidence)

    tracker_evidence = {
        "schema_version": "1.0",
        "tracker_evidence_id": f"W70_mf70_expression_region_LOCAL_MASK_SUPPORT_{RUN_STAMP}",
        "created_at": "2026-07-07T14:55:00-05:00",
        "project_root": str(root),
        "tracker_id": TRACKER_ID,
        "item_id": ITEM_ID,
        "mask_type_id": MASK_TYPE_ID,
        ...
        "actual_work_performed": [
            "Generated deterministic local brows/eyes/cheeks/mouth expression-region mask PNG.",
