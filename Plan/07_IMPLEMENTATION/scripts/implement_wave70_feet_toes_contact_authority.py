...
EVIDENCE_ID = f"W70_FEET_TOES_CONTACT_AUTHORITY_{RUN_STAMP}"
SCRIPT_REL = "Plan/07_IMPLEMENTATION/scripts/implement_wave70_feet_toes_contact_authority.py"

QA_DIR = PROJECT_ROOT / "Plan/Instructions/QA/Evidence/Mask_Factory/Wave70"
TRACKER_EVIDENCE_DIR = PROJECT_ROOT / "Plan/Tracker/Evidence"
RUNTIME_DIR = PROJECT_ROOT / "runtime_artifacts/mask_factory/wave70_feet_toes_contact_authority" / RUN_STAMP
...
def foot_gold_masks() -> dict[str, object]:
    manifest = load_json(GOLD_STANDARD_MANIFEST)
    masks = []
    labels = ("feet", "foot", "toe")
    ...
    return {
        ...
        "foot_toe_reference_mask_count": len(masks),
        "foot_toe_reference_masks": masks,
        "layout_interpretation": {
            "top_strip": "partial upper-body / one-third-body reference only; absent feet/toes here are not failures",
            "lower_strip": "primary full-body feet/toes mask validation area",
        },
    }
...
def full_reference_summary() -> dict[str, object]:
    ...
    return {
        ...
        "feet_toes_eligible_full_body_reference_count": len(eligible),
        "feet_toes_excluded_near_full_reference_count": len(excluded),
        "excluded_near_full_references": [
            {
                "path": item.get("path", ""),
...
def combined_reference_matrix_summary() -> dict[str, object]:
    ...
    return {
        ...
        "ref_image_1_feet_toes_category_present": ref1.get("category_presence", {}).get("legs_feet_toes", False),
        "ref_image_2_feet_toes_category_present": ref2.get("category_presence", {}).get("legs_feet_toes", False),
...
def make_panel(full_refs: dict[str, object], gold_masks: dict[str, object], combined_refs: dict[str, object]) -> Path:
    ...
    panel_path = RUNTIME_DIR / "feet_toes_contact_authority_reference_route_panel.png"
    ...
    draw.text((20, 20), "TRK-W70-0169 feet/toes/contact authority", fill=(0, 0, 0), font=font)
    draw.text((20, 44), "Ref_Image_1+2 feet/toe references available; production route incomplete.", fill=(130, 0, 0), font=font)
    lines = [
        f"All Full refs: {full_refs['recursive_image_count']}",
        f"Feet/toes eligible refs: {full_refs['feet_toes_eligible_full_body_reference_count']}",
        f"Excluded knees-to-head refs: {full_refs['feet_toes_excluded_near_full_reference_count']}",
        f"Foot/toe gold masks: {gold_masks['foot_toe_reference_mask_count']}",
        ...
        "Do not use Full/New folder for feet/toes proof.",
        "No mask promoted.",
        "Still needed:",
        "- source-derived foot/toe landmarks or parser",
        "- contact/occlusion ownership",
        "- floor/support boundary",
        "- canonical foot/toe polygons",
    ...
    for csv_path, target_id in targets:
        ...
        for row in rows:
            ...
            if "Status_Decision" in row:
                row["Status_Decision"] = "ref_images_1_2_full_body_feet_toes_references_available_route_not_complete"
            ...
            if "Coverage_Audit_Status" in row:
                row["Coverage_Audit_Status"] = append_unique(
                    ...
                    [
                        "ref_image_1_full_body_feet_toes_references_available",
                        "ref_image_2_full_body_feet_toes_references_available",
                        "ref_images_1_2_combined_body_reference_matrix_available",
                        "ref_image_1_full_new_folder_excluded_from_feet_toes_proof",
                        "feet_toes_contact_route_not_complete_no_promotion",
...
def main() -> int:
    ...
    body_reference_matrix_pass = bool(
        full_refs["feet_toes_eligible_full_body_reference_count"] > 0
        and gold_masks["foot_toe_reference_mask_count"] > 0
        and combined_refs["combined_gold_mask_count"] >= gold_masks["foot_toe_reference_mask_count"]
    )

    qa_evidence_path = QA_DIR / f"{EVIDENCE_ID}.json"
    qa_canonical_path = QA_DIR / "feet_toes_authority.json"
    tracker_evidence_path = TRACKER_EVIDENCE_DIR / f"{EVIDENCE_ID}.json"
    tracker_canonical_path = TRACKER_EVIDENCE_DIR / "feet_toes_authority.json"
    runtime_evidence_path = RUNTIME_DIR / "feet_toes_authority.json"
    ...
    note = (
        f"Feet toes contact authority {RUN_STAMP}: Ref_Image_1/Full provides "
        f"{full_refs['recursive_image_count']} references, with "
        f"{full_refs['feet_toes_eligible_full_body_reference_count']} eligible for feet/toes proof and "
        f"{full_refs['feet_toes_excluded_near_full_reference_count']} user-corrected knees-to-head reference excluded. "
        ...
        f"{gold_masks['foot_toe_reference_mask_count']} foot/toe masks. "
        "The old portrait visibility blocker is superseded for this reference evaluation. Row remains Required_Not_Complete "
        "because source-derived foot/toe landmarks or parser output, contact ownership, floor/support boundary, canonical foot/toe polygons, "
    ...
    payload = {
        ...
        "task": "Implement feet toes toenails shoe sock and floor contact authority for TRK-W70-0169 / ITEM-W70-0169 using lower-limb eligible Ref_Image_1/Full references and Ref_Image_1 gold masks.",
        "script": SCRIPT_REL,
        ...
        "ref_image_1_foot_toe_gold_masks": gold_masks,
        ...
        "feet_toes_contact_authority": {
            "result": "required_not_complete",
            "reference_source_available": full_refs["feet_toes_eligible_full_body_reference_count"] > 0,
            "foot_toe_gold_masks_available": gold_masks["foot_toe_reference_mask_count"] > 0,
            ...
            "foot_region_pass": False,
            "toe_toenail_region_pass": False,
            "shoe_sock_region_pass": False,
            "support_surface_boundary_pass": False,
            ...
            "blocked_reason": (
                "eligible_reference_sources_and_gold_foot_toe_masks_available_but_production_route_missing_source_"
                "derived_foot_toe_landmarks_or_parser_contact_ownership_floor_boundary_canonical_polygons_and_promotion_evidence"
            ),
            "findings": [
                "Ref_Image_1/Full is recorded as the primary body reference set for feet/toes rows.",
                ...
                "The image under Ref_Image_1/Full/New folder is knees-to-head only and is excluded from feet/toes proof.",
                "Ref_Image_1 gold-standard masks include both-feet, left-foot, right-foot, and toes/feet references.",
                "The prior portrait-only visibility blocker is superseded for this reference evaluation.",
                "Reference availability does not by itself create source-derived foot/toe landmarks, parser output, contact ownership, floor boundary, canonical polygons, generated output, visual QA, or promotion proof.",
                "No mask geometry was generated, changed, or promoted from this evidence.",
            ],
        },
        "qa_decision": "ref_images_1_2_full_body_feet_toes_references_available_route_not_complete",
        "promotion_decision": "no_mask_promoted_no_active_input_changed_feet_toes_contact_route_not_complete",
        "tracker_item_updates": row_updates,
        "next_step": "Run Wave70 geometry and promotion hard gates, attach gate evidence to TRK-W70-0169 / ITEM-W70-0169, then continue to TRK-W70-0170 hair/body-skin authority.",
    }
        ...
        json.dumps(
            {
                ...
                "all_full_reference_count": full_refs["recursive_image_count"],
                "feet_toes_eligible_reference_count": full_refs["feet_toes_eligible_full_body_reference_count"],
                "excluded_knees_to_head_count": full_refs["feet_toes_excluded_near_full_reference_count"],
                "foot_toe_gold_mask_count": gold_masks["foot_toe_reference_mask_count"],
