...
EVIDENCE_ID = f"W70_MF70_BELLY_BUTTON_UMBILICUS_{RUN_STAMP}"
SCRIPT_REL = "Plan/07_IMPLEMENTATION/scripts/implement_wave70_mf70_belly_button_umbilicus.py"

QA_DIR = PROJECT_ROOT / "Plan/Instructions/QA/Evidence/Mask_Factory/Wave70"
TRACKER_EVIDENCE_DIR = PROJECT_ROOT / "Plan/Tracker/Evidence"
RUNTIME_DIR = PROJECT_ROOT / "runtime_artifacts/mask_factory/wave70_mf70_belly_button_umbilicus" / RUN_STAMP
...
PREREQ_FILES = {
    ...
    "body_geometry_dependency_probe": QA_DIR / "body_geometry_dependency_probe.json",
...
def read_prerequisite(path: Path) -> dict[str, object]:
    ...
    return {
        ...
        "result": payload.get("result") or deep_get(wb, ["result"]) or deep_get(mbga, ["result"]),
        ...
        "contact_occlusion_ownership_pass": deep_get(wb, ["contact_occlusion_ownership_pass"]),
        ...
        or payload.get("canonical_polygon_export_pass"),
        "blocked_reason": deep_get(wb, ["blocked_reason"]) or deep_get(mbga, ["blocked_reason"]) or payload.get("blocked_reason"),
    }
...
def evaluate_belly_button_readiness(prereqs: dict[str, object]) -> dict[str, object]:
    source_visibility = {
        "source_framing": "head_neck_upper_chest_portrait",
        "belly_button_visible": False,
        "abdomen_visible": False,
        "waist_visible": False,
        "clothing_occlusion": "visible torso surface is jacket/clothing, not abdomen skin",
        "source_region_blocker": "Blocked_Body_Part_Not_Visible",
    }
    required_passes = {
        "source_belly_button_visible": source_visibility["belly_button_visible"],
        ...
        "whole_body_geometry_authority_pass": any(
            ...
            for name in ["body_region_geometry_authority", "body_reference_matrix", "body_hand_contact_geometry_authority"]
        ),
        "pose_hand_dense_landmark_or_segmentation_pass": any(
            ...
            for name in ["pose_landmark_authority", "body_region_geometry_authority", "body_reference_matrix"]
        ),
        "semantic_human_part_parsing_pass": any(
            prereqs.get(name, {}).get("semantic_human_part_parsing_pass") is True
            for name in ["human_part_parsing_authority", "body_region_geometry_authority", "body_reference_matrix"]
        ),
        "body_region_geometry_pass": any(
            prereqs.get(name, {}).get("body_region_geometry_pass") is True
            for name in ["body_region_geometry_authority", "body_reference_matrix"]
        ),
...
def make_blocker_panel(source: Image.Image, evaluation: dict[str, object]) -> Path:
    ...
    panel_path = RUNTIME_DIR / "mf70_belly_button_umbilicus_blocker_panel.png"
    ...
    unavailable_top = int(height * 0.62)
    draw.rectangle([22, unavailable_top, width - 22, height - 22], outline=(230, 40, 40), width=4)
    draw.line([22, unavailable_top, width - 22, height - 22], fill=(230, 40, 40), width=3)
    draw.line([width - 22, unavailable_top, 22, height - 22], fill=(230, 40, 40), width=3)

    draw.rectangle([20, 20, width - 20, 300], fill=(255, 255, 255), outline=(230, 40, 40), width=3)
    lines = [
        "TRK-W70-0154 blocked",
        "mf70_belly_button_umbilicus not visible.",
        f"Failed requirements: {len(evaluation['failed_requirements'])}",
        "Active source is head / neck / jacket portrait.",
        "Abdomen and umbilicus are outside crop.",
...
def update_wave70_rows(evidence_paths: list[str], note: str) -> dict[str, int]:
    ...
    targets = [(path, "TRK-W70-0154") for path in TRACKER_FILES] + [(path, "ITEM-W70-0154") for path in ITEM_FILES]
    for csv_path, target_id in targets:
        ...
        for row in rows:
            ...
            if "Status_Decision" in row:
                row["Status_Decision"] = "blocked_exact_local_belly_button_umbilicus_not_visible"
            if "Coverage_Audit_Status" in row:
                row["Coverage_Audit_Status"] = append_unique(
                    row.get("Coverage_Audit_Status", ""),
                    ["blocked_exact_local_belly_button_umbilicus_not_visible"],
                )
            if "Notes" in row:
...
def update_hydration(evidence_paths: list[str]) -> None:
    ...
    current_body = f"""Wave70 remains the active local-first mask-geometry milestone. TRK-W70-0154 / ITEM-W70-0154 was worked locally and is exactly blocked: mf70_belly_button_umbilicus cannot be produced from the active source because the abdomen and umbilicus are not visible in the crop, and whole-body/model-backed geometry authority remains blocked.
...
Next highest-value local tracker row found from current CSV state is TRK-W70-0155 / ITEM-W70-0155, prove Mask Factory mask type mf70_left_upper_arm. Work it locally under the same authority rules; if the source/authority prerequisites remain blocked, write exact local blocker evidence and keep masks fail-closed."""
    next_body = f"""TRK-W70-0154 / ITEM-W70-0154 is exactly blocked with local source-visibility and whole-body authority evidence. The active portrait does not expose the belly button or abdomen, so drawing a shortcut umbilicus mask would invent hidden anatomy.
...
Next local task: implement or exactly block TRK-W70-0155 / ITEM-W70-0155, mf70_left_upper_arm. Use only source-derived whole-body/model-backed geometry evidence. Do not start EC2, run generated-output proof, checkpoint Git, rerun broad helper loops, or promote any mask."""
    session_body = (
        "Worked TRK-W70-0154 / ITEM-W70-0154 locally. mf70_belly_button_umbilicus is blocked because the "
        "active source crop does not show the abdomen or umbilicus, and whole-body/model-backed authority remains blocked. "
        "No mask artifact, overlay, generated output, or promotion occurred."
    )
    blocker_body = (
        "TRK-W70-0154 / ITEM-W70-0154: Blocked_Body_Part_Not_Visible / "
        "blocked_exact_local_belly_button_umbilicus_not_visible."
    )

    prepend_section(HYDRATION_DIR / "CURRENT_PURSUING_GOAL.md", f"## Wave70 Belly Button Umbilicus Blocked Locally - {ISO_STAMP}", current_body)
    prepend_section(HYDRATION_DIR / "NEXT_ACTION.md", f"## Immediate Next Action - {ISO_STAMP} - Work TRK-W70-0155 Left Upper Arm Locally", next_body)
    prepend_section(HYDRATION_DIR / "RESUME_HERE_NEXT_CODEX_SESSION.md", f"## Resume Update - Wave70 Belly Button Umbilicus Blocked - {ISO_STAMP}", session_body + "\n\nNext exact action: work TRK-W70-0155 locally.")
    prepend_section(HYDRATION_DIR / "CURRENT_SESSION_STATE.md", f"## Session State Update - Wave70 Belly Button Umbilicus Blocked - {ISO_STAMP}", session_body)
    prepend_section(HYDRATION_DIR / "QA_EVIDENCE_INDEX.md", f"## Wave70 Belly Button Umbilicus Evidence - {ISO_STAMP}", f"{session_body}\n\n{evidence_block}")
    prepend_section(HYDRATION_DIR / "BLOCKERS.md", f"## Wave70 Belly Button Umbilicus Blocker - {ISO_STAMP}", blocker_body + "\n\n" + evidence_block)
    ...
    with proof_log.open("a", newline="", encoding="utf-8") as handle:
        ...
        writer.writerow(
            [
                ISO_STAMP,
                "70",
                "Wave70 mf70 belly button umbilicus blocker",
                "Worked TRK/ITEM-W70-0154 locally. Active source does not expose abdomen/umbilicus and whole-body/model-backed authority remains blocked. Wrote stamped/canonical evidence, generated blocker panel, and kept masks fail-closed.",
                "; ".join(evidence_paths),
                ...
                "Next work TRK-W70-0155 left upper arm locally; write exact blocker if authority remains blocked.",
            ]
        )
...
def main() -> int:
    ...
    evaluation = evaluate_belly_button_readiness(prereqs)
    panel_path = make_blocker_panel(source, evaluation)

    qa_evidence_path = QA_DIR / f"{EVIDENCE_ID}.json"
    qa_canonical_path = QA_DIR / "mf70_belly_button_umbilicus.json"
    tracker_evidence_path = TRACKER_EVIDENCE_DIR / f"{EVIDENCE_ID}.json"
    tracker_canonical_path = TRACKER_EVIDENCE_DIR / "mf70_belly_button_umbilicus.json"
    runtime_evidence_path = RUNTIME_DIR / "mf70_belly_button_umbilicus.json"
    ...
    note = (
        f"mf70_belly_button_umbilicus {RUN_STAMP}: exact local blocker. "
        "The active source crop does not expose abdomen or umbilicus, and whole-body/model-backed geometry authority "
        "remains blocked. No mask, overlay, generated output, or promotion occurred."
    )
    ...
    payload = {
        ...
        "task": "Prove or exactly block Mask Factory mask type mf70_belly_button_umbilicus for TRK-W70-0154 / ITEM-W70-0154.",
        "script": SCRIPT_REL,
        "source_image": {
        ...
        "belly_button_umbilicus_readiness": evaluation,
        "artifacts": {
        ...
        "mask_factory_row": {
            "mask_type_id": "mf70_belly_button_umbilicus",
            "target_region": "belly_button_umbilicus",
            "body_part": "torso",
            "subregion": "belly_button_umbilicus",
            "protected_regions": ["abdomen", "waist", "clothing", "hands"],
            "result": "blocked",
        ...
        "whole_body_geometry_authority": {
            ...
            "findings": [
                "The active source is a head/neck/upper-chest portrait with a jacket; abdomen skin and umbilicus are outside the crop.",
                "The requested belly-button/umbilicus region is not visible enough for source-derived geometry or protected-neighbor validation.",
                "Whole-body/model-backed authority prerequisites remain blocked, so no canonical body polygon can be exported.",
        ...
        "qa_decision": "blocked_exact_local_belly_button_umbilicus_not_visible",
        "promotion_decision": "no_mask_promoted_no_active_input_changed_belly_button_blocked",
        "tracker_item_updates": row_updates,
        "next_step": "Work TRK-W70-0155 / ITEM-W70-0155 mf70_left_upper_arm locally.",
    }
