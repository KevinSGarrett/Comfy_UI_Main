...
RUN_STAMP = "20260707T180000-0500"
MASK_TYPE_ID = "mf70_pupils_iris_sclera"
TRACKER_ID = "TRK-W70-0012"
ITEM_ID = "ITEM-W70-0012"
CONTRACT_ID = "mask_contract_wave70_mf70_pupils_iris_sclera"
SCENE_ID = "scene_wave70_canny_v3_pupils_iris_sclera"
MASK_ID = "scene_wave70_canny_v3__person_001__mf70_pupils_iris_sclera__minor"
RESULT = "pass_local_wave70_mask_artifact_routing_support_final_blocked_generated_output_target_runtime"
SOURCE_REQUIREMENT = "Plan/07_IMPLEMENTATION/mask_factory/ULTIMATE_MASK_FACTORY_TAXONOMY.md#L43"
...
def iris_boxes(width: int, height: int) -> list[tuple[int, int, int, int]]:
    return [
        (int(width * 0.393), int(height * 0.409), int(width * 0.441), int(height * 0.446)),
        (int(width * 0.536), int(height * 0.401), int(width * 0.589), int(height * 0.441)),
    ]
...
def make_mask(source: Image.Image) -> Image.Image:
    ...
    for box in iris_boxes(width, height):
        draw.ellipse(box, fill=255)
    return mask.filter(ImageFilter.GaussianBlur(radius=max(1, width // 420)))


def make_overlay(source: Image.Image, mask: Image.Image) -> Image.Image:
    rgba = source.convert("RGBA")
    color = Image.new("RGBA", rgba.size, (0, 255, 128, 0))
    alpha = mask.point(lambda v: min(170, int(v * 0.66)))
    color.putalpha(alpha)
    outline = Image.new("RGBA", rgba.size, (0, 0, 0, 0))
    draw = ImageDraw.Draw(outline)
    for box in iris_boxes(*rgba.size):
        draw.ellipse(box, outline=(255, 255, 255, 235), width=2)
...
def main() -> int:
    ...
    package_dir = root / "runtime_artifacts" / "mask_factory" / "wave70_mf70_pupils_iris_sclera"
    prepared_dir = root / "Plan" / "Instructions" / "Operations" / "Prepared_Input_Assets" / f"wave70_mf70_pupils_iris_sclera_{RUN_STAMP}"
    qa_path = root / "Plan" / "Instructions" / "QA" / "Evidence" / "Mask_Factory" / "Wave70" / "mf70_pupils_iris_sclera.json"
    tracker_path = root / "Plan" / "Tracker" / "Evidence" / f"W70_MF70_PUPILS_IRIS_SCLERA_LOCAL_MASK_SUPPORT_{RUN_STAMP}.json"
    ...
    mask_path = prepared_dir / "wave70_mf70_pupils_iris_sclera_mask.png"
    overlay_path = prepared_dir / "wave70_mf70_pupils_iris_sclera_overlay.png"
    mask.save(mask_path)
    mask.save(input_copy)
    overlay.save(overlay_path)
    metrics = count_pixels(mask)
    width, height = mask.size
    protected_regions = ["eyelids", "eyelashes", "catchlights", "skin", "hair_occlusion"]
    mask_sha = sha256_file(mask_path)
    overlay_sha = sha256_file(overlay_path)

    request = {
        "request_id": "wave70_mf70_pupils_iris_sclera_local_support",
        ...
        "mask_layers": [{"mask_id": MASK_ID, "scale": "minor", "target_type": "body_part", "person_instance_id": "person_001", "body_region_id": "pupils_iris_sclera", "source": "deterministic_wave70_local_artifact", "routing_intent": "edit_or_protect_facial_detail", "protected_regions": protected_regions, "wave70_mask_type_id": MASK_TYPE_ID}],
    }
    contract = {
        "contract_id": CONTRACT_ID,
        "scene_id": SCENE_ID,
        ...
        "wave70_taxonomy": {"mask_type_id": MASK_TYPE_ID, "body_part": "eyes", "subregion": "pupils_iris_sclera", "role": "edit_or_protect_facial_detail", "protected_regions": protected_regions, "source_citation": SOURCE_REQUIREMENT},
    }
    paths = {
    ...
    with paths["patch"].open("w", encoding="utf-8", newline="") as f:
        ...
        writer.writeheader()
        writer.writerow({"workflow_id": "sdxl_realvisxl_inpaint_detail_lane", "node_id": "10", "input_name": "mask", "mask_id": MASK_ID, "mask_path": rel(input_copy, root), "pass_id": "wave70_mf70_pupils_iris_sclera_local_support", "output_prefix": "codex_wave70_mf70_pupils_iris_sclera"})
    quality_score = 90.0
    write_json(paths["quality"], {"schema_version": "1.0", "mask_type_id": MASK_TYPE_ID, "mask_id": MASK_ID, "result": "pass_local_mask_quality_with_completion_gaps", "quality_score": quality_score, "minimum_required": 85, "quality_passed": True, "metrics": metrics, "protected_neighbor_check": {"result": "pass_with_notes", "protected_regions": protected_regions, "notes": ["Mask covers the visible pupil/iris/sclera cores of both eyes.", "Mask intentionally avoids broad eyelid, eyelash, eyebrow, cheek, nose bridge, skin, background, and clothing regions.", "Subject-right eye has nearby hair occlusion, so generated proof must use extremely low denoise and strict catchlight/gaze/hair QA."]}})
    runtime = {"schema_version": "1.0", "evidence_id": f"W70-MF70-PUPILS-IRIS-SCLERA-RUNTIME-EVIDENCE-{RUN_STAMP}", "timestamp": "2026-07-07T18:00:00-05:00", "project_root": str(root), "contract_id": CONTRACT_ID, "mask_type_id": MASK_TYPE_ID, "local_only": True, "aws_contacted": False, "github_api_contacted": False, "civitai_contacted": False, "ec2_started": False, "generation_executed": False, "source_requirement": SOURCE_REQUIREMENT, "source_image": rel(source_image, root), "source_image_sha256": sha256_file(source_image), "mask_png": rel(mask_path, root), "mask_sha256": mask_sha, "comfyui_input_copy": rel(input_copy, root), "comfyui_input_copy_sha256": sha256_file(input_copy), "preview_overlay": rel(overlay_path, root), "preview_overlay_sha256": overlay_sha, "width": width, "height": height, "metrics": metrics, "contract": rel(paths["contract"], root), "contract_validation": rel(paths["validation"], root), "quality_report": rel(paths["quality"], root), "workflow_routing_manifest": rel(paths["patch"], root)}
    write_json(paths["runtime"], runtime)
    qa = {"schema_version": "1.0", "evidence_id": f"W70-MF70-PUPILS-IRIS-SCLERA-LOCAL-SUPPORT-{RUN_STAMP}", "timestamp": "2026-07-07T18:00:00-05:00", "project_root": str(root), "qa_type": "wave70_ultimate_mask_factory_local_artifact_routing_support", "mask_type_id": MASK_TYPE_ID, "tracker_id": TRACKER_ID, "item_id": ITEM_ID, "local_only": True, "aws_contacted": False, "github_api_contacted": False, "civitai_contacted": False, "ec2_started": False, "generation_executed": False, "source_requirements": [SOURCE_REQUIREMENT, f"Plan/Tracker/wave70_ultimate_mask_factory_tracker.csv:{TRACKER_ID}", f"Plan/Items/wave70_ultimate_mask_factory_itemized_list.csv:{ITEM_ID}"], "artifacts": {"request": rel(paths["request"], root), "contract": rel(paths["contract"], root), "contract_validation": rel(paths["validation"], root), "runtime_evidence": rel(paths["runtime"], root), "quality_report": rel(paths["quality"], root), "patch_manifest": rel(paths["patch"], root), "mask_png": rel(mask_path, root), "comfyui_input_copy": rel(input_copy, root), "preview_overlay": rel(overlay_path, root)}, "mask_asset": {"mask_id": MASK_ID, "path": rel(mask_path, root), "sha256": mask_sha, "width": width, "height": height, "coverage_percent": metrics["coverage_percent"], "person_instance_id": "person_001", "body_region_id": "pupils_iris_sclera", "scale": "minor"}, "validation": {"contract_valid": True, "contract_errors": [], "contract_warnings": [], "quality_score": quality_score, "minimum_required": 85, "quality_passed": True, "workflow_routing_manifest_present": True, "preview_overlay_generated": True, "protected_neighbor_check_pass": True, "generated_output_proof_present": False, "target_runtime_proof_present": False, "blockers": ["generated_output_proof_required_before_item_completion", "target_runtime_proof_required_before_final_certification"]}, "overlay_visual_review": {"review_method": "direct local image inspection after generation", "result": "pending_direct_overlay_visual_review", "findings": []}, "result": RESULT, "boundary": "This proves local Wave70 mf70_pupils_iris_sclera mask artifact, overlay, quality score, and workflow routing support only. It does not mark the item complete because generated-output proof and target-runtime proof remain required."}
    write_json(qa_path, qa)
    tracker = {"schema_version": "1.0", "tracker_evidence_id": f"W70_MF70_PUPILS_IRIS_SCLERA_LOCAL_MASK_SUPPORT_{RUN_STAMP}", "created_at": "2026-07-07T18:00:00-05:00", "project_root": str(root), "tracker_id": TRACKER_ID, "item_id": ITEM_ID, "mask_type_id": MASK_TYPE_ID, "status": RESULT, "actual_work_performed": ["Generated deterministic local pupils/iris/sclera mask PNG.", "Copied mask into ComfyUI input.", "Generated preview overlay for strict visual review.", "Created source-cited request, contract, validation, runtime evidence, quality report, and workflow routing manifest."], "evidence": {"qa_evidence": rel(qa_path, root), "runtime_evidence": rel(paths["runtime"], root), "mask_png": rel(mask_path, root), "preview_overlay": rel(overlay_path, root), "comfyui_input_copy": rel(input_copy, root)}, "boundaries": {"local_only": True, "ec2_started": False, "generated_output_proof_present": False, "target_runtime_proof_present": False, "final_completion_allowed": False}, "next_action": "Perform direct overlay visual review, then run a bounded local generated-output proof before item completion. Target-runtime proof remains required before final certification."}
