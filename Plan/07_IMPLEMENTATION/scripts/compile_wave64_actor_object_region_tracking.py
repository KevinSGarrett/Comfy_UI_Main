...
def run_ci_fixture_ledger_gate(*, fixture_dir: Path | None = None) -> dict[str, Any]:
    ...
    refuses annotated tracking benchmark pass, visual-review, Row084 acceptance,
    and row completion claims.
    """
    directory = fixture_dir if fixture_dir is not None else DEFAULT_FIXTURE_DIR
    rebuilt = build_synthetic_tracking_benchmark_ledger(fixture_dir=directory)
    ...
    return {
        "status": "ok",
        "gate": "row085_ci_fixture_ledger_gate",
        ...
        "tracking_metric_expectation_count": expectation_receipt[
            "tracking_metric_expectation_count"
        ],
        "digest_drift_rejected": True,
        "production_benchmark": False,
        "annotated_tracking_benchmark_pass": False,
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "annotated tracking benchmark pass, visual review, or Row084 acceptance"
        ),
    )
    parser.add_argument(
        "--fixture-dir",
    ...
    if not args.input or not args.output:
        raise SystemExit(
            ...
            "verifying, or running the CI fixture ledger gate"
        )
