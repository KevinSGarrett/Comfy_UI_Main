...
RUN_STAMP = "20260707T174500-0500"
MASK_TYPE_ID = "mf70_right_eye"
TRACKER_ID = "TRK-W70-0011"
ITEM_ID = "ITEM-W70-0011"
CONTRACT_ID = "mask_contract_wave70_mf70_right_eye"
SCENE_ID = "scene_wave70_canny_v3_right_eye"
MASK_ID = "scene_wave70_canny_v3__person_001__mf70_right_eye__minor"
RESULT = "pass_local_wave70_mask_artifact_routing_support_final_blocked_generated_output_target_runtime"
SOURCE_REQUIREMENT = "Plan/07_IMPLEMENTATION/mask_factory/ULTIMATE_MASK_FACTORY_TAXONOMY.md#L42"
...
def right_eye_ellipse(width: int, height: int) -> tuple[int, int, int, int]:
    return (int(width * 0.365), int(height * 0.392), int(width * 0.461), int(height * 0.456))


def make_mask(source: Image.Image) -> Image.Image:
    width, height = source.size
    base = Image.new("L", (width, height), 0)
    ImageDraw.Draw(base).ellipse(right_eye_ellipse(width, height), fill=255)
    return base.filter(ImageFilter.GaussianBlur(radius=max(2, width // 275)))


def make_overlay(source: Image.Image, mask: Image.Image) -> Image.Image:
    rgba = source.convert("RGBA")
    color = Image.new("RGBA", rgba.size, (0, 255, 128, 0))
    alpha = mask.point(lambda value: min(155, int(value * 0.60)))
    color.putalpha(alpha)
    outline = Image.new("RGBA", rgba.size, (0, 0, 0, 0))
    ImageDraw.Draw(outline).ellipse(right_eye_ellipse(*rgba.size), outline=(255, 255, 255, 235), width=3)
...
def main() -> int:
    ...
    package_dir = root / "runtime_artifacts" / "mask_factory" / "wave70_mf70_right_eye"
    prepared_dir = root / "Plan" / "Instructions" / "Operations" / "Prepared_Input_Assets" / f"wave70_mf70_right_eye_{RUN_STAMP}"
    qa_path = root / "Plan" / "Instructions" / "QA" / "Evidence" / "Mask_Factory" / "Wave70" / "mf70_right_eye.json"
    tracker_path = root / "Plan" / "Tracker" / "Evidence" / f"W70_MF70_RIGHT_EYE_LOCAL_MASK_SUPPORT_{RUN_STAMP}.json"
    input_copy = root / "ComfyUI" / "input" / "wave70_mf70_right_eye_mask.png"
    ...
    mask_path = prepared_dir / "wave70_mf70_right_eye_mask.png"
    overlay_path = prepared_dir / "wave70_mf70_right_eye_overlay.png"
    mask.save(mask_path)
    mask.save(input_copy)
    overlay.save(overlay_path)

    protected_regions = ["pupil", "iris", "eyelids", "eyelashes", "skin", "eyebrow", "hair", "nose_bridge"]
    ...
    request = {
        "request_id": "wave70_mf70_right_eye_local_support",
        ...
        "item_id": ITEM_ID,
        "orientation_note": "right_eye means subject-right eye, visible on viewer-left side of the portrait near hair occlusion.",
        ...
        "mask_layers": [{
            ...
            "body_region_id": "right_eye_full_region",
    ...
    contract = {
        ...
        "wave70_taxonomy": {
            ...
            "body_part": "eyes",
            "subregion": "right_eye_full_region",
    ...
    with patch_path.open("w", encoding="utf-8", newline="") as handle:
        ...
        writer.writeheader()
        writer.writerow({"workflow_id": "sdxl_realvisxl_inpaint_detail_lane", "node_id": "10", "input_name": "mask", "mask_id": MASK_ID, "mask_path": rel(input_copy, root), "pass_id": "wave70_mf70_right_eye_local_support", "output_prefix": "codex_wave70_mf70_right_eye"})
    ...
    write_json(quality_path, {
        ...
        "protected_neighbor_check": {
            ...
            "protected_regions": protected_regions,
            "notes": [
                "Mask covers the subject-right eye aperture and immediate eyelid/eyelash region.",
                "Mask avoids broad eyebrow, nose bridge, cheek, background, and clothing.",
                "Nearby hair occlusion makes this more sensitive than the subject-left eye; generated proof must use very low denoise and strict gaze/hair QA.",
            ],
        },
    })
    ...
    runtime_evidence = {
        "schema_version": "1.0",
        "evidence_id": f"W70-MF70-RIGHT-EYE-RUNTIME-EVIDENCE-{RUN_STAMP}",
        "timestamp": "2026-07-07T17:45:00-05:00",
    ...
    qa_evidence = {
        "schema_version": "1.0",
        "evidence_id": f"W70-MF70-RIGHT-EYE-LOCAL-SUPPORT-{RUN_STAMP}",
        "timestamp": "2026-07-07T17:45:00-05:00",
        "project_root": str(root),
        ...
        "mask_asset": {"mask_id": MASK_ID, "path": rel(mask_path, root), "sha256": mask_sha, "width": width, "height": height, "coverage_percent": metrics["coverage_percent"], "person_instance_id": "person_001", "body_region_id": "right_eye_full_region", "scale": "minor"},
        ...
        "overlay_visual_review": {"review_method": "direct local image inspection after generation", "result": "pending_direct_overlay_visual_review", "findings": []},
        "result": RESULT,
        "boundary": "This proves local Wave70 mf70_right_eye mask artifact, overlay, quality score, and workflow routing support only. It does not mark the item complete because generated-output proof and target-runtime proof remain required.",
    }
    write_json(qa_path, qa_evidence)

    tracker_evidence = {
        "schema_version": "1.0",
        "tracker_evidence_id": f"W70_MF70_RIGHT_EYE_LOCAL_MASK_SUPPORT_{RUN_STAMP}",
        "created_at": "2026-07-07T17:45:00-05:00",
        "project_root": str(root),
        "tracker_id": TRACKER_ID,
        "item_id": ITEM_ID,
        "mask_type_id": MASK_TYPE_ID,
        "status": RESULT,
        "actual_work_performed": ["Generated deterministic local subject-right-eye mask PNG.", "Copied mask into ComfyUI input.", "Generated preview overlay for strict visual review.", "Created source-cited request, contract, validation, runtime evidence, quality report, and workflow routing manifest."],
