<#
.SYNOPSIS
Installs one EC2 ComfyUI input asset from S3 and verifies its SHA256.

.DESCRIPTION
Dry-run by default. With -Execute, starts the approved EC2 instance only long
enough to download one input asset from S3 into ComfyUI/input, verify SHA256,
then stops and verifies the instance is stopped. This is for LoadImage-style
runtime inputs, not model binaries.
#>
param(
...
if ([string]::IsNullOrWhiteSpace($FileName)) {
  if (![string]::IsNullOrWhiteSpace($SourceS3Uri)) {
    $FileName = [System.IO.Path]::GetFileName(([System.Uri]$SourceS3Uri.Replace("s3://", "https://s3.amazonaws.com/")).AbsolutePath)
  }
}

$stamp = (Get-Date -Format "yyyyMMddTHHmmsszzz").Replace(":", "")
if ([string]::IsNullOrWhiteSpace($OutFile)) {
  $OutFile = "C:\Comfy_UI_Main\Plan\Instructions\QA\Evidence\Runtime_Readiness\W68_EC2_INPUT_ASSET_INSTALL_$stamp.json"
}

$safeInputSubdir = $InputSubdir.Trim().Trim("/", "\")
if ($safeInputSubdir -match "\.\.") {
  throw "InputSubdir cannot contain '..'."
}
$remoteInputDir = "$RemoteComfyRoot/input"
if (![string]::IsNullOrWhiteSpace($safeInputSubdir)) {
  $remoteInputDir = "$remoteInputDir/$($safeInputSubdir.Replace('\','/'))"
}
$remoteInputPath = "$remoteInputDir/$FileName"

$record = [ordered]@{
  schema_version = "1.0"
  evidence_id = "EC2-INPUT-ASSET-INSTALL-$stamp"
  timestamp = (Get-Date).ToString("yyyy-MM-ddTHH:mm:sszzz")
  operation = "install_ec2_input_asset_from_s3"
  instance_id = $InstanceId
  region = $Region
  source_s3_uri = $SourceS3Uri
  remote_comfy_root = $RemoteComfyRoot
  remote_input_path = $remoteInputPath
  input_subdir = $safeInputSubdir
  file_name = $FileName
  ...
  git_lfs_used = $false
  result = $(if ($Execute) { "ready_for_input_asset_install_execute" } else { "dry_run_input_asset_install_plan" })
  failure_category = $null
  errors = @()
  next_action = "After input_asset_hash_verified, run EC2 workflow smoke that uses LoadImage."
}
...
if ([string]::IsNullOrWhiteSpace($FileName)) {
  $record.result = "blocked_missing_file_name"
  $record.failure_category = "missing_file_name"
}
...
try:
    ...
    os.makedirs(os.path.dirname(REMOTE_INPUT_PATH), exist_ok=True)
    tmp_dir = tempfile.mkdtemp(prefix="codex_input_asset_install_")
    tmp_path = os.path.join(tmp_dir, os.path.basename(REMOTE_INPUT_PATH))
    ...
    if actual != EXPECTED_SHA256:
        raise RuntimeError("input asset sha256 mismatch: expected %s observed %s" % (EXPECTED_SHA256, actual))
    shutil.move(tmp_path, REMOTE_INPUT_PATH)
    os.chmod(REMOTE_INPUT_PATH, 0o644)
    result["sha256_verified"] = sha256_file(REMOTE_INPUT_PATH).lower() == EXPECTED_SHA256
    result["size_bytes"] = os.path.getsize(REMOTE_INPUT_PATH)
    result["result"] = "input_asset_hash_verified" if result["sha256_verified"] else "input_asset_hash_failed_after_move"
...
try {
  ...
  if ($record.start_state -ne "running") {
    ...
    if ($record.start_exit_code -ne 0) {
      ...
      $record.result = "input_asset_install_start_failed"
  ...
  $payloadPath = Join-Path $env:TEMP "codex_input_asset_install_payload_$stamp.json"
...
if ($record.remote_result -and [string]$record.remote_result.result -eq "input_asset_hash_verified" -and $record.final_state -eq "stopped" -and @($record.errors).Count -eq 0) {
  $record.result = "input_asset_hash_verified"
  $record.failure_category = $null
  $record.next_action = "Run EC2 workflow smoke that uses LoadImage."
} else {
  if ($record.result -ne "input_asset_install_start_failed") {
    $record.result = "input_asset_install_failed"
  }
  if ([string]::IsNullOrWhiteSpace([string]$record.failure_category)) {
    $record.failure_category = "input_asset_install_failed"
  }
  if ($record.remote_result -and $record.remote_result.errors) {
    $record.errors += @($record.remote_result.errors)
  }
}
