...
Every third-party action is pinned to a full commit SHA and checkout disables persisted credentials. The default branch is protected by the active `Protect main with required preflight` ruleset. It has no bypass actors, `current_user_can_bypass=never`, requires a pull request with resolved review threads, and strictly requires `Required preflight and package`. Zero external approvals are required so the solo autonomous session can merge after CI without weakening the no-direct-push boundary.

All future checkpoints must use a `codex/*` branch and pull request. Codex remains the only GitHub mutation authority; Cursor may extract mechanical Git/GitHub evidence and Claude may synthesize semantic risk, but neither worker may push, open, merge, or alter a ruleset. Verify the live ruleset with `tools/github/Test-GitHubMainProtectionRulesetDrift.ps1`. Apply the canonical `configs/github/main-protection-ruleset.json` only with `tools/github/Set-GitHubMainProtectionRuleset.ps1 -Apply` under explicit Codex mutation authority.
