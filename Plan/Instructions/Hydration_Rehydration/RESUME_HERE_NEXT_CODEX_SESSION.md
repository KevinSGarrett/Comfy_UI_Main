...
First finish the current Wave66 local-smoke checkpoint: rerun Wave65 after these new Plan evidence files, validate JSON/CSV/PowerShell parsing, run `git diff --check`, run a staged forbidden-path and secret scan, commit the local RealVisXL smoke evidence plus required tracker/hydration/Wave65 refresh files, push, and verify `HEAD == origin/main`.
