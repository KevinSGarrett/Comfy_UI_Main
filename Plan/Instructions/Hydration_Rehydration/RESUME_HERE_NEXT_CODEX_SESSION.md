...
First finish the current Wave66 sample-1 checkpoint: rerun Wave65 after these new Plan evidence files, validate JSON/CSV/PowerShell parsing, run `git diff --check`, run a staged forbidden-path and secret scan, commit the sample-1 runtime/QA evidence plus required tracker/hydration/Wave65 refresh files, push, and verify `HEAD == origin/main`. Then rebuild and publish a fresh matrix bundle from the new clean pushed `HEAD` before running samples 2 and 3 one at a time.
