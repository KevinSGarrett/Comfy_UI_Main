...
First finish the current Wave66 stale-bundle static-proof checkpoint: validate JSON/CSV/PowerShell parsing, run `git diff --check`, run a staged forbidden-path and secret scan, commit the failed static-proof evidence plus required tracker/hydration/Wave65 refresh files, push, and verify `HEAD == origin/main`. Then rebuild and publish a fresh matrix bundle from the new clean pushed `HEAD` before retrying static proof.
