...
- `BLOCKER-AWS-AUTH-EXPIRED-001`
  ...
  - latest retest: 2026-07-06T06:19:38-05:00 `Test-ProjectReadinessSnapshot.ps1` now requires the runtime unblock handoff and records `result=pass_local_ready_runtime_blocked_auth`, `local_ready=true`, `ec2_start_allowed=false`, `generation_allowed=false`, and `runtime_unblock_handoff.result=handoff_ready_runtime_blocked_auth`; `Test-QAHelperStatic.ps1` contract-validates `local_only=true`, `aws_contacted=false`, `github_api_contacted=false`, `civitai_contacted=false`, `ec2_started=false`, `generation_executed=false`, `command_step_count=8`, and `markdown_written=true` with 0 contract failures.
