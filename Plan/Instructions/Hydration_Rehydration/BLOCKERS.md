...
- `BLOCKER-AWS-AUTH-EXPIRED-001`
  ...
  - latest retest: 2026-07-06T06:12:37-05:00 `New-RuntimeUnblockHandoff.ps1` records `result=handoff_ready_runtime_blocked_auth`, `failure_category=expired_session`, `next_required_action=complete_aws_browser_sso_login`, `aws_contacted=false`, `ec2_started=false`, and `generation_executed=false`; `Test-OperationsHelperStatic.ps1` validates the handoff smoke with 16 scripts, 9 local smokes, and 0 failures; `Test-ProjectReadinessSnapshot.ps1` still records `result=pass_local_ready_runtime_blocked_auth`, `local_ready=true`, `ec2_start_allowed=false`, and `generation_allowed=false`.
