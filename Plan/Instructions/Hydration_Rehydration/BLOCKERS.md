...
- `BLOCKER-AWS-AUTH-EXPIRED-001`
  ...
  - latest retest: 2026-07-06T05:12:35-05:00 `Test-AwsAuthGate.ps1` records top-level `result=blocked_expired_session` and `failure_category=expired_session`; `Test-AwsProfileAuthMatrix.ps1` checked 15 configured AWS CLI profiles and found zero profiles currently authenticated to expected account `029530099913`; `Test-LaneRuntimeReadiness.ps1` now records top-level `result=local_pre_ec2_ready_runtime_blocked_auth`, `failure_category=expired_session`, `local_pre_ec2_ready=true`, `ready_for_ec2_static_proof=false`, and `ready_for_generation=false`.
