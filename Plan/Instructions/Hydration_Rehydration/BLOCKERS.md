...
- `BLOCKER-AWS-AUTH-EXPIRED-001`
  ...
  - latest retest: 2026-07-06T07:19:43-05:00 `Test-AuthoredLaneEvidenceCoverage.ps1` records `result=pass_local_only`, authored lane count 2, failed lane count 0, `ec2_started=false`, and `generation_executed=false`; `Test-QAHelperStatic.ps1` now includes the authored-lane evidence coverage smoke and reports 8 QA scripts parsed, 11 local smokes passed, 0 smoke failures, and 0 project-readiness contract failures. `Test-ProjectReadinessSnapshot.ps1` still records `result=pass_local_ready_runtime_blocked_auth`, `ec2_start_allowed=false`, and `generation_allowed=false`.
