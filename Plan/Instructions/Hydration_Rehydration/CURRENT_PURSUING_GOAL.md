...
1. Checkpoint the completed sample 1 evidence, rerun Wave65, push, verify clean `HEAD == origin/main`, rebuild/upload a fresh clean-head S3 matrix bundle, then execute RealVisXL matrix samples 2 and 3 one at a time with emergency stop coverage, pullback hash verification, technical QA, and whole-image visual QA.
