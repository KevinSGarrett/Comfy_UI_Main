...
## Next Exact Work
First, commit and push the W68 Canny target-runtime preparation checkpoint after validation and staged-file secret/binary scans. Verify local ComfyUI port 8188 is closed, EC2 `i-0560bf8d143f93bb1` is `stopped`, and local `HEAD == origin/main` after push.

Second, create/verify an emergency stop schedule, then use the S3 install helpers to place the Canny ControlNet model and Canny input image on EC2. The model install should target `/home/ubuntu/ComfyUI/models/controlnet/controlnet-canny-sdxl-1.0-small.safetensors` with SHA256 `fde4888a5f0a5648118991cc50e0ac4d60a2356dbaddf5e0649dd69c1119a2f9`. The input install should target `/home/ubuntu/ComfyUI/input/controlnet_canny_corrected_white_edges_black_bg.png` with SHA256 `1af02b8bd12a9de394fbcc1becd72912f4604f843cb7e7a2fc80496835b8e9a5`. Commit/push those install evidence records before static proof because the EC2 proof helper requires a clean pushed head.

Third, run `Invoke-EC2LaneStaticProof.ps1` for `sdxl_realvisxl_controlnet_canny_lane` from the clean pushed head, rerun lane readiness, then run one bounded EC2 workflow smoke from `runtime_artifacts\run_packages\sdxl_realvisxl_controlnet_canny_lane_static_package_v1\RUN_PACKAGE_MANIFEST.json` only if static proof and readiness pass. Pull back artifacts, run technical image QA and whole-image visual QA, update tracker/evidence/hydration files, and certify only the lane result that actually passed.
