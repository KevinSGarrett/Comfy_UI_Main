...
- FLUX.2 is the next bounded image modernization. Planning/readiness scaffolding exists, but model registration, complete workflows, runtime proof, generated artifacts, and A/B visual QA are missing.
- Resolve exact official Dev/Klein assets and licenses, acquire/register the matching model stack, build text-to-image and reference/edit workflows, wire fail-closed routing, run genuine smoke proofs, and compare against FLUX.1/RealVisXL.
