...
Current Wave 63 terminal-state validation evidence is `Plan/Instructions/QA/Evidence/Operations_Static_Validation/W63_COST_CONTROL_TERMINAL_STATE_VALIDATION_20260706T140909-0500.json` and `Plan/Instructions/QA/Evidence/QA_Helper_Static_Validation/W63_QA_HELPER_TERMINAL_STATE_VALIDATION_20260706T141104-0500.json`. Operations validation reports `pass_local_only`, 20 operations scripts parsed, 14 local smokes, 0 smoke failures, 0 evidence-check failures, and 0 evidence-contract failures. QA validation reports `pass_local_only`, 10 QA scripts parsed, 13 local smokes, and 0 project-readiness contract failures. The runtime handoff smoke now has 16 command steps and requires S3 deploy-bundle, S3 model-install, emergency-stop instructions, and a no-rerun invariant for completed runtime smoke proofs. The dry-run local ComfyUI start path correctly reports local ComfyUI is not found until a real checkout path with `main.py` is supplied.
