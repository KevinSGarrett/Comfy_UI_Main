<#
.SYNOPSIS
Fail-closed promotion validator for Wave70 mask rows.

.DESCRIPTION
Blocks Wave70 mask pass/candidate/accepted statuses unless the row cites a
hard promotion-gate evidence record. This validator does not run ComfyUI,
AWS, GitHub, Civitai, or visual generation. It audits current Tracker/Items
state and makes stale or unsupported pass claims mechanically visible.
#>
param(
  [string]$ProjectRoot = "C:\Comfy_UI_Main",
  [string]$OutJson = "Plan/Instructions/QA/Evidence/Mask_Factory/Wave70/W70_MASK_PROMOTION_HARD_GATE_LATEST.json"
)
...
function Test-GateToken {
  param([string]$Text)
  if ([string]::IsNullOrWhiteSpace($Text)) { return $false }
  return ($Text -match "wave70_mask_promotion_gate_pass|W70_MASK_PROMOTION_HARD_GATE|MASK_PROMOTION_HARD_GATE")
}

function Test-ExplicitGateApprovalToken {
  param([string]$Text)
  if ([string]::IsNullOrWhiteSpace($Text)) { return $false }
  return ($Text -match "W70_MASK_PROMOTION_ROW_GATE_PASS_TRUE")
}

function Test-GeometryGateToken {
  param([string]$Text)
  if ([string]::IsNullOrWhiteSpace($Text)) { return $false }
  return ($Text -match "wave70_mask_geometry_gate_pass|W70_MASK_GEOMETRY_HARD_GATE|MASK_GEOMETRY_HARD_GATE")
}

function Test-ModelBackedGeometryToken {
  param([string]$Text)
  if ([string]::IsNullOrWhiteSpace($Text)) { return $false }
...
function Test-WholeBodyGeometryToken {
  param([string]$Text)
  if ([string]::IsNullOrWhiteSpace($Text)) { return $false }
...
function Get-RowText {
  param([object]$Row)
  $parts = @()
  foreach ($name in @("Acceptance_Evidence", "Evidence_Path", "Evidence_Required", "Validation_Method", "QA_Gates_Required", "Coverage_Audit_Status", "Status_Decision", "Notes")) {
...
if ($trackerRows.Count -ne $expectedWave70RowCount) {
  $failures += "Tracker row count must be $expectedWave70RowCount, found $($trackerRows.Count). Missing rows cannot be hidden behind promotion evidence."
}
...
if ($itemRows.Count -ne $expectedWave70RowCount) {
  $failures += "Item row count must be $expectedWave70RowCount, found $($itemRows.Count). Missing rows cannot be hidden behind promotion evidence."
}
...
foreach ($row in $trackerRows) {
  ...
  $hasGateRequirement = Test-GateToken -Text $rowText
  $hasGeometryRequirement = Test-GeometryGateToken -Text $rowText
  $hasModelBackedGeometryRequirement = Test-ModelBackedGeometryToken -Text $rowText
  $hasWholeBodyGeometryRequirement = Test-WholeBodyGeometryToken -Text $rowText
  $isPassLike = Test-PassLikeStatus -Status $row.Status
  if (!$hasGateRequirement) {
    $failures += "Tracker row $($row.Tracker_ID) lacks wave70_mask_promotion_gate_pass requirement/evidence token."
  }
  ...
  if ($isPassLike -and !$hasGateRequirement) {
    $failures += "Tracker row $($row.Tracker_ID) has pass-like status without hard gate evidence: $($row.Status)"
  }
  if ($isPassLike -and !(Test-ExplicitGateApprovalToken -Text $rowText)) {
    $failures += "Tracker row $($row.Tracker_ID) has pass-like status but no explicit passed hard-gate evidence: $($row.Status)"
  }
  ...
  $checkedRows += [ordered]@{
    ...
    has_gate_token = $hasGateRequirement
    has_geometry_gate_token = $hasGeometryRequirement
    has_model_backed_geometry_token = $hasModelBackedGeometryRequirement
    has_whole_body_geometry_token = $hasWholeBodyGeometryRequirement
...
foreach ($row in $itemRows) {
  ...
  $isPassLike = Test-PassLikeStatus -Status $row.Status
  if (!$hasGateRequirement) {
    $failures += "Item row $($row.Item_ID) lacks wave70_mask_promotion_gate_pass requirement/evidence token."
  }
  if (!$hasGeometryRequirement) {
  ...
  if ($isPassLike -and !$hasGateRequirement) {
    $failures += "Item row $($row.Item_ID) has pass-like status without hard gate evidence: $($row.Status)"
  }
  if ($isPassLike -and !(Test-ExplicitGateApprovalToken -Text $rowText)) {
    $failures += "Item row $($row.Item_ID) has pass-like status but no explicit passed hard-gate evidence: $($row.Status)"
  }
  if ($isPassLike -and !(Test-ExplicitGeometryApprovalToken -Text $rowText)) {
...
$blockedCount = @($checkedRows | Where-Object { $_.status -match "Blocked|Needs_Revision|Fail|Unreviewed" }).Count
$result = if ($failures.Count -eq 0) { "pass_wave70_mask_promotion_hard_gate" } else { "fail_wave70_mask_promotion_hard_gate" }
...
$evidence = [ordered]@{
  schema_version = "1.0"
  evidence_id = "W70-MASK-PROMOTION-HARD-GATE"
  timestamp = (Get-Date).ToString("yyyy-MM-ddTHH:mm:sszzz")
  project_root = $ProjectRoot
  qa_type = "wave70_mask_promotion_hard_gate_validator"
  validator = Convert-ToProjectPath -Path $PSCommandPath
  result = $result
  wave70_mask_promotion_gate_pass = ($failures.Count -eq 0)
  pass_like_row_count = $passLikeCount
  blocked_or_untrusted_row_count = $blockedCount
  checked_row_count = $checkedRows.Count
  failures = $failures
  warnings = $warnings
  checked_rows = $checkedRows
  rule = "No Wave70 mask pass/candidate/complete status is allowed without explicit hard-gate, geometry-gate, model-backed geometry authority, and whole-body geometry authority evidence."
}

Write-JsonNoBom -Value $evidence -Path $outResolved -Depth 90

if ($failures.Count -gt 0) {
  Write-Error "Wave70 mask promotion hard gate failed with $($failures.Count) failure(s). Evidence: $outResolved"
}

Write-Output "Wave70 mask promotion hard gate passed. Evidence: $outResolved"
