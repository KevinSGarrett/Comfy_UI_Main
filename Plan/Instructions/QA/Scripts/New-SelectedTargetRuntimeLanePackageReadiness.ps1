...
$checks = @(
  ...
  (New-Check -Name "deploy_bundle_records_dirty_git_as_blocker" -Passed (-not [bool]$deployBundle.source_git_clean -and [int]$deployBundle.source_git_status_count -gt 0) -Observed ([ordered]@{ source_git_clean = $deployBundle.source_git_clean; source_git_status_count = $deployBundle.source_git_status_count }) -Expected "dirty Git is visible and does not permit EC2 execution"),
