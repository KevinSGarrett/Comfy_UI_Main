...
$checks = @(
  (New-Check -Name "target_plan_selects_lane_and_blocks_execution" -Passed ([string]$targetPlan.result -in @("blocked_target_runtime_execution_plan_waiting_for_explicit_selection", "blocked_target_runtime_execution_plan_waiting_for_explicit_selection_and_clean_git") -and $laneId -eq "sdxl_realvisxl_inpaint_detail_lane" -and -not [bool]$targetPlan.execute_allowed_now) -Observed ([ordered]@{ result = $targetPlan.result; lane_id = $laneId; execute_allowed_now = $targetPlan.execute_allowed_now }) -Expected "blocked plan for sdxl_realvisxl_inpaint_detail_lane with execute_allowed_now=false"),
