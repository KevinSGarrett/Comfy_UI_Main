...
SCRIPT = Path(__file__).parents[3] / "07_IMPLEMENTATION" / "scripts" / "finalize_wave64_speech_rows128_130.py"
SPEC = importlib.util.spec_from_file_location("speech_acoustics_finalizer", SCRIPT)
MODULE = importlib.util.module_from_spec(SPEC)
assert SPEC and SPEC.loader
SPEC.loader.exec_module(MODULE)


class SpeechAcousticsFinalizerTests(unittest.TestCase):
    ...
    def test_csv_update_is_idempotent_and_preserves_other_rows(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            rows = [{"Item_ID": f"ITEM-W64-{number}", "Status": "Planned", "Coverage_Audit_Status": "planned", "Notes": "old"} for number in (128, 129, 130)]
