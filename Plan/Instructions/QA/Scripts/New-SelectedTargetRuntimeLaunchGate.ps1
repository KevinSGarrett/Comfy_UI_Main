...
$checks = @(
  (New-Check -Name "target_plan_still_selects_inpaint_lane" -Passed ([string]$targetPlan.result -in @("blocked_target_runtime_execution_plan_waiting_for_explicit_selection", "blocked_target_runtime_execution_plan_waiting_for_explicit_selection_and_clean_git") -and $laneId -eq "sdxl_realvisxl_inpaint_detail_lane" -and $workOrderId -eq "WO-W66-SDXL_REALVISXL_INPAINT_DETAIL_LANE-TARGET-RUNTIME-PROOF" -and -not [bool]$targetPlan.execute_allowed_now) -Observed ([ordered]@{ result = $targetPlan.result; lane_id = $laneId; work_order_id = $workOrderId; execute_allowed_now = $targetPlan.execute_allowed_now }) -Expected "selected inpaint target-runtime work order with execute_allowed_now=false"),
  ...
  (New-Check -Name "git_checkpoint_state_is_accounted" -Passed ($gitGatePasses -or [string]$gitGate.result -ne "pass_git_checkpoint_ready") -Observed ([ordered]@{ result = $gitGate.result; clean_worktree = $gitGate.clean_worktree; local_matches_origin = $gitGate.local_matches_origin; passes_for_ec2_execute = $gitGatePasses }) -Expected "Git checkpoint either passes clean/synced local readiness or remains represented as a launch blocker"),
...
$record = [ordered]@{
  ...
  next_action = $(if ($gitGatePasses) { "Keep EC2 stopped. Git is clean/synced for the local gate; future live execution still requires explicit live intent, S3 Execute proofs for deploy bundle/input/model assets, EC2 install hash proof, EC2 start authorization, and selected target-runtime gates." } else { "Keep EC2 stopped. If the user explicitly selects this target-runtime task later, resolve the Git checkpoint, rebuild/revalidate the deploy bundle from a clean checkpoint, publish through the approved S3 path, and then run EC2 static proof before any workflow smoke." })
}
