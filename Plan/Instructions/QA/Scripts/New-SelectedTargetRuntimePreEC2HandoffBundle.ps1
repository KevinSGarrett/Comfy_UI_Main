...
$gitCheckpointPassesForEc2 = (
  ([bool]$targetPlan.git_checkpoint_summary.passes_for_ec2_execute) -or
  ([bool]$readiness.git_checkpoint_passes_for_ec2) -or
  ([bool]$launchGate.git_checkpoint_passes_for_ec2)
)
if ($gitCheckpointPassesForEc2) {
  $supersededGitBlockers = @(
    "git_checkpoint_gate_not_clean_for_ec2_execute",
    "runtime_handoff_git_gate_not_passing"
  )
  $exactBlockers = @($exactBlockers | Where-Object { $supersededGitBlockers -notcontains [string]$_ })
}
if ($s3PublishDryRunReady) {
...
if (-not $gitCheckpointPassesForEc2) {
  $requiredBlockers += "git_checkpoint_gate_not_clean_for_ec2_execute"
}
...
$checks = @(
  ...
  (New-Check -Name "required_blockers_are_preserved" -Passed ($missingRequiredBlockers.Count -eq 0) -Observed ([ordered]@{ required_blockers = @($requiredBlockers); git_checkpoint_passes_for_ec2 = $gitCheckpointPassesForEc2; missing_required_blockers = @($missingRequiredBlockers); exact_blockers = @($exactBlockers) }) -Expected "explicit selection is preserved; Git blockers are preserved only when the Git checkpoint gate is not passing")
)
