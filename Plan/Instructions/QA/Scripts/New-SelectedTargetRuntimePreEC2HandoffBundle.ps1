...
$exactBlockers = @($launchBlockers + $targetBlockers + $readinessBlockers + $matrixBlockers |
...
$record = [ordered]@{
  ...
  next_action = "Keep EC2 stopped. Use this bundle as the selected inpaint target-runtime handoff anchor only after explicit live-window selection; then rerun the allowed local rechecks, require a clean Git checkpoint, rebuild or revalidate the deploy bundle from that clean checkpoint, and proceed to S3/EC2 only after all gates pass."
}
