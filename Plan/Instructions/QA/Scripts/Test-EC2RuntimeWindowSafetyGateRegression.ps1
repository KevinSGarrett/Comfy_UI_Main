...
try {
  ...
  $tests = @(
    ...
    (New-Test "smoke_deploy_explicitly_copies_manifest_inputs" ($smokeExecutorText -match 'required_assets = manifest\.get\("required_input_assets"\)' -and $smokeExecutorText -match 'extracted_asset = os\.path\.realpath' -and $smokeExecutorText -match 'if not expected_asset_sha or extracted_sha != expected_asset_sha:' -and $smokeExecutorText -match 'shutil\.copy2\(extracted_asset, deployed_asset\)' -and $smokeExecutorText -match 'deployed_sha != expected_asset_sha') "manifest-declared extraction copy with containment and mandatory SHA proof" "required inputs are explicitly copied from the live extraction root before cleanup and missing hashes fail closed"),
