...
try {
  ...
  $tests = @(
    ...
    (New-Test "smoke_deploy_streams_exact_manifest_inputs" ($smokeExecutorText -match 'required_assets = manifest\.get\("required_input_assets"\)' -and $smokeExecutorText -match 'matching_members = \[info for info in archive_files if info\.filename == rel\]' -and $smokeExecutorText -match 'if len\(matching_members\) != 1:' -and $smokeExecutorText -match 'with asset_zip\.open\(matching_members\[0\], "r"\)' -and $smokeExecutorText -match 'shutil\.copyfileobj\(source_stream, destination_stream\)' -and $smokeExecutorText -match 'if not expected_asset_sha:' -and $smokeExecutorText -match 'deployed_sha != expected_asset_sha') "exact archive-member stream with containment and mandatory SHA proof" "required inputs are streamed directly from the verified ZIP and post-copy hashes fail closed"),
