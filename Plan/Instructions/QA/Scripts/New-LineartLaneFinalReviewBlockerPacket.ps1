<#
.SYNOPSIS
Creates a lane-scoped final-review blocker packet for the ControlNet Lineart lane.

.DESCRIPTION
Reviews existing local Lineart v4 and local robustness evidence for
sdxl_realvisxl_controlnet_lineart_lane. The helper records why the lane cannot
be closed as a final-review packet yet: local model/input proof, local
generation smoke, preferred v4 QA, and three-sample robustness exist, but the
lane's own promotion rule requires target-runtime object_info/path/hash/input
proof, bounded target-runtime output, pullback, technical QA, strict
whole-image QA, and final certification review before closure.
#>
param(
  [string]$ProjectRoot = "C:\Comfy_UI_Main",
  [string]$LaneId = "sdxl_realvisxl_controlnet_lineart_lane",
...
if ([string]::IsNullOrWhiteSpace($OutFile)) {
  ...
  $OutFile = "Plan\Instructions\QA\Evidence\Done_Certifications\W66_LINEART_LANE_FINAL_REVIEW_BLOCKER_PACKET_$stamp.json"
}
...
$modelProvisioningPath = Resolve-ProjectPath -Path "Plan/Instructions/QA/Evidence/Model_Registry/W69_LOCAL_CONTROLNET_LINEART_MODEL_PROVISIONING_20260707T060000-0500.json"
$runtimeExecutePath = Resolve-ProjectPath -Path "Plan/Instructions/QA/Evidence/Workflow_Runtime/W69_LOCAL_LINEART_V4_PLAIN_BACKDROP_SEED711370003_EXECUTE_20260707T093000-0500.json"
$visualQaPath = Resolve-ProjectPath -Path "Plan/Instructions/QA/Evidence/Image_Artifact_QA/W69_LOCAL_LINEART_V4_PLAIN_BACKDROP_VISUAL_QA_20260707T093300-0500.json"
$robustnessQaPath = Resolve-ProjectPath -Path "Plan/Instructions/QA/Evidence/Image_Artifact_QA/W69_LOCAL_LINEART_V4_MULTISEED_ROBUSTNESS_VISUAL_QA_20260707T094200-0500.json"
$trackerFollowupPath = Resolve-ProjectPath -Path "Plan/Tracker/Evidence/W69_LOCAL_LINEART_V4_PLAIN_BACKDROP_FOLLOWUP_20260707T093300-0500.json"
$trackerRobustnessPath = Resolve-ProjectPath -Path "Plan/Tracker/Evidence/W69_LOCAL_LINEART_V4_MULTISEED_ROBUSTNESS_20260707T094200-0500.json"
...
$checks = @(
  ...
  (New-Check -Name "queue_rule_requires_target_runtime_before_promotion" -Passed ($queueLane.Count -gt 0 -and $queuePromotionRule -match "Do not promote to target-runtime proven or final Lineart certification" -and $queuePromotionRule -match "Target-runtime object_info/path/hash/input proof" -and $queuePromotionRule -match "strict whole-image visual QA") -Observed ([ordered]@{ status = $(if ($queueLane.Count -gt 0) { [string]$queueLane[0].status } else { "missing" }); promotion_rule = $queuePromotionRule }) -Expected "runtime queue explicitly requires target-runtime proof before final Lineart certification"),
  (New-Check -Name "local_lineart_model_and_input_hash_verified" -Passed ([string]$modelProvisioning.result -eq "pass_local_lineart_controlnet_model_and_input_hash_verified" -and [bool]$modelProvisioning.local_only -and -not [bool]$modelProvisioning.ec2_started) -Observed ([ordered]@{ result = $modelProvisioning.result; local_only = $modelProvisioning.local_only; ec2_started = $modelProvisioning.ec2_started; model_sha256 = $modelProvisioning.model.sha256; input_sha256 = $modelProvisioning.paired_control_image.sha256 }) -Expected "local Lineart ControlNet model and paired control image hash proof exist"),
  (New-Check -Name "local_lineart_v4_runtime_smoke_passed" -Passed ([string]$runtimeExecute.result -eq "pass_local_run_package_generation_smoke" -and [bool]$runtimeExecute.local_only -and -not [bool]$runtimeExecute.ec2_started -and [bool]$runtimeExecute.generation_executed -and @($runtimeExecute.errors).Count -eq 0) -Observed ([ordered]@{ result = $runtimeExecute.result; local_only = $runtimeExecute.local_only; ec2_started = $runtimeExecute.ec2_started; generation_executed = $runtimeExecute.generation_executed; errors = @($runtimeExecute.errors).Count; run_package_lane = $runtimeExecute.run_package.lane_id }) -Expected "local Lineart v4 run package generation smoke passed"),
  (New-Check -Name "lineart_v4_visual_qa_passes_but_disallows_certification" -Passed ([string]$visualQa.qa_result -eq "pass_with_notes_promote_v4_as_preferred_local_lineart_background_candidate" -and [bool]$visualQa.local_only -and -not [bool]$visualQa.ec2_started -and -not [bool]$visualQa.certification_allowed) -Observed ([ordered]@{ qa_result = $visualQa.qa_result; local_only = $visualQa.local_only; ec2_started = $visualQa.ec2_started; certification_allowed = $visualQa.certification_allowed; blocking_defects = $visualBlockingDefects.Count; remaining_risks = $visualRemainingRisks }) -Expected "Lineart v4 local visual QA passes with notes but explicitly disallows certification"),
  (New-Check -Name "lineart_v4_multiseed_robustness_passes_but_disallows_certification" -Passed ([string]$robustnessQa.qa_result -eq "pass_with_notes_local_lineart_v4_plain_backdrop_multiseed_robustness" -and [bool]$robustnessQa.local_only -and -not [bool]$robustnessQa.ec2_started -and -not [bool]$robustnessQa.certification_allowed -and $robustnessBlockingDefects.Count -eq 0) -Observed ([ordered]@{ qa_result = $robustnessQa.qa_result; local_only = $robustnessQa.local_only; ec2_started = $robustnessQa.ec2_started; certification_allowed = $robustnessQa.certification_allowed; blocking_defects = $robustnessBlockingDefects.Count; remaining_risks = $robustnessRemainingRisks }) -Expected "Lineart v4 local robustness passes with notes but explicitly disallows certification"),
  (New-Check -Name "tracker_records_local_lineart_iterations" -Passed ([string]$trackerFollowup.lane_id -eq $LaneId -and [string]$trackerRobustness.lane_id -eq $LaneId -and [string]$trackerFollowup.result -eq "pass_with_notes_promote_v4_as_preferred_local_lineart_background_candidate" -and [string]$trackerRobustness.result -eq "pass_with_notes_local_lineart_v4_plain_backdrop_multiseed_robustness" -and [string]$trackerRobustness.remaining_boundary -match "not final Lineart lane certification") -Observed ([ordered]@{ followup_lane = $trackerFollowup.lane_id; followup_result = $trackerFollowup.result; robustness_lane = $trackerRobustness.lane_id; robustness_result = $trackerRobustness.result; robustness_boundary = $trackerRobustness.remaining_boundary }) -Expected "tracker mirrors classify Lineart evidence as local pass-with-notes iteration")
)
...
$knownIssues = @(
  "W69 Lineart model provisioning proves local model and paired control image hash only.",
  "W69 Lineart v4 plain-backdrop visual QA passes with notes, but certification_allowed is false.",
  "W69 Lineart v4 three-sample robustness passes locally with notes, but exact identity, hands, full-body anatomy, contact points, and broader scene backgrounds remain uncertified.",
  "The Lineart lane promotion rule requires target-runtime object_info/path/hash/input proof, bounded target-runtime generation, pullback, technical QA, strict whole-image visual QA, and final certification review before promotion.",
  "This blocker packet does not consume candidate masks as truth, promote masks, rerun Wave70 hard gates, or activate Wave71+."
)

$result = if ($reviewDefects.Count -eq 0) { "blocked_lineart_lane_final_review_target_runtime_proof_missing" } else { "fail_lineart_lane_final_review_blocker_packet" }
$finalDecision = "blocked"

$record = [ordered]@{
  schema_version = "1.0"
  artifact_type = "lane_final_review_blocker_packet"
  blocker_id = "BLOCK-W66-SDXL-LINEART-LANE-FINAL-REVIEW-$(Get-Date -Format 'yyyyMMddTHHmmss-0500')"
  created_at = (Get-Date -Format "yyyy-MM-ddTHH:mm:sszzz")
  task_tracker_id = "WO-W66-SDXL_REALVISXL_CONTROLNET_LINEART_LANE-FINAL-CERTIFICATION-REVIEW"
  lane_id = $LaneId
  result = $result
  ...
  artifact_scope = "Lane-scoped final-review blocker packet for sdxl_realvisxl_controlnet_lineart_lane."
  implementation_summary = "Reviewed local Lineart model/input proof, local v4 run-package generation smoke, preferred v4 visual QA, three-sample robustness QA, tracker mirrors, target-runtime plan, and runtime queue promotion rule without rerunning live runtime."
  ...
  certification_boundary = "Lane-scoped blocker review only. This does not certify full project completion, final Lineart lane quality, target-runtime readiness, body-mask readiness, Wave70 mask promotion, or Wave71+ activation."
  next_action = "Do not close the Lineart lane final-review work order from current local evidence. Target-runtime proof must be explicitly selected and pass clean Git, clean deploy-bundle, object_info/path/hash/input, bounded output, pullback, technical QA, strict visual QA, and final certification gates first."
}
