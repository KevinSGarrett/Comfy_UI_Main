...
- created_at: 2026-07-06T14:08:06-05:00
- result: handoff_runtime_smoke_qa_complete
- failure_category:
- next_required_action: checkpoint_runtime_smoke_evidence_and_advance_next_goal
...
- Auth gate: pass, safe_to_start_ec2=True, account_match=True, failure_category=
- Profile matrix: pass_profile_available, matching profiles=2, expected account=029530099913
- Lane readiness: ready_for_generation, lane_id=sdxl_realvisxl_base_lane, lane_match=True, local_pre_ec2_ready=True, ready_for_ec2_static_proof=True, ready_for_generation=True
- Project readiness: pass_runtime_smoke_qa_complete, lane_id=sdxl_realvisxl_base_lane, lane_match=True, local_ready=True, ec2_start_allowed=False, generation_allowed=False, scan_hit_count=0
- Runtime lane queue: pass_local_only, first_runtime_lane_id=sdxl_low_risk_fallback_lane, current_runtime_lane_id=sdxl_realvisxl_base_lane, current_runtime_lane_match=True, selected_lane_order=2, queue_allows_selected_lane_ec2_static_proof=True
...
- Workflow smoke: workflow_smoke_generation_complete, run_id=aws_gpu_workflow_smoke_20260706T132206-0500, complete=True, final_state=stopped, local_pullback_status=s3_bucket_not_configured
- Pullback record: pullback_hashes_verified, hashes_verified=True, complete=True, file_count_remote=4, file_count_local=4
- Image QA technical: pending_visual_review, technical_integrity=pass, resolution_check=pass, complete=True
- Image QA visual: pass_with_notes_for_runtime_smoke, qa_score=88, pass_threshold=80, complete=True
...
- Do not start EC2 unless runtime lane queue validation reports `current_runtime_lane_id=sdxl_realvisxl_base_lane`, selected lane in queue, and failed check count `0`.
- Do not start EC2 unless model registry coverage reports `result=pass_local_only`, selected lane `sdxl_realvisxl_base_lane` result `pass`, and failed check count `0`.
- Do not start EC2 unless local Git is clean and `HEAD` equals `origin/main`.
- Do not run EC2 static proof unless lane readiness reports `lane_id=sdxl_realvisxl_base_lane` and `ready_for_ec2_static_proof=true`.
...
Expected evidence: result=pass_local_only, current_runtime_lane_id=sdxl_realvisxl_base_lane, selected lane in queue, failed_check_count=0.
...
Expected evidence: result=pass_local_only, selected lane sdxl_realvisxl_base_lane has result=pass, failed_check_count=0, registry records and runtime-validation queue rows exist.
...
```powershell
powershell -ExecutionPolicy Bypass -File C:\Comfy_UI_Main\tools\Test-LocalComfyUIDevPreflight.ps1 -ProjectRoot C:\Comfy_UI_Main -LaneId sdxl_realvisxl_base_lane -LocalComfyRoot <path-to-local-ComfyUI>
```
...
```powershell
powershell -ExecutionPolicy Bypass -File C:\Comfy_UI_Main\tools\New-EC2DeployBundle.ps1 -ProjectRoot C:\Comfy_UI_Main -LaneId sdxl_realvisxl_base_lane -RunPackageManifestFile <run-package-manifest>
```
...
```powershell
powershell -ExecutionPolicy Bypass -File C:\Comfy_UI_Main\Plan\Instructions\Operations\Scripts\Test-LaneRuntimeReadiness.ps1 -LaneId sdxl_realvisxl_base_lane -OutFile C:\Comfy_UI_Main\Plan\Instructions\QA\Evidence\Runtime_Readiness\W61_LANE_RUNTIME_READINESS_<timestamp>.json
```

Expected evidence: local_pre_ec2_ready=true, lane_id=sdxl_realvisxl_base_lane, and ready_for_ec2_static_proof=true before EC2 static proof.
