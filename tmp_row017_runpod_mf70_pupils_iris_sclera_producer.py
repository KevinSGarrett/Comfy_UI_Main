#!/usr/bin/env python3
"""Row017 Class E future-producer: RunPod mf70_pupils_iris_sclera -> GLOBAL_REVIEW (no COMPLETE/CSV)."""
...
REGION = "mf70_pupils_iris_sclera"
CLIENT_ID = f"cursor-row017-mf70-pupils-runpod-{uuid.uuid4().hex[:8]}"
SEED = 210811
DENOISE = 0.035
CFG = 3.4
STEPS = 18
...
RUNTIME_DIR = ROOT / "runtime_artifacts" / f"wave64_row017_runpod_mf70_pupils_iris_sclera_{STAMP}"
OUT_DIR = (
    ROOT
    / "Plan/Instructions/Operations/Pulled_Back_Artifacts"
    / f"runpod_comfyui_row017_mf70_pupils_iris_sclera_{STAMP}"
)
IMG_DIR = OUT_DIR / "images"
PREFIX = f"codex_row017_mf70_pupils_iris_sclera_{STAMP}"
SRC_UPLOAD = f"wave64_row017_mf70_pupils_src_{STAMP}.png"
MASK_UPLOAD = f"wave64_row017_mf70_pupils_mask_{STAMP}.png"

POS = (
    "extremely subtle photoreal pupil iris sclera preservation pass, preserve exact person identity, gaze direction, eye symmetry, iris color, pupil size, sclera tone, catchlights, eyelids, eyelashes, eyebrow position, nearby hair occlusion, surrounding skin texture, face lighting, white blazer, background, and portrait composition unchanged"
)
NEG = (
    "identity change, different person, gaze change, mismatched eyes, crossed eyes, asymmetrical pupils, enlarged pupils, blown catchlights, missing catchlights, iris color change, glassy eyes, doll eyes, anime eyes, red eyes, sclera discoloration, eyelid deformation, eyelash mutation, eyebrow change, hair mutation, visible seam, skin tone change, clothing change, background change, watermark, text"
)
...
def package_evidence(receipt: dict, out_path: Path, metrics: dict) -> dict:
    ...
    qa_path = (
        ...
        / f"ROW017_RUNPOD_MF70_PUPILS_IRIS_SCLERA_VISUAL_QA_{STAMP}.json"
    )
    global_path = (
        ROOT
        / "Plan/Instructions/QA/Evidence/Image_Artifact_QA/Row017_Canonical_Global_Reviews"
        / f"ROW017_RUNPOD_MF70_PUPILS_IRIS_SCLERA_{STAMP}_GLOBAL_REVIEW.json"
    )
    exec_path = (
        ROOT
        / "Plan/Instructions/QA/Evidence/Workflow_Runtime"
        / f"W64_RUNPOD_COMFYUI_ROW017_MF70_PUPILS_IRIS_SCLERA_EXECUTE_{STAMP}.json"
    )
    climb_qa = (
        ROOT
    ...
    qa = {
        "schema_version": "1.0",
        "evidence_id": f"ROW017-RUNPOD-MF70-PUPILS-IRIS-SCLERA-VISUAL-QA-{STAMP}",
        "created_at": CREATED,
        ...
        "qa_decision": "pass_bounded_localized_mf70_pupils_iris_sclera_runpod_runtime_candidate",
        ...
        "whole_frame_findings": [
            (
                f"Fresh RunPod ComfyUI mf70 eyebrows masked inpaint on "
            ...
            "Whole-frame identity, hair, navy blazer, patterned shirt, collar, dark studio background, lighting, and framing remain stable versus source.",
            (
                f"Pixel delta is localized to mf70 eyebrows bbox {metrics['face_mask_bbox']}; "
                ...
                f"vs {metrics['mask_pixel_count']}-pixel eyebrows mask."
            ),
            "Direct whole-frame + face/eye/mouth crop inspection: subtle eyebrow hair-stroke preservation refinement without gray-mask failure, identity drift, wardrobe rewrite, background change, hard mask-edge seam, or gaze/iris rewrite.",
    ...
    global_review = {
        ...
        "required_target_region_check": {
            ...
            "target_region": REGION,
            "findings": [
                "Fresh RunPod mf70 eyebrows climb remains a bounded eyebrow hair-stroke preservation refinement with no gray-mask, face-replacement, gaze drift, or hard-edge failure."
            ],
        },
        "required_non_target_region_scan": {
            ...
            "findings": [
                "Identity, eyes/lashes/lids outside the brow mask, mouth, hair, wardrobe, background, lighting, and framing remain stable; brow refinement stays inside the eyebrows mask."
            ],
        },
        "hands_face_body_background_contact_lighting_check": {
            ...
            "face": {
                ...
                "inspected": True,
                "status": "pass",
                "reason": "Face identity remains stable; only the eyebrow mask is refined; eyes/lashes/lids and mouth must hold.",
            },
            "body": {
                "visibility": "visible",
                "inspected": True,
    ...
    execute = {
        "schema_version": "1.0",
        "evidence_id": f"W64-RUNPOD-COMFYUI-ROW017-MF70-PUPILS-IRIS-SCLERA-EXECUTE-{STAMP}",
        "created_at": CREATED,
        "tracker_id": "TRK-W64-017",
        "item_id": "ITEM-W64-017",
        "mode": "execute",
    ...
    climb = {
        ...
        "highest_proof_tier_achieved": "VISUAL_QA_PASS_BOUNDED",
        "decision": (
            "Advanced Row017 Class E with a real RunPod ComfyUI localized mf70_pupils_iris_sclera producer "
        ...
        "remaining_blocker": (
            ...
            "post-clearance RunPod mf70_pupils_iris_sclera emission, but product completion still requires broader campaign "
    ...
    packet = {
        ...
        "tracker_id": "TRK-W64-017",
        "item_id": "ITEM-W64-017",
        "mutation_this_landing": "runpod_mf70_pupils_iris_sclera_producer_global_review_emission",
        "row_complete": False,
        "product_completion_claimed": False,
...
def main() -> int:
    ...
    shutil.copy2(MASK, OUT_DIR / "mask_eyebrows.png")
