#!/usr/bin/env python3
"""LOCAL PRIMARY CSV mutator: sync TRK/ITEM-W64-076 Notes for full-library reconcile start.
...
def assert_pid_alive(pid: int) -> None:
    # Windows: tasklist /FI; fail closed if not found
...
def main() -> None:
    ...
    stamp = {
        ...
        "csv_stamp": "synced_by_primary_csv_mutator_row076_reconcile_start_sync",
        "csv_sync_tip": tip,
        "evidence_id": f"TRK-W64-076-REVERB-RECONCILE-START-{stamp_local}",
        "explicit_non_claims": [
            "COMPLETE",
            "product_completion",
            "library_authority",
        ...
        "process": {
            "action": "started_full_library_index_retained_resume_after_row074_hold",
            "alive": True,
            "command": RESUME_CMD,
    ...
    evidence_path = (
        f"{ANALYSIS}; {SUMMARY}; {rel_stamp}; {POST073_RANK}; {DELTA_REL}; {ROW074_HOLD}"
    )
    notes = (
        f"In progress: full-library index-retained reverb/dryness reconcile PID {pid} healthy "
        f"({tip} stamp); resumed from limit-25 probe after Row074 coverage_complete HOLD "
        ...
        + "|".join(BLOCKERS)
        + f". Evidence: {rel_stamp}; {POST073_RANK}; FACE_04 landed eb018424 (disjoint)."
    )
    ...
    delta["csv_sync"] = "synced_by_primary_csv_mutator_row076_reconcile_start_sync"
    delta["csv_sync_tip"] = tip
    delta["ledger_vocabulary_sync"] = {
        "ledger_status": STATUS,
        "note": (
            f"Reconcile start sync {processed}/{total}; PID {pid} left alone; "
