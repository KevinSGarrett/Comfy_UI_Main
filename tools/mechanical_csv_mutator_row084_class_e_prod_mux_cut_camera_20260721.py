#!/usr/bin/env python3
"""LOCAL PRIMARY CSV mutator: Row084 Class E prod-media cut/camera/mux probe. Row074 untouched."""
...
ROW084_PACKET = (
    f"{EVID}/TRK-W64-084_ROW084-011_CLASS_E_RUNPOD_PROD_MEDIA_CUT_CAMERA_MUX_PROBE_20260721.json"
)
...
ROW084_RECEIPT = (
    f"{EVID}/fixtures/row084/runtime/runpod_class_e_prod_mux_cut_camera_20260721T101712Z/"
    "class_e_prod_media_cut_camera_mux_receipt.json"
)
...
def main() -> None:
    ...
    assert receipt["cut_camera_probe"]["hard_cut_count"] == 0
    ...
    row084_decision = "row084_class_e_prod_media_cut_camera_mux_probed_no_complete"
    row084_notes = (
        f"Class E prod-media cut/camera/mux deepen: RunPod genuine AV-sync MKV "
        f"sha={MEDIA_SHA[:12]} ({PROOF}); frames=49 hard_cuts=0 observed_class=null "
        f"(unclassified under held-out thresholds); mux identity V/A present but "
        f"production_mux_replay_pass=false; compiler hard-fail retained. "
        f"Prior Comfy gen prompt_id={PRIOR_PROMPT} retained NON-AUTHORITY (not COMPLETE). "
        "ROW084-011 Class E FAIL/OPEN retained; ROW084-012 Class C OPEN_HOLD unchanged "
        "(0e0c3d86); row_complete=false; NEVER Complete; Row074 left alone; "
        f"RunPod ONLY (no EC2); no new :8188 gen. Evidence: {ROW084_PACKET}; "
        f"{ROW084_RECEIPT}; {ROW084_DELTA}; {ROW084_HOLD_012}"
    )
    row084_evidence = (
        f"{ROW084_ARTIFACT}; {ROW084_DELTA}; {ROW084_PACKET}; "
        f"{ROW084_RECEIPT}; {ROW084_HOLD_012}"
    )
    ...
    delta["ledger_vocabulary_sync"] = {
        ...
        "note": (
            f"Mechanical CSV mutator Row084 Class E prod-media cut/camera/mux from tip {tip}; "
            "no COMPLETE."
        ),
    ...
    delta["csv_sync"] = "synced_by_primary_csv_mutator_row084_class_e_prod_mux_cut_camera"
    delta["csv_sync_tip"] = tip
    dump_json(ROW084_DELTA, delta)

    packet["csv_sync"] = "synced_by_primary_csv_mutator_row084_class_e_prod_mux_cut_camera"
    packet["csv_sync_tip"] = tip
    dump_json(ROW084_PACKET, packet)

    print("tip", tip)
    print("synced TRK/ITEM-W64-084 Class E prod-media cut/camera/mux", MEDIA_SHA[:12])
    print("ROW084-011 FAIL/OPEN retained; ROW084-012 OPEN_HOLD; row_complete=false")
    print("Row074 left alone; EC2 unused; no COMPLETE from gen receipt")
