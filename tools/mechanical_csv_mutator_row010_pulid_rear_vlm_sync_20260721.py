#!/usr/bin/env python3
"""LOCAL PRIMARY CSV mutator: sync TRK/ITEM-W64-010 Notes for PuLID REAR VLM.
...
ROW010_SIDE_VLM = (
    f"{EVID}/TRK-W64-010_RUNPOD_C1_PULID_SIDE_VLM_20260721T151831Z.json"
)
OLLAMA_VLM_SMOKE = (
...
SYNC_MARKER = "synced_by_primary_csv_mutator_row010_pulid_rear_vlm_sync_20260721"
...
def latest_rear_pair() -> tuple[Path, Path]:
    calibs = sorted(EVID.glob("TRK-W64-010_RUNPOD_C1_PULID_REAR_CALIB_*.json"))
    vlms = sorted(EVID.glob("TRK-W64-010_RUNPOD_C1_PULID_REAR_VLM_*.json"))
    if not calibs or not vlms:
        raise RuntimeError("missing landed REAR calib/vlm evidence under QA Evidence")
...
def main() -> None:
    tip = git_short()
    calib_path, vlm_path = latest_rear_pair()
    ...
    if already_synced(notes, vlm_stamp):
        print("tip", tip)
        print("no-op: TRK/ITEM-W64-010 PuLID REAR VLM Notes already synced")
    ...
    row010_notes = (
        ...
        "prior PuLID SIDE lock-trait FAIL retained (151831Z solo_lock 0.0); "
        "RunPod PuLID REAR (FACE_04 PuLID + C1_REAR_AUTHORITY_PRIMARY / "
        f"C1_ASS_AUTHORITY_FULLBODY_PRIMARY composition + latina 0.78/0.58 weight 1.12) "
        f"VLM {outcome} ({tip}/{vlm_stamp} prompt_id={prompt_id}): "
        ...
        "LOCKFRONT FACE_01, BODYFORWARD FRONT, and SIDE envelopes; no invented faces; "
        ...
        f"{ROW010_FACE04_VLM}; {ROW010_BF_VLM}; {ROW010_SIDE_VLM}; "
        f"{row010_calib_rel}; {row010_vlm_rel}"
    )
    ...
    row010_evidence = (
        ...
        f"{ROW010_FACE04_VLM}; {ROW010_BF_VLM}; {ROW010_SIDE_VLM}; "
        f"{row010_calib_rel}; {row010_vlm_rel}; {OLLAMA_VLM_SMOKE}"
    )
    ...
    print("tip", tip)
    print(
        f"synced TRK/ITEM-W64-010 PuLID REAR VLM {outcome}; "
