#!/usr/bin/env python3
"""LOCAL PRIMARY CSV mutator: Row084 Class E prod sparse_motion reclass. Row074 untouched."""
...
ROW084_PACKET = (
    f"{EVID}/TRK-W64-084_ROW084-011_CLASS_E_PROD_SPARSE_MOTION_RECLASS_20260721.json"
)
...
ROW084_RECEIPT = (
    f"{EVID}/fixtures/row084/runtime/local_class_e_prod_sparse_motion_reclass_20260721T103500Z/"
    "class_e_prod_sparse_motion_reclass_receipt.json"
)
ROW084_HARD_FAIL = (
...
def main() -> None:
    ...
    assert receipt["cut_camera_probe"]["observed_class"] == "sparse_motion"
    ...
    row084_decision = "row084_class_e_prod_sparse_motion_reclass_no_complete"
    row084_notes = (
        f"Class E prod sparse_motion reclass: genuine AV-sync sha={MEDIA_SHA[:12]} "
        f"({PROOF}); observed_class=sparse_motion under production profile "
        "(mean=0.0297 max=0.3770 moderate=14/24 mod_frac=0.2917); held-out still null "
        "(frac>=0.5 retained; STATIC_MAX_MAX not inflated); production_mux_replay_pass="
        "true retained; production_cut_camera_authority_granted=false; compiler hard-fail "
        "retained. Prior Comfy gen prompt_id="
        f"{PRIOR_PROMPT} NON-AUTHORITY. ROW084-011 Class E FAIL/OPEN; ROW084-012 "
        "OPEN_HOLD unchanged (0e0c3d86); row_complete=false; NEVER Complete; Row074 "
        f"left alone; Local+RunPod (no EC2). Evidence: {ROW084_PACKET}; {ROW084_RECEIPT}; "
        f"{ROW084_HARD_FAIL}; {ROW084_DELTA}; {ROW084_HOLD_012}"
    )
    row084_evidence = (
    ...
    delta["ledger_vocabulary_sync"] = {
        ...
        "note": (
            f"Mechanical CSV mutator Row084 Class E prod sparse_motion reclass "
    ...
    print("Notes synced for TRK/ITEM-W64-084; Row074 untouched; observed_class=sparse_motion")
