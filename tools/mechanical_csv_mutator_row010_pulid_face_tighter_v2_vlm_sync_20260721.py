#!/usr/bin/env python3
"""LOCAL PRIMARY CSV mutator: sync TRK/ITEM-W64-010 Notes for PuLID face-tighter-v2 VLM."""
...
ROW010_FTV2_CALIB = (
    f"{EVID}/TRK-W64-010_RUNPOD_C1_PULID_FACE_TIGHTER_V2_CALIB_20260721T100836Z.json"
)
ROW010_FTV2_VLM = (
...
SYNC_MARKER = "synced_by_primary_csv_mutator_row010_pulid_face_tighter_v2_vlm_sync_20260721"
...
def already_synced(notes: str) -> bool:
    return (
        "PuLID FACE_TIGHTER_V2 VLM GATE CLEARED" in notes
        and "face_consistency_mean 0.75" in notes
        and "101156Z" in notes
...
def main() -> None:
    ...
    if already_synced(notes):
        print("tip", tip)
        print("no-op: TRK/ITEM-W64-010 PuLID face-tighter-v2 VLM Notes already synced")
        print("Row074 untouched")
        return

    vlm = load_json(ROW010_FTV2_VLM)
    calib = load_json(ROW010_FTV2_CALIB)
    assert vlm.get("row_complete") is False
    assert calib.get("row_complete") is False
    assert vlm["aggregate_scores"]["face_consistency_mean"] == 0.75
    assert vlm["gate"]["runtime_pass_bounded"] is True
    assert vlm["gate"]["faces_ok"] is True
    assert vlm["prompt_id"] == "0232946c-a157-4cd5-a26f-0e3ce6fda388"
    ...
    row010_notes = (
        ...
        "RunPod PuLID FACE_TIGHTER_V2 (FACE_02 + head-shoulders crop + latina 0.78/0.58) "
        f"VLM GATE CLEARED ({tip}/101156Z): qwen2.5vl:7b side_by_side_panel_v2 "
        "face_consistency_mean 0.75 (n=4; face_01=0.7/face_02=0.85/face_03=0.85/face_04=0.6)/"
        "body_silhouette_mean 0.9/same_person_likelihood_mean 0.6667/"
        "solo_lock_trait_alignment 0.0; faces_ok=true body_ok=true lock_ok=false "
        "runtime_pass_bounded=true; prompt_id 0232946c-a157-4cd5-a26f-0e3ce6fda388; "
        "distinct from exhausted FACE_01/03 envelopes; no invented faces; "
        ...
        f"{ROW010_PULID01_VLM}; {ROW010_LOCKFRONT_VLM}; {ROW010_FTV2_CALIB}; "
        f"{ROW010_FTV2_VLM}"
    )
    ...
    row010_evidence = (
        ...
        f"{ROW010_PULID01_VLM}; {ROW010_LOCKFRONT_VLM}; {ROW010_FTV2_CALIB}; "
        f"{ROW010_FTV2_VLM}; {OLLAMA_VLM_SMOKE}"
    )
    ...
    for rel in (
        ROW010_FTV2_VLM,
        "Plan/Tracker/Evidence/TRK-W64-010_RUNPOD_C1_PULID_FACE_TIGHTER_V2_VLM_20260721T101156Z.json",
        ROW010_FTV2_CALIB,
        "Plan/Tracker/Evidence/TRK-W64-010_RUNPOD_C1_PULID_FACE_TIGHTER_V2_CALIB_20260721T100836Z.json",
    ):
        obj = load_json(rel)
    ...
    print("tip", tip)
    print(
        "synced TRK/ITEM-W64-010 PuLID FACE_TIGHTER_V2 VLM GATE CLEARED Notes; "
        "face_mean 0.75; NEVER Complete; row_complete=false"
    )
    print("Row074 untouched")
