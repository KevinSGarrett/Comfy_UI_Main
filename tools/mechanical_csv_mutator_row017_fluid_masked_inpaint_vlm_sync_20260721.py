#!/usr/bin/env python3
"""LOCAL PRIMARY CSV mutator: sync TRK/ITEM-W64-017 Notes for fluid_masked_inpaint VLM tip."""
...
ROW017_EMISSION = (
    f"{EVID}/TRK-W64-017_RUNPOD_FUTURE_PRODUCER_EMISSION_PACKET_20260720T234848-0500.json"
)
ROW017_CLIMB = f"{EVID}/ROW017_RUNPOD_FUTURE_PRODUCER_EMISSION_CLIMB_20260720T234848-0500.json"
ROW017_VLM_DEEPEN = (
    f"{EVID}/ROW017_RUNPOD_FLUID_MASKED_INPAINT_VLM_DEEPEN_20260720T234941-0500.json"
)
ROW017_VLM_OBS = (
    f"{EVID}/TRK-W64-017_RUNPOD_FLUID_MASKED_INPAINT_VLM_OBSERVATION_20260720T234941-0500.json"
)
ROW017_GLOBAL_REVIEW = (
    "Plan/Instructions/QA/Evidence/Image_Artifact_QA/Row017_Canonical_Global_Reviews/"
    "ROW017_RUNPOD_FLUID_MASKED_INPAINT_20260720T234848-0500_GLOBAL_REVIEW.json"
)
ROW017_VISUAL_QA = (
    "Plan/Instructions/QA/Evidence/Image_Artifact_QA/"
    "ROW017_RUNPOD_FLUID_MASKED_INPAINT_VISUAL_QA_20260720T234848-0500.json"
)
...
SYNC_MARKER = "synced_by_primary_csv_mutator_row017_fluid_masked_inpaint_vlm"
PRODUCER_STAMP = "20260720T234848-0500"
VLM_STAMP = "20260720T234941-0500"
...
def build_notes(land_commit: str, output_sha: str, face_mean: float) -> str:
    return (
        ...
        "fluid_masked_inpaint future-producer GLOBAL_REVIEW pass "
...
def already_synced(notes: str, land_commit: str) -> bool:
    return (
        "fluid_masked_inpaint future-producer GLOBAL_REVIEW pass" in notes
...
def main() -> None:
    tip = git_short()
    # Prove against the fluid land commit, not a later unrelated tip.
    land_commit = subprocess.check_output(
        ["git", "rev-parse", "--short", "a51d8158"], cwd=ROOT, text=True
    ).strip()
    notes = read_row017_notes()
    if already_synced(notes, land_commit):
        print("tip", tip)
        print("no-op: TRK/ITEM-W64-017 fluid_masked_inpaint tip Notes already synced")
    ...
    ledger_vocab = {
        "note": (
            f"Mechanical CSV mutator Row017 fluid_masked_inpaint tip sync from {land_commit}; "
    ...
    for rel in (
        ROW017_VLM_DEEPEN,
        "Plan/Tracker/Evidence/ROW017_RUNPOD_FLUID_MASKED_INPAINT_VLM_DEEPEN_20260720T234941-0500.json",
        ROW017_VLM_OBS,
        "Plan/Tracker/Evidence/TRK-W64-017_RUNPOD_FLUID_MASKED_INPAINT_VLM_OBSERVATION_20260720T234941-0500.json",
        ROW017_EMISSION,
        "Plan/Tracker/Evidence/TRK-W64-017_RUNPOD_FUTURE_PRODUCER_EMISSION_PACKET_20260720T234848-0500.json",
        ROW017_CLIMB,
        "Plan/Tracker/Evidence/ROW017_RUNPOD_FUTURE_PRODUCER_EMISSION_CLIMB_20260720T234848-0500.json",
    ):
        stamp(rel)
    ...
    item_report["notes"] = (
        f"RunPod fluid_masked_inpaint producer+VLM climb {PRODUCER_STAMP}/{VLM_STAMP}: "
    ...
    print("tip", tip)
    print("synced TRK/ITEM-W64-017 fluid_masked_inpaint tip GLOBAL_REVIEW+VLM Notes")
    print("Row074 untouched")
