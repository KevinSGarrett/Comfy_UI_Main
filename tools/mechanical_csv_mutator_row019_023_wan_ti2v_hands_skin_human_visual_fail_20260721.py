#!/usr/bin/env python3
"""LOCAL PRIMARY CSV mutator: Row019/023 Wan TI2V hands/skin climb HUMAN VISUAL FAIL.

I2V from athletic still with reduced motion complexity + explicit hand/skin prompts.
Raw mp4 cleared ≥250KB. Both Ollama VLM and Cursor human frame Read FAIL/REJECT
(mushy/fused hands, plastic skin, near-static). Not Proof_Landed.
"""
...
STAMP = "20260721T035115-0500"
EVIDENCE = f"{EVID}/TRK-W64-019_023_WAN_TI2V_HANDS_SKIN_HUMAN_VISUAL_FAIL_{STAMP}.json"
EVIDENCE_TRACKER = (
    "Plan/Tracker/Evidence/Wave64/"
    f"TRK-W64-019_023_WAN_TI2V_HANDS_SKIN_HUMAN_VISUAL_FAIL_{STAMP}.json"
)
...
PULLBACK_DIR = (
    ...
    "runpod_wan_ti2v_hands_skin_20260721T084451Z"
)
PULLBACK_MP4 = (
    f"{PULLBACK_DIR}/"
    "w64_019_023_runpod_wan_ti2v_hands_skin_20260721T084451Z_00001_.mp4"
)
POD_MP4 = (
...
POLICY_RESULT = (
    f"{EVID}/TRK-W64-019_023_WAN_TI2V_HANDS_SKIN_HUMAN_VISUAL_FAIL_POLICY_{STAMP}.json"
)

PROMPT_ID = "5d9adb26-ce70-46a4-8824-1ba048982cc2"
ARTIFACT_SHA = "18cc5ed71fcbf8b3efe764400415e25e28fb25c7d096dd288bb665774d40ffce"
ARTIFACT_BYTES = 273717
SYNC_MARKER = (
    "synced_by_primary_csv_mutator_row019_023_wan_ti2v_hands_skin_human_visual_fail"
)

STATUS_019 = (
    "Blocked_Product_Visual_Qa_Open_Bounded_Wan_Ti2v_Hands_Skin_Human_Visual_FAIL"
)
STATUS_023 = (
    "Blocked_Video_Frame_Repair_Product_Visual_Qa_Open_Bounded_Wan_Ti2v_"
    "Hands_Skin_Human_Visual_FAIL"
)
DECISION = "runpod_wan_ti2v_hands_skin_human_visual_fail_reject"

BLUNT_DEFECTS = [
    "mushy_fused_blurred_hands_fingers_poorly_separated",
    "plastic_waxy_oversmoothed_skin_no_pore_microtexture",
    "near_static_blink_head_turn_only_motion_weak",
    "stylized_pelvis_v_seam_still_present",
    "flux_hand_skin_loras_present_on_pod_but_not_wan_compatible",
]
...
def sync_fixtures() -> None:
    ...
    for name in (
        "ffprobe.json",
        "vlm_review.json",
        "w64_019_023_hands_skin_20260721T084451Z_vlm.json",
        "w64_019_023_hands_skin_submit_20260721T084451Z.json",
    ):
        p = src / name
        if p.exists():
...
def build_packet(tip: str) -> dict:
    return {
        "schema_version": "1.0",
        "evidence_id": (
            f"TRK-W64-019_023_WAN_TI2V_HANDS_SKIN_HUMAN_VISUAL_FAIL_{STAMP}"
        ),
        "created_utc": "2026-07-21T08:51:15Z",
        "created_iso": "2026-07-21T03:51:15-05:00",
        "claim_tier": "class_e_attempt_fail",
        "status": "CLASS_E_PROOF_ATTEMPT_FAIL_REJECT_HUMAN_VISUAL",
        "verdict": "WAN_TI2V_BOUNDED_HANDS_SKIN_HUMAN_VISUAL_FAIL_REJECT",
        "proof_tier": "RUNPOD_WAN_TI2V_BOUNDED_HANDS_SKIN_HUMAN_VISUAL_FAIL",
        "highest_proof_tier_achieved": (
            "RUNPOD_WAN_TI2V_BOUNDED_HANDS_SKIN_HUMAN_VISUAL_FAIL"
        ),
        "mutation_this_landing": (
            "bounded_runpod_wan_ti2v_hands_skin_human_visual_fail"
        ),
        ...
        "lane": {
            ...
            "motion_complexity_reduced": True,
            "hands_skin_prompt_strengthened": True,
        },
        "generation": {
            "prompt_id": PROMPT_ID,
            "completed": True,
        ...
        "reencode": {
            "performed": False,
            "note": "not needed; raw artifact already 273717B",
        },
        "technical_qa": {
            "performed": True,
            "technical_pass": True,
            ...
            "note": (
                "ffprobe OK 704x1280/81f/h264/3.375s/bit_rate~637kbps. Technical decode "
        ...
        "human_frame_read": {
            ...
            "blunt_human_verdict": (
                "Hands/skin climb FAIL. Across 5 frames: fingers mushy/fused/poorly "
                "separated (worse on left hand against thigh), plastic airbrushed skin "
                "with no pore microtexture, motion reduced to blink/head-tilt (near-static "
                "vs prior crouch). Bytes OK. Do not Proof_Landed. Do not COMPLETE."
            ),
        },
        "visual_qa": {
            "performed": True,
            "pass": False,
            "result": "fail_human_frame_review_and_vlm_fail",
            ...
            "blunt_human_verdict": (
                "Human Cursor frame Read FAIL + Ollama VLM FAIL (hands deformed/fused, "
                "plastic skin, near-static). Do not Proof_Landed. Do not COMPLETE."
            ),
            "note": (
                "Proof_Landed requires human_frame_read=pass; this attempt fails that gate."
            ),
        },
        "vlm_review": {
            "performed": True,
            ...
            "parsed": {
                ...
                "near_static": True,
                "defects": [
                    "Hands appear deformed and fused",
                    "Skin looks oversmoothed",
                ],
                "summary": (
                    "The image exhibits issues with the hands appearing deformed and "
                    "fused, along with an oversmoothed appearance of the skin."
                ),
            },
        ...
        "comfy_regen": {
            "attempted": True,
            "prompt_id": PROMPT_ID,
            "params": "704x1280/81f/40steps/cfg6.0/seed2272509",
            "waited_for_idle": True,
            "did_not_kill_foreign_jobs": True,
            "note": (
                "Queue idle at submit; I2V from wan22_ti2v_fullbody_seed711670301.png; "
                "reduced motion complexity (no crouch); explicit hand anatomy + natural "
                "skin prompt/negative. Did not re-fetch Wan 3/3. Did not touch Row017 "
                "lanes or Row074."
            ),
        },
        "detail_lora_note": {
            ...
            "note": (
                "Flux detail LoRAs cannot be wired into wan_2_2_ti2v_5b_primary_lane; "
                "next climb needs a Wan-compatible detail LoRA or a sharper start still "
                "with already-correct hands/skin."
            ),
        },
        "high_end_note": (
            "Proof_Landed requires human_frame_read=pass. Product visual accept still "
            "needs human/Claude; no COMPLETE."
        ),
        "class_ladder": {
            "class_f": "ASSET_PRESENT retained",
            "class_e": (
                "hands/skin climb raw ≥250KB but HUMAN+VLM visual FAIL/REJECT "
                "— not Proof_Landed"
            ),
        ...
        "summary": (
            f"Hands/skin climb prompt_id={PROMPT_ID} raw {ARTIFACT_BYTES}B "
            f"({Path(PULLBACK_MP4).name}) 704x1280/81f/40steps/cfg6.0. Bytes gate OK; "
            "Ollama qwen2.5vl:7b FAIL; HUMAN Cursor frame Read FAIL (mushy/fused hands, "
            "plastic skin, near-static). Status FAIL/REJECT not Proof_Landed. "
            f"row_complete=false; no COMPLETE; Row074 untouched; EC2 untouched. tip={tip}."
        ),
        "next_fix_advice": (
            "1) Use a different start still with already-sharp separated fingers and "
            "visible skin microtexture (current athletic still hands soft at tips). "
            "2) Do not attach Flux hand/skin LoRAs to Wan — need Wan-native detail LoRA "
            "if available, else stay prompt-only. "
            "3) Prefer medium shot / hands-forward framing over full-body tip-tiny-hands. "
            "4) Keep motion simple but not near-static (breathing + clear blink + small "
            "weight shift without crouch). "
            "5) Keep human_frame_read hard gate; never Proof_Landed on VLM-only."
        ),
        "next_action": (
            "Keep row_complete=false; no COMPLETE; Row074 alone; iterate start still + "
            "framing before another Wan TI2V attempt."
        ),
    }
...
def build_notes(tip: str, row: str) -> str:
    row_bit = (
        ...
        if row == "023"
        else "video pipeline hands/skin climb human visual FAIL; "
    )
    return (
        f"Wan TI2V RunPod hands/skin climb HUMAN VISUAL FAIL/REJECT "
        f"({tip}/{STAMP}): "
        f"prompt_id={PROMPT_ID}; "
        "artifact=w64_019_023_runpod_wan_ti2v_hands_skin_20260721T084451Z_00001_.mp4; "
        f"sha256={ARTIFACT_SHA}; bytes={ARTIFACT_BYTES} (>=250000 OK raw, no pad); "
        "params=704x1280/81f/40steps/cfg6.0/seed2272509; "
        "I2V start=wan22_ti2v_fullbody_seed711670301.png; motion complexity reduced; "
        "Ollama qwen2.5vl:7b VLM FAIL + Cursor human_frame_read FAIL "
        "(mushy/fused hands, plastic/waxy skin, near-static); "
        "not Proof_Landed; claim policy requires human_frame_read=pass; "
        f"{row_bit}"
        "claim_tier=class_e_attempt_fail; "
        "proof_tier=RUNPOD_WAN_TI2V_BOUNDED_HANDS_SKIN_HUMAN_VISUAL_FAIL; "
        "next=different sharper-hand start still or medium-shot framing; "
        "Flux hand/skin LoRAs on pod are NOT Wan-compatible; "
...
def main() -> None:
    ...
    dump_json(
        "Plan/Tracker/Evidence/Wave64/"
        f"TRK-W64-019_023_WAN_TI2V_HANDS_SKIN_HUMAN_VISUAL_FAIL_POLICY_{STAMP}.json",
    ...
    dump_json(
        f"{EVID}/TRK-W64-019_023_WAN_TI2V_HANDS_SKIN_HUMAN_VISUAL_FAIL_NOTES_SYNC_RECEIPT_{STAMP}.json",
        sync_receipt,
    )
    dump_json(
        "Plan/Tracker/Evidence/Wave64/"
        f"TRK-W64-019_023_WAN_TI2V_HANDS_SKIN_HUMAN_VISUAL_FAIL_NOTES_SYNC_RECEIPT_{STAMP}.json",
        sync_receipt,
    )
    print("tip", tip)
    print("synced TRK/ITEM-W64-019/023 Wan TI2V hands/skin HUMAN VISUAL FAIL")
    print("bytes", ARTIFACT_BYTES, "gate OK; VLM FAIL; human FAIL")
