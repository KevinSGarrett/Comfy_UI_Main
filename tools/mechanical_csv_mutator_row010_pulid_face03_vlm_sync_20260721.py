#!/usr/bin/env python3
"""LOCAL PRIMARY CSV mutator: sync TRK/ITEM-W64-010 Notes for PuLID FACE03 VLM."""
...
ROW010_PULID_CALIB = (
    f"{EVID}/TRK-W64-010_RUNPOD_C1_PULID_FACE03_CALIB_20260721T045449Z.json"
)
ROW010_PULID_VLM = (
    f"{EVID}/TRK-W64-010_RUNPOD_C1_PULID_FACE03_VLM_20260721T045654Z.json"
)
OLLAMA_VLM_SMOKE = (
...
SYNC_MARKER = "synced_by_primary_csv_mutator_row010_pulid_face03_vlm_sync_20260721"
...
def already_synced(notes: str) -> bool:
    return (
        "PuLID FACE03 VLM GATE CLEARED" in notes
        and "face_consistency_mean 0.775" in notes
        and "045654Z" in notes
...
def main() -> None:
    ...
    if already_synced(notes):
        print("tip", tip)
        print("no-op: TRK/ITEM-W64-010 PuLID FACE03 VLM Notes already synced")
        print("Row074 untouched")
        return

    vlm = load_json(ROW010_PULID_VLM)
    assert vlm.get("row_complete") is False
    assert vlm["aggregate_scores"]["face_consistency_mean"] == 0.775
    ...
    row010_notes = (
        ...
        f"({FACE_TIGHTER_PROVE}/043808Z): face_consistency_mean 0.475 retained; "
        "RunPod personal-calibration PuLID FACE03 VLM GATE CLEARED "
        f"({tip}/045654Z): qwen2.5vl:7b side_by_side_panel_v2 face_consistency_mean 0.775 "
        "(n=4; face_01=0.8/face_02=0.85/face_03=0.85/face_04=0.6)/body_silhouette_mean 0.9/"
        "same_person_likelihood_mean 0.6667/solo_lock_trait_alignment 0.0; "
        "faces_ok=true body_ok=true lock_ok=false runtime_pass_bounded=true; "
        "PuLID pulid_flux_v0.9.1 + FACE_03 + character1_flux_calibration/balhaw LoRAs "
        "(045449Z) retained; still NONCANONICAL; multi-character USER_AUTHORITY retained "
        "blocked; does NOT clear generic multi-character USER_AUTHORITY chain; "
        ...
        "PORTABLE_MULTI_CHARACTER_REFERENCE_CHAIN_ABSENT|"
        ...
        f"{ROW010_PULID_CALIB}; {ROW010_PULID_VLM}"
    )

    vlm_endpoint_note = (
        f"Durable RunPod Ollama VLM/LLM reviewer UP (e1401895/{tip}): {VLM_ENDPOINT} "
        f"(llava:13b / qwen2.5vl:7b / qwen2.5:7b-instruct); Flux canary VLM smoke "
        f"PASS_WITH_NOTES. Evidence: {OLLAMA_VLM_SMOKE}"
    )

    row010_evidence = (
        f"{ROW010_BLOCKER}; {ROW010_CALIB}; {ROW010_VLM_PANEL_V2}; "
        f"{ROW010_FACE_TIGHTER_CALIB}; {ROW010_FACE_TIGHTER_VLM}; "
        f"{ROW010_PULID_CALIB}; {ROW010_PULID_VLM}; {OLLAMA_VLM_SMOKE}"
    )
    ...
    for rel in (
        ROW010_PULID_VLM,
        "Plan/Tracker/Evidence/TRK-W64-010_RUNPOD_C1_PULID_FACE03_VLM_20260721T045654Z.json",
        ROW010_PULID_CALIB,
        "Plan/Tracker/Evidence/TRK-W64-010_RUNPOD_C1_PULID_FACE03_CALIB_20260721T045449Z.json",
    ):
        obj = load_json(rel)
    ...
    print("tip", tip)
    print(
        "synced TRK/ITEM-W64-010 PuLID FACE03 VLM GATE CLEARED Notes; "
        "panel-v2 + face-tighter retained; NEVER Complete"
    )
    print("Row074 untouched")
