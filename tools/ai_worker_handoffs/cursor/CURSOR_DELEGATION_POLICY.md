...
A Cursor call counts as useful delegation when `handoff_record.json` has `status: PASS`, no blocking `issues`, and a completed classification: `CURSOR_HANDOFF_COMPLETED`, `CURSOR_HANDOFF_COMPLETED_WITH_FINDINGS`, or `CURSOR_HANDOFF_COMPLETED_BLOCKED`. A reported blocker is useful completed diagnosis, not implementation success. The final result must include `status:`, `summary:`, `files inspected:`, `blockers:`, `confidence:`, and `recommended Codex follow-up:`. Promise-style answers such as "I will inspect next" or "next I will read" are incomplete handoffs and must be rerun with a narrower work order or recorded as Cursor unavailable.
...
The canonical wrapper is versioned in `tools/ai_worker_handoffs/cursor` and installed by hash. Cursor uses only plain `gpt-5.3-codex`. Ask/plan remain read-only. Guarded agent mode is allowed only in a registered isolated worktree with `-AllowWrites`, exact `-AllowedPaths`, and exact `-DeclaredAgentCommands`; the wrapper supplies `--force` only through that contract and rejects changes outside the allowed paths. Fast models, Claude models, credential-store reads, Git/GitHub/AWS/Jira mutations, mask promotion, Items/Tracker status mutation, and final-authority decisions remain prohibited. Codex reviews the compact result, diff, and tests and exclusively owns commit, PR, and merge.

Every bounded packet validates gate, worker lane, file count, per-file bytes and SHA-256, aggregate bytes, and repository containment. Safe aliases such as `confirmed`, `pass_with_findings`, and `verified_blocked_as_intended` are normalized. A changed scoped file fails a read-only handoff; guarded implementation changes are checked against exact allowed paths. Unrelated concurrent drift is a warning when hash-bound scope remains intact. Static verification checks wrapper syntax, model policy, Git LFS, status parsing, scope enforcement, credential scrubbing, and drift attribution without making a Cursor request. A live probe is explicit and never scheduled by the combined monitor.
