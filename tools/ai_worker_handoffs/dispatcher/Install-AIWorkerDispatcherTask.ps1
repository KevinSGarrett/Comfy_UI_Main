...
$hiddenHost=Join-Path $root 'Invoke-HiddenPowerShell.vbs'
if(-not(Test-Path -LiteralPath $wscript -PathType Leaf)){throw "Windowless script host missing: $wscript"}
if(-not(Test-Path -LiteralPath $hiddenHost -PathType Leaf)){throw "Installed windowless PowerShell host missing: $hiddenHost"}
$definitions=@(
...
foreach($definition in $definitions){
  ...
  $powershellArguments=@('-NoLogo','-NoProfile','-NonInteractive','-WindowStyle','Hidden','-File',$scriptPath,'-DispatcherRoot',$root)+@($definition.args)
  $quotedArguments=@($hiddenHost,$powershell)+$powershellArguments|ForEach-Object{
    $value=[string]$_
    if($value.Contains('"')){throw "Scheduled task argument contains an unsupported quote: $value"}
    '"'+$value+'"'
  }
  $arguments='//B //NoLogo '+($quotedArguments-join' ')
  if($Apply-and$PSCmdlet.ShouldProcess($definition.name,'Register production worker task')){$action=New-ScheduledTaskAction -Execute $wscript -Argument $arguments;$trigger=New-ScheduledTaskTrigger -Once -At(Get-Date).AddMinutes(1) -RepetitionInterval(New-TimeSpan -Minutes $definition.minutes) -RepetitionDuration(New-TimeSpan -Days 3650);$settings=New-ScheduledTaskSettingsSet -MultipleInstances IgnoreNew -ExecutionTimeLimit(New-TimeSpan -Minutes $definition.limit) -StartWhenAvailable -AllowStartIfOnBatteries -DontStopIfGoingOnBatteries;Register-ScheduledTask -TaskName $definition.name -Action $action -Trigger $trigger -Settings $settings -Description $definition.description -Force|Out-Null}
