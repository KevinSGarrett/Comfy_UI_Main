...
foreach($name in @('ComfyUIMain AI Worker Admission Router','ComfyUIMain AI Worker Cursor Lane','ComfyUIMain AI Worker Claude Lane','ComfyUIMain AI Worker Worktree Lifecycle')){
  ...
  $windowHidden=if($task){@($task.Actions|Where-Object{[IO.Path]::GetFileName([string]$_.Execute)-ieq'wscript.exe'-and[string]$_.Arguments-match'(?i)(?:^|\s)//B(?:\s|$)'-and[string]$_.Arguments-match'(?i)Invoke-HiddenPowerShell\.vbs'}).Count-eq@($task.Actions).Count}else{$false}
  $tasks+=[ordered]@{name=$name;present=($null-ne$task);state=$(if($task){[string]$task.State}else{'MISSING'});last_result=$(if($info){[int]$info.LastTaskResult}else{$null});allows_battery_start=$(if($task){-not[bool]$task.Settings.DisallowStartIfOnBatteries}else{$false});continues_on_battery=$(if($task){-not[bool]$task.Settings.StopIfGoingOnBatteries}else{$false});window_hidden=$windowHidden}
  if(-not$task){$issues+="Scheduled task missing: $name"}elseif($info.LastTaskResult-ne0-and$info.LastRunTime.Year-gt2000){$issues+="Scheduled task failed: $name result=$($info.LastTaskResult)"}elseif($task.Settings.DisallowStartIfOnBatteries-or$task.Settings.StopIfGoingOnBatteries){$issues+="Scheduled task is not battery-resilient: $name"}elseif(-not$windowHidden){$issues+="Scheduled task can open a visible console window: $name"}
}
$packageDrift=$null
