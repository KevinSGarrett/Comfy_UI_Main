...
foreach($name in @('ComfyUIMain AI Worker Admission Router','ComfyUIMain AI Worker Cursor Lane','ComfyUIMain AI Worker Claude Lane','ComfyUIMain AI Worker Worktree Lifecycle')){
  ...
  $tasks+=[ordered]@{name=$name;present=($null-ne$task);state=$(if($task){[string]$task.State}else{'MISSING'});last_result=$(if($info){[int]$info.LastTaskResult}else{$null})}
  if(-not$task){$issues+="Scheduled task missing: $name"}elseif($info.LastTaskResult-ne0-and$info.LastRunTime.Year-gt2000){$issues+="Scheduled task failed: $name result=$($info.LastTaskResult)"}
}
$packageDrift=$null
try{$packageDrift=&(Join-Path $PSScriptRoot 'Test-AIWorkerHandoffPackageDrift.ps1') -PackageRoot (Join-Path $project 'tools\ai_worker_handoffs') -CodexHome (Split-Path -Parent $root)|ConvertFrom-Json;if($packageDrift.status-ne'PASS'){$issues+='Canonical/live worker package drift detected.'}}catch{$issues+="Package drift check failed: $($_.Exception.Message)"}
$qualification=$null
