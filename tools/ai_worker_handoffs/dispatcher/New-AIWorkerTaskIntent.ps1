...
function ConvertTo-IntentRelativePath {
 param([Parameter(Mandatory=$true)][string]$Path)
 if([IO.Path]::IsPathRooted($Path)){
  $full=[IO.Path]::GetFullPath($Path).TrimEnd('\')
  $prefix=$projectRootFull+'\'
  if(-not$full.StartsWith($prefix,[StringComparison]::OrdinalIgnoreCase)){throw "Intent path is outside ProjectRoot: $Path"}
  $Path=$full.Substring($prefix.Length)
 }
 return Normalize-AIWorkerRelativePath $Path
}
$normalizedCandidates=@($CandidatePaths|ForEach-Object{ConvertTo-IntentRelativePath $_}|Sort-Object -Unique)
$normalizedAllowed=@($AllowedPaths|ForEach-Object{ConvertTo-IntentRelativePath $_}|Sort-Object -Unique)
$now=[DateTimeOffset]::Now;$id=('intent_{0}_{1}_{2}'-f $now.UtcDateTime.ToString('yyyyMMddTHHmmssfffZ'),$TaskName,[guid]::NewGuid().ToString('N').Substring(0,8))
$intent=[ordered]@{schema_version=1;artifact_type='ai_worker_task_intent';status='READY';intent_id=$id;created_at=$now.ToString('o');project_root=$projectRootFull;task_name=$TaskName;work_type=$WorkType;authority_class=$AuthorityClass;risk_class=$RiskClass;quality_profile=$QualityProfile;objective=$Objective;candidate_paths=$normalizedCandidates;allowed_paths=$normalizedAllowed;validator_commands=@($ValidatorCommands);acceptance_contract=$(if($AcceptanceContract.Count){$AcceptanceContract}else{[ordered]@{behavior=$Objective;tests='Applicable validators pass.';scope='No undeclared path changes.';regressions='Existing behavior remains covered.';evidence='Worker and validator outputs are hash-bound.';rollback='Reject and remove the isolated worker worktree.';performance='No material regression in the bounded path.';security='No authority or credential boundary is crossed.'}});priority=$Priority;estimated_codex_minutes=$EstimatedCodexMinutes;ttl_minutes=$TtlMinutes;max_attempts=$MaxAttempts;semantic_preflight_required=[bool]$SemanticPreflightRequired;codex_authority_reason=$CodexAuthorityReason}
