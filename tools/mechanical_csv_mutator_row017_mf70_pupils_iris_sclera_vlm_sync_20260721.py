#!/usr/bin/env python3
"""LOCAL PRIMARY CSV mutator: sync TRK/ITEM-W64-017 Notes for mf70_pupils_iris_sclera VLM tip."""
...
ROW017_EMISSION = (
    f"{EVID}/TRK-W64-017_RUNPOD_FUTURE_PRODUCER_EMISSION_PACKET_20260721T040603-0500.json"
)
ROW017_CLIMB = f"{EVID}/ROW017_RUNPOD_FUTURE_PRODUCER_EMISSION_CLIMB_20260721T040603-0500.json"
ROW017_VLM_DEEPEN = (
    f"{EVID}/ROW017_RUNPOD_MF70_PUPILS_IRIS_SCLERA_VLM_DEEPEN_20260721T040655-0500.json"
)
ROW017_VLM_OBS = (
    f"{EVID}/TRK-W64-017_RUNPOD_MF70_PUPILS_IRIS_SCLERA_VLM_OBSERVATION_20260721T040655-0500.json"
)
ROW017_GLOBAL_REVIEW = (
    "Plan/Instructions/QA/Evidence/Image_Artifact_QA/Row017_Canonical_Global_Reviews/"
    "ROW017_RUNPOD_MF70_PUPILS_IRIS_SCLERA_20260721T040603-0500_GLOBAL_REVIEW.json"
)
ROW017_VISUAL_QA = (
    "Plan/Instructions/QA/Evidence/Image_Artifact_QA/"
    "ROW017_RUNPOD_MF70_PUPILS_IRIS_SCLERA_VISUAL_QA_20260721T040603-0500.json"
)
...
SYNC_MARKER = "synced_by_primary_csv_mutator_row017_mf70_pupils_iris_sclera_vlm"
PRODUCER_STAMP = "20260721T040603-0500"
VLM_STAMP = "20260721T040655-0500"
REGION = "mf70_pupils_iris_sclera"
PROMPT_ID = "3c2fdce0-ddc4-4a20-92f5-7bd73b103f5c"
NEXT_ACTION = (
    "Keep Row017 blocked/non-complete; prefer next unused prepared localized lane "
    "(e.g. mf70_nose / mf70_mouth_lips) when queue idle, or Row010 face-tighter personal-calib "
    "re-VLM; leave Row074 alone; no HOLD 090+; no COMPLETE."
)
...
def build_notes(land_commit: str, output_sha: str, face_mean: float) -> str:
    return (
        ...
        f"{REGION} future-producer GLOBAL_REVIEW pass "
        f"VISUAL_QA_PASS_BOUNDED ({PRODUCER_STAMP}) + Ollama qwen2.5vl:7b VLM observation "
        f"vlm_ok ({land_commit}/{VLM_STAMP}); prompt_id {PROMPT_ID}; "
...
def already_synced(notes: str, land_commit: str) -> bool:
    return (
        f"{REGION} future-producer GLOBAL_REVIEW pass" in notes
...
def main() -> None:
    ...
    if already_synced(notes, land_commit):
        print("tip", tip)
        print(f"no-op: TRK/ITEM-W64-017 {REGION} tip Notes already synced")
    ...
    ledger_vocab = {
        "note": (
            f"Mechanical CSV mutator Row017 {REGION} tip sync from {land_commit}; "
    ...
    for rel in (
        ROW017_VLM_DEEPEN,
        f"Plan/Tracker/Evidence/ROW017_RUNPOD_MF70_PUPILS_IRIS_SCLERA_VLM_DEEPEN_{VLM_STAMP}.json",
        ROW017_VLM_OBS,
        f"Plan/Tracker/Evidence/TRK-W64-017_RUNPOD_MF70_PUPILS_IRIS_SCLERA_VLM_OBSERVATION_{VLM_STAMP}.json",
        ROW017_EMISSION,
        f"Plan/Tracker/Evidence/TRK-W64-017_RUNPOD_FUTURE_PRODUCER_EMISSION_PACKET_{PRODUCER_STAMP}.json",
        ROW017_CLIMB,
        f"Plan/Tracker/Evidence/ROW017_RUNPOD_FUTURE_PRODUCER_EMISSION_CLIMB_{PRODUCER_STAMP}.json",
    ):
        stamp(rel)
    ...
    deepen["evidence_id"] = (
        f"ROW017-RUNPOD-MF70-PUPILS-IRIS-SCLERA-VLM-DEEPEN-{VLM_STAMP}"
    )
    deepen.setdefault("decision", {})
    deepen["decision"]["safe_next_action"] = NEXT_ACTION
    ...
    item_report["notes"] = (
        f"RunPod {REGION} producer+VLM climb {PRODUCER_STAMP}/{VLM_STAMP}: "
        f"prompt_id {PROMPT_ID}; GLOBAL_REVIEW pass + Ollama qwen2.5vl:7b VLM observation vlm_ok; "
        f"output sha {output_sha}...; Status remains {ROW017_STATUS}; row_complete=false; "
        "NEVER Complete."
    )
