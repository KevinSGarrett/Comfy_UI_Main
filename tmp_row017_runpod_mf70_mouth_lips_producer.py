#!/usr/bin/env python3
"""Row017 Class E future-producer: RunPod mf70_mouth_lips -> GLOBAL_REVIEW (no COMPLETE/CSV)."""
...
REGION = "mf70_mouth_lips"
CLIENT_ID = f"cursor-row017-mf70-mouth-lips-runpod-{uuid.uuid4().hex[:8]}"
SEED = 210827
DENOISE = 0.03
CFG = 3.1
STEPS = 18
...
MASK = ASSET_DIR / "wave70_mf70_mouth_lips_mask.png"

RUNTIME_DIR = ROOT / "runtime_artifacts" / f"wave64_row017_runpod_mf70_mouth_lips_{STAMP}"
OUT_DIR = (
    ROOT
    / "Plan/Instructions/Operations/Pulled_Back_Artifacts"
    / f"runpod_comfyui_row017_mf70_mouth_lips_{STAMP}"
)
IMG_DIR = OUT_DIR / "images"
PREFIX = f"codex_row017_mf70_mouth_lips_{STAMP}"
SRC_UPLOAD = f"wave64_row017_mf70_mouth_lips_src_{STAMP}.png"
MASK_UPLOAD = f"wave64_row017_mf70_mouth_lips_mask_{STAMP}.png"

POS = (
    "extremely subtle photoreal outer lip texture and contour preservation pass, preserve exact person identity, neutral expression, closed relaxed mouth shape, upper lip shape, lower lip shape, lip line, natural lip texture, teeth hidden or unchanged, tongue hidden, philtrum, nose, chin, cheeks, surrounding skin, gaze direction, eye shape, iris color, pupil size, sclera tone, catchlights, eyebrows, hairline, nearby hair occlusion, face lighting, white blazer, background, and portrait composition unchanged"
)
NEG = (
    "identity change, different person, expression change, smile, frown, open mouth, teeth change, visible teeth added, tongue visible, lip shape change, enlarged lips, thinner lips, swollen lips, asymmetric lips, lipstick, gloss, mouth line mutation, philtrum mutation, chin shape change, cheek shape change, skin patch, over-smoothed skin, plastic skin, waxy skin, skin tone change, nose mutation, gaze change, mismatched eyes, iris color change, glassy eyes, catchlight change, eyebrow change, hair mutation, visible seam, clothing change, background change, watermark, text"
)
...
def package_evidence(receipt: dict, out_path: Path, metrics: dict) -> dict:
    ...
    qa_path = (
        ...
        / f"ROW017_RUNPOD_MF70_MOUTH_LIPS_VISUAL_QA_{STAMP}.json"
    )
    global_path = (
        ROOT
        / "Plan/Instructions/QA/Evidence/Image_Artifact_QA/Row017_Canonical_Global_Reviews"
        / f"ROW017_RUNPOD_MF70_MOUTH_LIPS_{STAMP}_GLOBAL_REVIEW.json"
    )
    exec_path = (
        ROOT
        / "Plan/Instructions/QA/Evidence/Workflow_Runtime"
        / f"W64_RUNPOD_COMFYUI_ROW017_MF70_MOUTH_LIPS_EXECUTE_{STAMP}.json"
    )
    climb_qa = (
        ROOT
    ...
    qa = {
        "schema_version": "1.0",
        "evidence_id": f"ROW017-RUNPOD-MF70-NOSE-VISUAL-QA-{STAMP}",
        "created_at": CREATED,
        ...
        "qa_decision": "pass_bounded_localized_mf70_mouth_lips_runpod_runtime_candidate",
        ...
        "mask": {
            ...
            "filename": "wave70_mf70_mouth_lips_mask.png",
        },
        ...
        "whole_frame_findings": [
            (
                f"Fresh RunPod ComfyUI mf70_mouth_lips masked inpaint on "
            ...
            (
                f"Pixel delta is localized to mf70_mouth_lips bbox {metrics['face_mask_bbox']}; "
                ...
                f"vs {metrics['mask_pixel_count']}-pixel mouth_lips mask."
            ),
            "Direct whole-frame + face/eye/mouth crop inspection: subtle outer lip texture/contour preservation refinement without gray-mask failure, identity drift, wardrobe rewrite, background change, hard mask-edge seam, or gaze/iris rewrite.",
    ...
    global_review = {
        ...
        "required_target_region_check": {
            ...
            "target_region": REGION,
            "findings": [
                "Fresh RunPod mf70_mouth_lips climb remains a bounded outer lip texture/contour preservation refinement with no gray-mask, expression drift, open-mouth/teeth rewrite, face-replacement, gaze drift, or hard-edge failure."
            ],
        },
        "required_non_target_region_scan": {
            ...
            "findings": [
                "Identity, eyes/nose/teeth/chin outside the mouth_lips mask, hair, wardrobe, background, lighting, and framing remain stable; lip refinement stays inside the mouth_lips mask."
            ],
        },
        "hands_face_body_background_contact_lighting_check": {
            ...
            "face": {
                ...
                "inspected": True,
                "status": "pass",
                "reason": "Face identity remains stable; only the outer mouth/lips mask is refined; eyes/nose/chin/cheeks outside the mouth_lips mask must hold.",
            },
            "body": {
                "visibility": "visible",
                "inspected": True,
    ...
    execute = {
        "schema_version": "1.0",
        "evidence_id": f"W64-RUNPOD-COMFYUI-ROW017-MF70-NOSE-EXECUTE-{STAMP}",
        "created_at": CREATED,
        "tracker_id": "TRK-W64-017",
        "item_id": "ITEM-W64-017",
        "mode": "execute",
    ...
    climb = {
        ...
        "highest_proof_tier_achieved": "VISUAL_QA_PASS_BOUNDED",
        "decision": (
            "Advanced Row017 Class E with a real RunPod ComfyUI localized mf70_mouth_lips producer "
        ...
        "remaining_blocker": (
            ...
            "post-clearance RunPod mf70_mouth_lips emission, but product completion still requires broader campaign "
    ...
    packet = {
        ...
        "tracker_id": "TRK-W64-017",
        "item_id": "ITEM-W64-017",
        "mutation_this_landing": "runpod_mf70_mouth_lips_producer_global_review_emission",
        "row_complete": False,
        "product_completion_claimed": False,
...
def main() -> int:
    ...
    shutil.copy2(MASK, OUT_DIR / "mask_mouth_lips.png")
