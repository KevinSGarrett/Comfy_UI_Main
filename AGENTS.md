...
Route mechanical extraction and bounded drafting to signed Cursor workers. Route semantic synthesis, architecture, contradiction review, and test strategy to signed Claude Sonnet 5 workers. During a Codex Desktop shift, Codex retains final Git/GitHub/AWS/Jira/mask/visual-QA/project-state and acceptance authority. During a user-started top-level interactive Cursor shift, that interactive Cursor session is an authorized integration authority under the rules below; this does not expand the authority of dispatched workers.
