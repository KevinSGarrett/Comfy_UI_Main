#!/usr/bin/env python3
"""Deepen Row017 mf70_pupils_iris_sclera producer with Ollama qwen2.5vl:7b (no COMPLETE)."""
...
def http_json(url: str, payload: dict | None = None, timeout: float = 420.0) -> dict:
...
def discover_stamp() -> str:
    ...
    if not cands:
        raise FileNotFoundError("no mf70_pupils_iris_sclera pullback dirs found")
    name = cands[0].name
    prefix = "runpod_comfyui_row017_mf70_pupils_iris_sclera_"
...
def run_vlm(image_path: Path, panel_path: Path | None) -> dict:
    ...
    prompt = (
        "You are a bounded Wave64 Row017 VLM critic for a localized SDXL mf70_pupils_iris_sclera inpaint. Image 1 is the producer output; image 2 (if present) is source|output side-by-side. The mask covers pupil/iris/sclera cores of both eyes; planned change is a very subtle pupil/iris/sclera preservation pass while preserving identity, gaze symmetry, iris color, pupil size, sclera tone, catchlights, eyelids, eyelashes, eyebrows, nearby hair occlusion, mouth, nose bridge, neck, clothing, background, lighting, and framing. Assess whole-frame identity preservation, gaze/iris/catchlight stability, sclera naturalness, mouth stability, hard mask edges, and whether any refinement stayed inside the pupils/iris/sclera mask. Return ONLY compact JSON with keys: frame_ok (bool), identity_preserved (bool), eyes_ok (bool), mouth_ok (bool), background_ok (bool), hard_mask_edge (bool), target_region_refined (bool), global_defects (array of strings), summary (string <= 280 chars), promotion_allowed (always false), row_complete_allowed (always false), uncertainty (0..1). Do not invent media. Do not claim COMPLETE."
    )
    payload = {
        "model": VLM_MODEL,
        "stream": False,
    ...
    try:
        resp = http_json(f"{OLLAMA}/api/chat", payload, timeout=420.0)
...
def main() -> int:
    ...
    if out_path is None or not out_path.exists():
        cands = sorted(
            p
            for p in img_dir.glob(f"codex_row017_mf70_pupils_iris_sclera_{stamp}_*.png")
    ...
    obs_path = WAVE_DIR / f"TRK-W64-017_RUNPOD_MF70_PUPILS_IRIS_SCLERA_VLM_OBSERVATION_{VLM_STAMP}.json"
    obs_trk = TRK_DIR / f"TRK-W64-017_RUNPOD_MF70_PUPILS_IRIS_SCLERA_VLM_OBSERVATION_{VLM_STAMP}.json"
    vlm_path = WAVE_DIR / f"ROW017_RUNPOD_MF70_PUPILS_IRIS_SCLERA_VLM_DEEPEN_{VLM_STAMP}.json"
    vlm_trk = TRK_DIR / f"ROW017_RUNPOD_MF70_PUPILS_IRIS_SCLERA_VLM_DEEPEN_{VLM_STAMP}.json"

    observation = {
        "schema_version": "1.0.0",
        "record_type": "autonomous_reviewer_observation",
        "reviewer_observation_id": f"row017_runpod_mf70_pupils_iris_sclera_vlm_{VLM_STAMP}",
        "revision": "1",
        ...
        "provenance": {
            "producer": "tmp_row017_runpod_mf70_pupils_iris_sclera_vlm_deepen.py",
    ...
    deepen = {
        "schema_version": "1.0",
        "evidence_id": f"ROW017-RUNPOD-MF70-EYEBROWS-VLM-DEEPEN-{VLM_STAMP}",
        "created_iso": CREATED,
        "tracker_id": "TRK-W64-017",
        "item_id": "ITEM-W64-017",
        "mutation_this_landing": "mf70_pupils_iris_sclera_producer_plus_ollama_vlm_deepen",
        ...
        "decision": {
            ...
            "safe_next_action": (
                ...
                "(e.g. mf70_pupils_iris_sclera / mf70_nose / mf70_mouth_lips) when queue idle, or Row010 face-tighter personal-calib re-VLM; "
    ...
    if ITEM.exists():
        ...
        note = (
            f"RunPod mf70_pupils_iris_sclera producer+VLM {stamp}/{VLM_STAMP}: Ollama {VLM_MODEL}; "
        ...
        item["exact_blocker"] = (
            ...
            "RunPod mf70_pupils_iris_sclera emission packaging + bounded VLM observation, but product "
