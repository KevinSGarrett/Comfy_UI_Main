#!/usr/bin/env python3
"""Row017 Class E future-producer: RunPod mf70_nose -> GLOBAL_REVIEW (no COMPLETE/CSV)."""
...
REGION = "mf70_nose"
CLIENT_ID = f"cursor-row017-mf70-nose-runpod-{uuid.uuid4().hex[:8]}"
SEED = 210826
DENOISE = 0.045
CFG = 3.5
STEPS = 18
...
MASK = ASSET_DIR / "wave70_mf70_nose_visible_surface_v4_mask.png"

RUNTIME_DIR = ROOT / "runtime_artifacts" / f"wave64_row017_runpod_mf70_nose_{STAMP}"
OUT_DIR = (
    ROOT
    / "Plan/Instructions/Operations/Pulled_Back_Artifacts"
    / f"runpod_comfyui_row017_mf70_nose_{STAMP}"
)
IMG_DIR = OUT_DIR / "images"
PREFIX = f"codex_row017_mf70_nose_{STAMP}"
SRC_UPLOAD = f"wave64_row017_mf70_nose_src_{STAMP}.png"
MASK_UPLOAD = f"wave64_row017_mf70_nose_mask_{STAMP}.png"

POS = (
    "extremely subtle photoreal nose tip bridge alae and nostril shape preservation pass, preserve exact person identity, nose proportions, tip highlight, alar contour, nostril opening, nearby under-eye and cheek skin texture, philtrum, mouth, eyes, eyebrows, hair occlusion, face lighting, white blazer, background, and portrait composition unchanged"
)
NEG = (
    "identity change, different person, nose reshape, enlarged nose, flattened nose, crooked nose, nostril flare rewrite, tip bulb rewrite, plastic skin, waxy skin, mouth change, lip mutation, eye change, gaze change, eyebrow change, hair mutation, visible seam, skin tone change, clothing change, background change, watermark, text"
)
...
def package_evidence(receipt: dict, out_path: Path, metrics: dict) -> dict:
    ...
    qa_path = (
        ...
        / f"ROW017_RUNPOD_MF70_NOSE_VISUAL_QA_{STAMP}.json"
    )
    global_path = (
        ROOT
        / "Plan/Instructions/QA/Evidence/Image_Artifact_QA/Row017_Canonical_Global_Reviews"
        / f"ROW017_RUNPOD_MF70_NOSE_{STAMP}_GLOBAL_REVIEW.json"
    )
    exec_path = (
        ROOT
        / "Plan/Instructions/QA/Evidence/Workflow_Runtime"
        / f"W64_RUNPOD_COMFYUI_ROW017_MF70_NOSE_EXECUTE_{STAMP}.json"
    )
    climb_qa = (
        ROOT
    ...
    qa = {
        ...
        "qa_decision": "pass_bounded_localized_mf70_nose_runpod_runtime_candidate",
        ...
        "mask": {
            ...
            "filename": "wave70_mf70_nose_visible_surface_v4_mask.png",
        },
        ...
        "whole_frame_findings": [
            (
                f"Fresh RunPod ComfyUI mf70 nose masked inpaint on "
            ...
            (
                f"Pixel delta is localized to mf70 nose bbox {metrics['face_mask_bbox']}; "
                ...
                f"vs {metrics['mask_pixel_count']}-pixel nose mask."
            ),
            "Direct whole-frame + face/eye/mouth crop inspection: subtle nose tip/bridge/alae preservation refinement without gray-mask failure, identity drift, wardrobe rewrite, background change, hard mask-edge seam, or gaze/iris rewrite.",
    ...
    global_review = {
        ...
        "required_target_region_check": {
            ...
            "target_region": REGION,
            "findings": [
                "Fresh RunPod mf70 nose climb remains a bounded nose tip/bridge/alae preservation refinement with no gray-mask, face-replacement, gaze drift, or hard-edge failure."
            ],
        },
        "required_non_target_region_scan": {
            ...
            "findings": [
                "Identity, eyes/mouth outside the nose mask, mouth, hair, wardrobe, background, lighting, and framing remain stable; nose refinement stays inside the nose mask."
            ],
        },
        "hands_face_body_background_contact_lighting_check": {
            ...
            "face": {
                ...
                "inspected": True,
                "status": "pass",
                "reason": "Face identity remains stable; only the nose mask is refined; eyes/mouth/cheeks outside the nose mask must hold.",
            },
            "body": {
                "visibility": "visible",
                "inspected": True,
    ...
    climb = {
        ...
        "highest_proof_tier_achieved": "VISUAL_QA_PASS_BOUNDED",
        "decision": (
            "Advanced Row017 Class E with a real RunPod ComfyUI localized mf70_nose producer "
        ...
        "remaining_blocker": (
            ...
            "post-clearance RunPod mf70_nose emission, but product completion still requires broader campaign "
    ...
    packet = {
        ...
        "tracker_id": "TRK-W64-017",
        "item_id": "ITEM-W64-017",
        "mutation_this_landing": "runpod_mf70_nose_producer_global_review_emission",
        "row_complete": False,
        "product_completion_claimed": False,
...
def main() -> int:
    ...
    shutil.copy2(MASK, OUT_DIR / "mask_nose.png")
